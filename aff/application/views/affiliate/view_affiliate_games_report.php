                ...
                <div class="panel-body">
                    <form id="form-filter" class="form-horizontal" method="GET" onsubmit="return validateForm(); ">
                        <div class="row">
                            ...
                            <div class="col-md-4 col-lg-4">
                                ...
                                <input class="form-control dateInput" id="datetime_range" data-start="#datetime_from" data-end="#datetime_to" data-time="true" style="height:36px;"/>
                            ...
                            <div class="col-md-2 col-lg-2">
                                ...
                                <?php for($i = 12;  $i >= -12; $i--): ?>
                                    ...
                                    <?php else: ?>
                                        <option value="<?php echo $i > 0 ? "+{$i}" : $i ;?>" <?php echo ($i==8) ? 'selected' : ''?>> <?php echo $i > 0 ? "+{$i}" : $i ;?>:00</option>
                                    <?php endif;?>
                                <?php endfor;?>
                            </select>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="total_bet_to" id="total_bet_to" class="form-control input-sm number_only"f
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" name="show_multiselect_filter" id="show-multiselect-filter" value="false">
                            <?php echo lang('Show Multiselect Game Filter')?>
                        </label>
                        <div id="gameTypeCheckBoxes" style="display:none;">
                        <?php foreach($mulitple_select_game_map as $k => $gt ): ?>
                            <fieldset >
                            <h5><b><?php echo $game_apis_map[$k]  ?></b></h5>
                        ...
                        <input type="hidden" id="namespace" name="namespace" value="affiliateBo"/>
                    </div>
                </div>
                <div class="row">
...
<div class="panel panel-primary" >
    ...
    <div class="panel-body" >
        ...
        <table class="table table-bordered table-hover " id="myTable" >
            ...
            <tfoot>
                <tr>
                    ...
                    <th class="text-left text-primary"><span class="total-game-revenue">0.00</span></th>
                    <th class="text-left text-primary"><span class="total-payout-rate">0%</span></th>
                </tr>
               <!--  <tr>
                    <th colspan="17" style="text-align: right;">
                        <?=lang('aff.as24');?>: <span class="sub-total-player">0.00</span><br>
                        <?=lang('report.g09');?>: <span class="sub-total-bet">0.00</span><br>
                        <?=lang('report.g10');?>: <span class="sub-total-win">0.00</span><br>
                        <?=lang('report.g11');?>: <span class="sub-total-loss">0.00</span><br>
                        <?php echo lang('Payout');?>: <span class="sub-total-payout">0.00</span><br>
                        <?php echo lang('sys.payoutrate');?>: <span class="sub-total-payout-rate">0.00</span>
                    </th>
                </tr> -->
        ...
        <div style="min-height:400px;">
            <div id="player-bets-per-game-container" style="overflow-x: auto;font-size:13px;">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var timezoneVal = parseInt($('#timezone').val());
        ...
        $('#timezone').change(function(){
            var timezone = parseInt($(this).val());
        ...
        //Previous Game search
        // if(loadParams.platformId) loadGamePlatformData(loadParams.platformId, loadParams.gameTypeId,loadParams.gameTypeIdMultiple);

        // $('#gamePlatform').on('change', function(){
        //     loadParams.gameTypeId = 0;
        //     loadGamePlatformData(this.value, loadParams.gameTypeId);
        //     disableGameType($(this));
        // });

        //disableGameType($('#group_by'));
...
//---------------------Multi select search start----------------------------
    ...
    //loop through checkboxes and attach each event listener
        ...
        var column_visibility_report = <?=$this->utils->isEnabledFeature('column_visibility_report')? '1':'0'?>;
        ...
        var dataTable = $('#myTable').DataTable({
                ...
                stateSave: (column_visibility_report===1),
                ...
                buttons: [
                    ...
                    ,{
                        ...
                        action: function ( e, dt, node, config ) {
                            ...
                            // $.post(base_url + "export_data/export_game_report_from_aff", d, function(data){
                            //     // utils.safelog(data);
                            ...
                            <?php if($this->utils->isEnabledFeature('export_excel_on_queue')){?>
                                        $("#_export_excel_queue_form").attr('action', base_url + "export_data/export_game_report_from_aff");
                ...
                //console.log(formData);
                    ...
                    $.post(baseUrl + "api/gameReports", data, function(data) {// console.log(data)
                    ...
                    if(data.recordsTotal > 0){
                        ...
                        $('.total-game-revenue').text(addCommas(parseFloat(data.summary[0].total_loss - data.summary[0].total_win).toFixed(2)));
                        $('.total-payout-rate').text(parseFloat((data.summary[0].total_loss-data.summary[0].total_win)/data.summary[0].total_bet*100).toFixed(2) + '%');
                                ...
                                for (var i = 0; i < len; i++) {
                                    ...
                                    for (var n=0; n < hlen;  n++) {
                                        ...
                                        //  console.log(betDetails[existTableHeaders[n]])
                                                ...
                                                tds += '<td class ="text-right" >0.00</td><td class ="text-right text-success" >0.00</td><td class ="text-right text-danger" >0.00</td><td class ="text-right text-warning" >0.00</td>';
                                            }
                                        }
                                    ...
                                    buttons: [
                                        ...
                                        //,
                                        {
                                            ...
                                            action: function ( e, dt, node, config ) {
                                                ...
                                                $.ajax({
                                                    url: base_url + "export_data/game_report_results/true" ,
                                                    type: 'POST',
            ...
            drawCallback : function(data, type, row) {
                // $('#myTable tbody .totalPlayers').on( 'click',function () {
                //     var id = $(this).closest('tr').find('.gamePlatformId').text();
                //     if(id) {
                //         if(id != 'N/A') {
                //             $('#gamePlatform option[value="'+id+'"]').prop('selected', true);
                //         }
                //     }
                //     $('#group_by option[value=player_id]').prop('selected', true);
                //     $('#totalPlayerField').val($(this).text());
                //     $('#search_main').trigger('click');
                // } );
            },
            "rowCallback": function( row, data, index ) {
                // if(data[3] != 'N/A') {
                //     var totalPlayer = $(data[5]).text();
                //     //console.log('remove link');
                //     $('td:eq(5)', row).html( totalPlayer );
                // } else {
                //     //console.log('N/A');
                // }
            },
    ...
    //  Previous Game search
    // function loadGamePlatformData(gamePlatformId, gameTypeId){
    //     var option = '';
    //     if(gamePlatformId == 0) {
    //         option += '<option value=0>'+message.gameType+'</option>';
    //         $('#gameType').html(option);
    //     } else {
    //           $.post(baseUrl + 'game_type/getGameTypeByPlatformId/' + gamePlatformId,function(data){

    //             var dataList = data;
    //             //old filter for game_type  dropdown
    //             option += '<option value=0>'+message.gameType+'</option>';
    ...
    // function disableGameType(gameTypeId) {
    //     if($('#gamePlatform').val() == '0') {
    //         $('#gameType').attr('disabled', 'disabled');
    //     } else {
    //         $('#gameType').removeAttr('disabled');
    //     }
