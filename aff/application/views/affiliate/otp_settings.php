    ...
    <div class="panel-body">
        <div>
            ...
            <div class="otp_init">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <p><?=lang('Backend time')?>: <?=$this->utils->getNowForMysql()?></p>
...
$(document).ready(function(){
    $(".btn_validate_code").click(function(){
        if(confirm("<?=lang('Do you want to validate and enable 2FA')?>?")){
            var url="<?=site_url('/affiliate/validate_and_enable_otp')?>";
    ...
    $(".btn_enable_otp").click(function(){
        ...
        if(confirm(prompt_msg)){
            //load secret info
            var url="<?=site_url('/affiliate/init_otp_secret')?>";
            $.ajax(url,{
                cache: false,
                dataType: 'json',
                method: 'POST',
    ...
    $(".btn_disable_otp").click(function(){
        if(confirm("<?=lang('Do you want to disable 2FA')?>?")){
            var url="<?=site_url('/affiliate/disable_otp')?>";
