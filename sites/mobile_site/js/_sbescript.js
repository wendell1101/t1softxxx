	...
	*/
	...
	var _smartbackend = {
		loginBox: 'header .header__column--user',
		// Announcement Box
		announcementBox:'.news-ticker',
		urlName: document.location.hostname,
		// urlName : /((\w+\.){1,2}|staging\.(\w+\.){1,2})(com|net|org|la|local|cloub|info|biz|co|online|link)(?!\w)/.exec(document.location.hostname)[0].replace('www.','').replace('player.',''),
		siteName : 'default',
		siteType: 'player',
		prefix : window.location.protocol+'//player.',
		protocol: window.location.protocol,
		debugLog: true,

		safelog: function(msg){
			//check exists console.log
			if(_smartbackend.debugLog && typeof(console)!='undefined' && console.log){
				console.log(msg);
			}
		},

		checkAndGoMobile: function($){
			var goM = false;
			...
			if (goM == true){
				if(confirm("检查到您现在使用的是移动设备，是否前往移动端？")){
					_smartbackend.goToMobile($);
				}
			}
		...
		getAffCodeFromParam: function($){
			var code=_smartbackend.getParam($, 'code');
			if(code==''){
				code=_smartbackend.getParam($, 'aid');
				if(code==''){
					code=_smartbackend.getParam($, 'aff');
				}
			}
			return code;
		},
		...
		goToMobile: function($){
			...
			if(arr[0]!='www' && arr[0]!='player'){
				code=arr[0];
			}else{
				code=_smartbackend.getAffCodeFromParam($);
			}

			arr[0]='m';

			var url='http://'+arr.join('.');
			if(code!=''){
				url=url+'?code='+code;
			}
		...
		initLoginBox: function($){
			$(_smartbackend.loginBox).html("<div id='_player_login_area' class='loginbox fn-right'>Loading...</div>");
		},

		init: function($){
			var urlArr=_smartbackend.urlName.split('.');
			if(urlArr.length>0){
				//remove first
				urlArr.shift();
				_smartbackend.urlName=urlArr.join('.');
			}
			//if (this.urlName == 'og.local'){
			//    this.prefix = this.protocol+'//';
			//}
			_smartbackend.playerSiteUrl = _smartbackend.prefix+_smartbackend.urlName;
			_smartbackend.playerMainJsUrl=_smartbackend.playerSiteUrl+"/pub/"+_smartbackend.siteName+"/player_main.js";
			// this.safelog(this.playerSiteUrl+","+this.playerMainJsUrl);
		},

		applyBlockStatus: function($){
			//load country rules
			$.ajax({
				cache: false,
				method: 'GET',
				crossDomain: true,
				dataType: 'jsonp',
				url: _smartbackend.playerSiteUrl+'/pub/check_block_site_status',
				success: function(data){
					_smartbackend.safelog(data);
					if(data && data.success){
						if(data.blocked){
							//go to block page
							window.location.href=data['block_page_url'];
						}
					}
				}
			}
		...
		initAnnouncementBox: function($){

			var announcementBox ='<div class="news-ticker__inner container">';
			announcementBox  +='<span class="news-ticker__label">';
			announcementBox  +='<span class="news-ticker__label__text">Latest News</span>';
			announcementBox  +='</span>';
			announcementBox  +='<div class="news-ticker__view" >';
			announcementBox  +='<ul id="news-ticker" style="border:0px solid yellow;" >';
			announcementBox  +='<li class="news-ticker__entry" >';
			announcementBox  +='<iframe id="marquee-announcement" src="/pub/announcement" scrolling="no" style="height:16px;border:0px solid white;width:1200px;font-size:11px;" ></iframe>';
			announcementBox  +='</li>';
			announcementBox  +='</ul>';
			announcementBox  +='</div>';
			announcementBox  +='</div>';
		...
		getAnnouncementSrc: function($){

			// check if request exists
			var url = "/pub/announcement/";

			var code=_smartbackend.getAffCodeFromParam($);
	...
	(function($){
		...
		_smartbackend.applyBlockStatus($);

		$('#_login_form').attr("action",_smartbackend.playerSiteUrl+"/iframe/auth/login");

		$('#_login_form').submit(function(){
			$("#wrapper-right").toggleClass("active");
		});

		require([_smartbackend.playerMainJsUrl]);

		// var form =  $('#_login_form');
	...
	function callback_after_build_logged_form(container, $){
