<?php
    ...
    $countries_specifications       = $enabled_withdrawal_custom_page['withdrawal_countries_specifications'];
?>
<script type="text/javascript" src="<?=$this->utils->getPlayerCmsUrl('/common/js/player_center/withdraw_custom.js')?>"></script>
<div id="withdrawal" class="panel">
    ...
    <div id="fm-withdrawal-custom" class="panel-body withdrawal-form fmd-step1">
        <form name="withdrawFormCustom" role="form" action="<?=site_url('player_center2/withdraw/verify_custom_withdrawal')?>" method="POST">
            ...
            <?php
                ...
                foreach ($display_items as $key => $item) {
                    switch ($item) {
                            ...
                            <div class="form-group select-player-withdrawal-bank-account select-player-withdrawal-bank-account-wrapper">
                                ...
                                <div class="form-group has-feedback player-withdrawal-bank-account-content">
                                    <div class="col-md-6 current-withdrawal-bank">
                                        ...
                                        <ul class="bank-list">
                                            <li class="active">
                                                <a data-toggle="modal" href="#player-banks">
                                                    <?php if(empty($playerCustoBankDetails)) :
                                                        $bankTypeId = $bankDetailsId = $bankName = $bankAccNum = $bankAccName = $bankAddress = $bankBranch = $mobileNum = $bank_icon_url = '';
                                                        ?>
                                                    <?php else :
                                                        $bankTypeId = $playerDefaultCustoBankDetail['bankTypeId'];
                                                        $bankDetailsId = $playerDefaultCustoBankDetail['playerBankDetailsId'];
                                                        $bankName = $playerDefaultCustoBankDetail['bankName'];
                                                        $bankAccNum = $playerDefaultCustoBankDetail['bankAccountNumber'];
                                                        $bankAccName = $playerDefaultCustoBankDetail['bankAccountFullName'];
                                                        $bankAddress = $playerDefaultCustoBankDetail['bankAddress'];
                                                        $bankBranch = $playerDefaultCustoBankDetail['branch'];
                                                        $bankBranch = $bankBranch ?: '';
                                                        $mobileNum = $playerDefaultCustoBankDetail['phone'];
                                                        $mobileNum = $mobileNum ?: '';
                                                        $bank_icon_url = isset($playerDefaultCustoBankDetail['bank_icon_url']) ? $playerDefaultCustoBankDetail['bank_icon_url'] : '';
                                                    endif; ?>
                                        ...
                                        <div class="hidden">
                                            <div class="dispBankInfo <?= !empty($playerDefaultCustoBankDetail) ? '' : 'hide'; ?>" style="font-size:13px;">
                                    ...
                                    <div class="col-md-6 other-withdrawal-bank">
                                        ...
                                        <p>
                                            <a class="change-withdrawal-bank" href="#player-withdrawal-banks-custom" data-toggle="modal">
                        ...
                        case 'CUSTOM_RECEIVIING_ACCOUNT': ?>

                            <div class="form-group withdraw-custom-receiviing-account-wrapper">
                                <p class="step">
                                    <span class='step-icon'><?=$step++?></span>
                                    <label class="control-label"><?= lang('Please select bank information') ?></label>
                                </p>

                                <!-- country -->
                                <div class="country-list" id="country-list">
                                    <label class="control-label"><?= lang('Your country') ?></label>
                                    <select class="form-control" name="country" id="country" required>
                                        <option value=""><?= lang('Select country') ?></option>
                                        <!-- countries_specifications -->
                                        <?php if(!empty($countries_specifications)):?>
                                            <?php foreach ($countries_specifications as $countries => $val) :?>
                                                <option value="<?=$val['country']?>"><?=lang($countries)?></option>
                                            <?php endforeach;?>
                                        <?php endif;?>
                                    </select>
                                </div>
                                ...
                                <div class="currency-list" id="currency-list">
                                    <label class="control-label"><?= lang('Your currency') ?></label>
                                ...
                                <!-- bank_code -->
                                <!-- https://api-stg.directa24.com/v3/banks?country=BR -->
                                <div class="bank-code-list" id="bank-code-list">
                                    <label class="control-label"><?= lang('Bank code') ?></label>
                                    <select class="form-control" name="bank_code" id="bank_code" required>
                                      <option value=""><?= lang('Select bank code') ?></option>
                                    </select>
                                </div>

                                <!-- account_type -->
                                <div class="account-type-list" id="account-type-list">
                                ...
                                <!-- bank_account -->
                                <div class="input-group bank-account">
                                    <label class="control-label"><?= lang('Type your bank account') ?></label>
                                    <input type="text" class="form-control" id="bank_account" name="bank_account" placeholder="<?= lang('Type your bank account') ?>" required>
                                </div>

                                <!-- beneficiary_name -->
                                <!-- <div class="input-group beneficiary-name">
                                    <input type="text" class="form-control" id="beneficiary_name" name="beneficiary_name" placeholder="<?= lang('Type your name') ?>" required>
                                </div> -->
                                <!-- beneficiary_lastname -->
                                <!-- <div class="input-group beneficiary-lastname">
                                    <input type="text" class="form-control" id="beneficiary_lastname" name="beneficiary_lastname" placeholder="<?= lang('Type your lastname') ?>" required>
                                </div> -->

                                <!-- document_type -->
                                <div class="document-type-list" id="document-type-list">
                                    <label class="control-label"><?= lang('Document type') ?></label>
                                    <select class="form-control" name="document_type" id="document_type" required>
                                      <option value=""><?= lang('Select document type') ?></option>
                                    </select>
                                </div>
                                ...
                                <div class="input-group document-id">
                                    <label class="control-label"><?= lang('Document id') ?></label>
                                    <input type="text" id="document-id" class="form-control " name="document_id"
                                                placeholder="<?= lang('Type your document id') ?>"/>
                                </div>
                            </div>
                            <div class="clearfix"></div>
                            <br>
                        ...
                        case 'WITHDRAWAL_AMOUNT': ?>
                            ...
                            <div class="form-group has-feedback withdrawal-amount-input-wrapper">
                                <p class="step">
                                    <span class='step-icon'><?=$step++?></span>
                                    <label class="control-label custom-withdrawal-amount-label"><?= lang('Please Input Withdrawal Amount') ?></label>
                                </p>
                                ...
                                <?php if(!empty($countries_specifications)){
                                    include __DIR__ . '/exchangerate_conversion.php';
                                }?>
                                ...
                                <div class="input-group">
                                    ...
                                    <?php if($this->CI->config->item('enable_thousands_separator_in_the_withdraw_amount')): ?>
                                        <input type="text" id="thousands_separator_amount" class="form-control" name="thousands_separator_amount"
                                            ...
                                            <?php endif; ?>
                                            style="width:260px"
                                            onChange = "display_thousands_separator_custom_withdrawal()"
                                            oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"
                                            />
                                    <?php endif; ?>
                                ...
                                <div class="helper-content">
                                    ...
                                    <?php if (isset($accumulatedMonthlyWithdrawalAmount)) : ?>
                                        <p class="deposit-limit"><span><?= sprintf(lang('fee.withdraw.accumulatedmonthlyamt'),$this->utils->formatCurrency($accumulatedMonthlyWithdrawalAmount, $this->utils->isEnabledFeature('enable_currency_symbol_in_the_withdraw'))) ?></span></p>
                                    <?php endif; ?>
                                    ...
                                    <?php if(stristr(lang('Min withdraw per transaction'),'%s') && stristr(lang('Max withdraw per transaction'),'%s') && stristr(lang('Daily Limit'),'%s')) :?>
                                        <p class="deposit-limit min">
                                            <?=sprintf(lang('Min withdraw per transaction'), '<span>'.$this->utils->formatCurrency($withdrawSetting['min_withdraw_per_transaction'], $this->utils->isEnabledFeature('enable_currency_symbol_in_the_withdraw')).'</span>'); ?>
                                        </p>
                                        <?php if($showMaxWithdrawalPerTransaction):?>
                                            <p class="deposit-limit max">
                                                <?=sprintf(lang('Max withdraw per transaction'), '<span>'.$this->utils->formatCurrency($withdrawSetting['max_withdraw_per_transaction'], $this->utils->isEnabledFeature('enable_currency_symbol_in_the_withdraw')).'</span>'); ?>
                                            </p>
                                        <?php endif;?>
                                        <?php if($showDailyMaxWithdrawalAmount):?>
                                            <?php if(lang('Daily Limit')) : ?>
                                                <p class="deposit-limit">
                                                    <?=sprintf(lang('Daily Limit'), '<span>'.$this->utils->formatCurrency($withdrawSetting['daily_max_withdraw_amount'], $this->utils->isEnabledFeature('enable_currency_symbol_in_the_withdraw')).'</span>'); ?>
                                                </p>
                                            <?php endif; ?>
                                        <?php endif;?>
                                    <?php else: ?>
                                        ...
                                        <?php else: ?>
                                            ...
                                            <p class="deposit-limit min">
                                                <?=lang('Min withdraw per transaction')?> : <span><?=$this->utils->formatCurrency($withdrawSetting['min_withdraw_per_transaction'], $this->utils->isEnabledFeature('enable_currency_symbol_in_the_withdraw'))?></span>
                                            </p>
                                            <?php if($showMaxWithdrawalPerTransaction):?>
                                                <p class="deposit-limit max">
                                                    <?=lang('Max withdraw per transaction')?> : <span><?=$this->utils->formatCurrency($withdrawSetting['max_withdraw_per_transaction'], $this->utils->isEnabledFeature('enable_currency_symbol_in_the_withdraw'))?></span>
                                                </p>
                                            <?php endif;?>
                                        <?php endif ?>
                                        ...
                                        <?php if($showDailyMaxWithdrawalAmount):?>
                                            <?php if(lang('Daily Limit')) : ?>
                                                <p class="deposit-limit"><?= lang('Daily Limit') ?>: <span><?=$this->utils->formatCurrency($withdrawSetting['daily_max_withdraw_amount'], $this->utils->isEnabledFeature('enable_currency_symbol_in_the_withdraw'))?> </span></p>
                                            <?php endif; ?>
                                        <?php endif;?>
                                    <?php endif ?>
            ...
            <div class="form-group">
                <input class="btn form-control" id="submitBtn" value="<?= lang('Withdraw') ?>" type="submit" />
...
<div class="modal fade" id="player-withdrawal-banks-custom" tabindex="-1" role="dialog">
			...
			<div class="modal-body withdrawal-modal-body">
				<div class="clearfix">
					<div class="col-md-12 fmd-step1">
						<?php if(!empty($playerCustoBankDetails)) : ?>
							<ul class="bank-list bank-list-main" id="currentBankListCustom">
							<?php foreach($playerCustoBankDetails as $bank) : ?>
							...
							<div class="row">
								<div class="text-center col-md-6 top-buffer">
									<button type="button" class="btn btn-primary" id="saveChosenBankNew"><?= lang("lang.save") ?></button>
								</div>
							</div>
						<?php else : ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        var countries_specifications = <?=json_encode($countries_specifications)?>;

        $('#country').change(function(){
            var w_country = $('#country').val();
            disableWithdrawalConversion();
            $('#currency').empty();
            $("#currency").append($('<option>').text("<?= lang('Select currency') ?>").val(''));

            $.each(countries_specifications ,function (k,v) {
                if(v.country == w_country){
                    $.each(v.currency ,function (key,val) {
                        $("#currency").append($('<option>').val(val).text(val));
                    });

                    if (typeof v.symbol != 'undefined') {
                        $("#currency_symbol").val(v.symbol);
                    }

                    if (typeof v.document_type != 'undefined') {
                        $('#document_type').empty().attr('required');
                        $("#document-type-list").show();
                        $("#document_type").append($('<option>').text("<?= lang('Select document type') ?>").val(''));
                        $.each(v.document_type ,function (key,val) {
                            $("#document_type").append($('<option>').val(val).text(val));
                        });
                    }else{
                        $("#document-type-list").hide();
                        $("#document_type").removeAttr('required');
                    }

                    if (typeof v.account_type != 'undefined') {
                        $('#account_type').empty().attr('required');
                        $("#account-type-list").show();
                        $("#account_type").append($('<option>').text("<?= lang('Select account type') ?>").val(''));
                        $.each(v.account_type ,function (key,val) {
                            $("#account_type").append($('<option>').val(key).text(val));
                        });
                    }else{
                        $("#account-type-list").hide();
                        $("#account_type").removeAttr('required');
                    }
                }
            });

            if (typeof w_country != 'undefined' && w_country != '') {
                $.ajax({
                    "url": '/api/getEndpointsApi/banks',
                    'type' : 'POST',
                    'data': {
                        'country' :w_country
                    }
                    ...
                    if (data['success']) {
                        $('#bank_code').val('');
                        $("#bank_code").empty();
                        $("#bank_code").append($('<option>').text("<?= lang('Select bank code') ?>").val(''));

                        $.each(data['result'] ,function (key,val) {
                            $("#bank_code").append($('<option>').val(val.code).text(val.name));
                        });
                    }
            ...
            changeCurrencySymbol($('#currency').val());
        });

        disableWithdrawalConversion();

        $('#currency').change(function(){
            let w_currency = $('#currency').val();
            let w_country = $('#country').val();
            if (typeof w_currency != 'undefined' && w_currency != '') {
                if (w_currency != 'USD') {
                    enableWithdrawalConversion(w_country);
                    changeCurrencySymbol(w_currency);
                }
                else{
                    disableWithdrawalConversion();
                    changeCurrencySymbol(w_currency);
    ...
    $(document).on("change",'#withdrawal_conversion_amt', function(){
        let w_country = $('#country').val();
        let w_amount = $('#withdrawal_conversion_amt').val();

        if ($('#withdrawal_conversion_amt').hasClass('active')) {
            if (typeof w_country != 'undefined' && w_country != '') {
                if (w_amount > 0) {
                    if (fx_rate > 0) {
                        const amt = roundToTwo(w_amount * fx_rate);
                        $('#fm-withdrawal-custom #amount').val(amt.toFixed(2)).change();
                        $('#fm-withdrawal-custom #thousands_separator_amount').val(amt.toFixed(2));
                    }else{
                        withdrawalConverterCurrency(w_country, w_amount).then((data) => {
                            if (data['success']) {
                                const _amount = 1;
                                const res = data['result'];
                                const rate_to_currency = (_amount / res['fx_rate']).toFixed(5);

                                const amt = roundToTwo(w_amount * rate_to_currency);
                                $('#fm-withdrawal-custom #amount').val(amt.toFixed(2)).change();;
                                $('#fm-withdrawal-custom #thousands_separator_amount').val(amt.toFixed(2));
    ...
    if(enable_thousands_separator_in_the_withdraw_amount){
        $('#fm-withdrawal-custom #amount').addClass('hide');
        $(document).on("change", "#fm-withdrawal-custom #amount" , function() {
            $('#fm-withdrawal-custom #amount').focus();
        });
    }
