<?php if (Playerbankdetails::AllowAddBankDetail(Playerbankdetails::WITHDRAWAL_BANK, $bank_details['withdrawal'])): ?>
<div class="bank_account_container col-xs-10 col-xs-push-1 col-sm-10 col-sm-push-1 col-md-6 col-md-push-0">
    <div class="bank_account_info empty">
        ...
        <div class="bank_account_helper">
            ...
            <?php if(in_array(Financial_account_setting::PAYMENT_TYPE_FLAG_BANK, $active_flag['withdrawal']) ||
                     in_array(Financial_account_setting::PAYMENT_TYPE_FLAG_PIX, $active_flag['withdrawal'])) :?>
                <button class="btn btn-default add-bank-account" data-bank-type="withdrawal" data-payment-type-flag="<?=Financial_account_setting::PAYMENT_TYPE_FLAG_BANK?>">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <?=lang('Add Bank Account')?>
                </button>
            <?php endif;?>
            <?php if(in_array(Financial_account_setting::PAYMENT_TYPE_FLAG_EWALLET, $active_flag['withdrawal'])) :?>
                <button class="btn btn-default add-bank-account" data-bank-type="withdrawal" data-payment-type-flag="<?=Financial_account_setting::PAYMENT_TYPE_FLAG_EWALLET?>" style="top: 33%;" >
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <span><?=lang('Add E-wallet Account')?></span>
                </button>
            <?php endif;?>
            <?php if(in_array(Financial_account_setting::PAYMENT_TYPE_FLAG_CRYPTO, $active_flag['withdrawal'])) :?>
                <button class="btn btn-default add-bank-account" data-bank-type="withdrawal" data-payment-type-flag="<?=Financial_account_setting::PAYMENT_TYPE_FLAG_CRYPTO?>" style="top: 66%;" >
                    <i class="fa fa-plus" aria-hidden="true"></i>
...
<?php endif ?>
<?php foreach ((array)$bank_details['withdrawal'] as $player_bank_account): ?>
<div class="bank_account_container col-xs-10 col-xs-push-1 col-sm-10 col-sm-push-1 col-md-6 col-md-push-0">
    ...
    <div class="bank_account_info <?= ($valid_bank) ? '': 'invalid_bank'; ?>">
        ...
        <?php if($valid_bank) :?>
            <div class="bank_account_helper text-center">
                ...
                <?php else: ?>
                    <button class="btn btn-info set-default-bank-account" data-bank-type="withdrawal" data-bank-type-status="<?=$player_bank_account['banktypeStatus']?>" data-bank-account-id="<?=$player_bank_account['playerBankDetailsId']?>" data-err-msg="<?=lang('financial_account.blocked_bank.msg')?>">
                        <?=lang('cashier.110');?>
                    </button>
                <?php endif ?>
                <button class="btn btn-info view-bank-account" data-bank-type="withdrawal" data-bank-type-status="<?=$player_bank_account['banktypeStatus']?>" data-bank-account-id="<?=$player_bank_account['playerBankDetailsId']?>" data-err-msg="<?=lang('financial_account.blocked_bank.msg')?>">
                    <?=lang('View');?>
                </button>
                <!--
                ...
                <?php endif ?>
                -->
                <?php if ($this->operatorglobalsettings->getSettingValueWithoutCache('financial_account_allow_delete') && ($player_bank_account['isDefault'] != TRUE)): ?>
                    <button class="btn btn-info delete-bank-account" data-bank-type="withdrawal" data-bank-account-id="<?=$player_bank_account['playerBankDetailsId']?>">
                        <?=lang('cashier.108');?>
                    </button>
                <?php endif ?>
            </div>
