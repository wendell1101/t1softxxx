...
function load_pt(gpid,status,type){
    ...
    switch (type){
        case 'plba':
            ...
            if(gpid=="420987656201"){
                accountid = '420987656200';
                url = '/liveCasino/nPt?gameCode=plba';
            }
            break;
        default :
            url = '/game/pt?gameCode='+type;
            if(gpid=="420987656202"){
                accountid = '420987656200';
                url = '/game/nPt?gameCode='+type;
            }
            break;
    }
...
function load_bbin_ld(status,gpId,page_site){
    var flag = gameNotify(status);
    if(gpId==9){gpId="350808494186211"};
    if(flag){
        var accid = "350808494186201";
...
function makeModal(gpid, url, title) {
    ...
    html += '<div class="txt"><span id="t_balance" class="fl" style="font-size:22px; color:#ff7800;"></span><span onclick="javascript:window.open(\'/wallet/deposit\');" title="充值" class="btn-deposit">去存款</span></div>';
    html += '</li>';
    html += '<li>';
