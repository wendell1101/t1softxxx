$(document).ready(function(){
    ...
    hack_virtual_keyboard();
    loadPlayerVipGroupDetails();
});

var hack_virtual_keyboard_timer;

function hack_virtual_keyboard(){
    ...
    var check = function(){
        ...
        query.each(function(){
            is_focused = $(this).is(':focus') ? true : is_focused;
        });

        if(is_focused){
            $('.container-fluid').addClass('kbactive');
        }else{
            $('.container-fluid').removeClass('kbactive');
        }
    };

    if(query.length){
        if(hack_virtual_keyboard_timer){
            clearInterval(hack_virtual_keyboard_timer);
        }
        hack_virtual_keyboard_timer = window.setInterval(function(){
            check();
        }, 500);
    }
...
function login_password_toggle() {
    var input_passwd = 'input#dl2';
    var type=$(input_passwd).attr('type');
    var type_new = type == 'password' ? 'text' : 'password';
    $(input_passwd).attr('type', type_new);
}

function showPassWord(){
    if($("#dl2").attr('type') == 'password'){
...
function loadPlayerVipGroupDetails(){
    $.ajax({
        ...
        success: function(data){
            ...
            $('#vip-icon').attr("src", data.current_vip_level.vip_group_lvl_badge);
            ...
            if (!data.disabled_mobile_expbar_vip_level_name) {
