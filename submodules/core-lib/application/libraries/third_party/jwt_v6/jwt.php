<?php
require_once dirname(__FILE__) . '/SignatureInvalidException.php';
require_once dirname(__FILE__) . '/BeforeValidException.php';
require_once dirname(__FILE__) . '/ExpiredException.php';
...
class JWTBetby
{
    ...
    public static function encode($payload, $key, $alg, $keyId = null, $head = null)
    {
        ...
        // print_r($header);exit();
        $segments = array();
        $segments[] = JWTBetby::urlsafeB64Encode(JWTBetby::jsonEncode($header));
        $segments[] = JWTBetby::urlsafeB64Encode(JWTBetby::jsonEncode($payload));
        $signing_input = implode('.', $segments);

        $signature = JWTBetby::sign($signing_input, $key, $alg);
        $segments[] = JWTBetby::urlsafeB64Encode($signature);
    ...
    {
        if (empty(JWTBetby::$supported_algs[$alg])) {
            throw new DomainException('Algorithm not supported');
        }
        list($function, $algorithm) = JWTBetby::$supported_algs[$alg];
        switch ($function) {
            case 'hash_hmac':
