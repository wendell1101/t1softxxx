<?php

defined('BASEPATH') OR exit('No direct script access allowed');

class Migration_add_table_mgw_seamless_wallet_transactions_20230921 extends CI_Migration {
    
	private $tableName = 'mgw_seamless_wallet_transactions';
	public function up() {
		$fields = array(
			'id' => array(
				'type' => 'BIGINT',
				'null' => false,
				'auto_increment' => TRUE,
            ),
            'player_id' => array(
				'type' => 'VARCHAR',
				'constraint' => '100',
				'null' => true,
			),
			'merchant_id' => array(
				'type' => 'VARCHAR',
				'constraint' => '100',
				'null' => true,
			),
            'site_id' => array(
				'type' => 'VARCHAR',
				'constraint' => '100',
				'null' => true,
			),
            'firm_id' => array(
				'type' => 'VARCHAR',
				'constraint' => '50',
				'null' => true,
			),
            'game_id' => array(
				'type' => 'INT',
				'constraint' => '10',
				'null' => false,
            ),
            'game_class' => array(
				'type' => 'VARCHAR',
				'constraint' => '50',
				'null' => true,
			),   
            'risk' => array(
				'type' => 'VARCHAR',
				'constraint' => '50',
				'null' => true,
			),   
            'game_round_id' => array(
				'type' => 'VARCHAR',
				'constraint' => '64',
				'null' => true,
			),   
            'wager_id' => array(
				'type' => 'VARCHAR',
				'constraint' => '64',
				'null' => true,
			),   
            'gold' => array(
                'type' => 'double',
                'null' => true,
            ), 
			'vgold' => array(
                'type' => 'double',
                'null' => true,
            ),  
			'win_gold' => array(
                'type' => 'double',
                'null' => true,
            ),  
			'unused_gold' => array(
                'type' => 'double',
                'null' => true,
            ),  
			'commission_gold' => array(
                'type' => 'double',
                'null' => true,
            ),
			'action' => array(
                'type' => 'VARCHAR',
				'constraint' => '50',
                'null' => true,
            ),   
            'result' => array(
                'type' => 'VARCHAR',
				'constraint' => '10',
				'null' => true,
            ),   
            'order_date' => array(
                'type' => 'DATETIME',
				'null' => true,
            ),  
            'add_date' => array(
				'type' => 'DATETIME',
				'null' => true,
			),   
            'result_date' => array(
				'type' => 'DATETIME',
				'null' => true,
            ), 
            'freegame' => array(
				'type' => 'VARCHAR',
				'constraint' => '20',
				'null' => true,
			), 
            'utoken' => array(
                'type' => 'VARCHAR',
				'constraint' => '100',
                'null' => true,
			),   
            'uid' => array(
				'type' => 'VARCHAR',
				'constraint' => '100',
				'null' => true,
			),  
            'ip' => array(
				'type' => 'VARCHAR',
				'constraint' => '100',
				'null' => true,
			),  
            'device' => array(
				'type' => 'VARCHAR',
				'constraint' => '20',
				'null' => true,
			), 
			
		
			# SBE additional info
            'currency' => array(
				'type' => 'VARCHAR',
				'constraint' => '10',
				'null' => true,
			),
            'trans_type' => array(
				'type' => 'VARCHAR',
				'constraint' => '20',
				'null' => true,
			),
            'trans_status' => array(
				'type' => 'INT',
				'constraint' => '10',
				'null' => false,
            ),   
			'balance_adjustment_amount' => array(
                'type' => 'double',
                'null' => true,
            ),
            'balance_adjustment_method' => array(
				'type' => 'VARCHAR',
				'constraint' => '50',
				'null' => true,
			), 
            'before_balance' => array(
                'type' => 'double',
                'null' => true,
            ), 
            'after_balance' => array(
                'type' => 'double',
                'null' => true,
            ), 
            'response_result_id' => array(
                'type' => 'INT',
                'constraint' => '11',
                'null' => true,
            ),            
			'external_uniqueid' => array(
				'type' => 'VARCHAR',
				'constraint' => '100',
				'null' => true,
            ),
            'game_platform_id' => array(
                'type' => 'INT',
                'constraint' => '11',
                'null' => true,
            ),
            'elapsed_time' => array(
                'type' => 'INT',
                'null' => true,
            ),
            'created_at DATETIME DEFAULT CURRENT_TIMESTAMP' => array(
                'null' => false,
            ),
            'updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP' => array(
                'null' => false,
            ),
		);

		if(!$this->db->table_exists($this->tableName)){
			$this->dbforge->add_field($fields);
			$this->dbforge->add_key('id', TRUE);
			$this->dbforge->create_table($this->tableName);
			# Add Index
	        $this->load->model('player_model');	        
            $this->player_model->addIndex($this->tableName, 'idx_player_id', 'player_id');	
            $this->player_model->addIndex($this->tableName, 'idx_game_id', 'game_id');	        
            $this->player_model->addIndex($this->tableName, 'idx_trans_type', 'trans_type');	                    
            $this->player_model->addIndex($this->tableName, 'idx_updated_at', 'updated_at');	        
            $this->player_model->addIndex($this->tableName, 'idx_trans_status', 'trans_status');	        
            $this->player_model->addIndex($this->tableName, 'idx_game_platform_id', 'game_platform_id');	        
	        $this->player_model->addUniqueIndex($this->tableName, 'idx_external_uniqueid', 'external_uniqueid');
	    }
	}

	public function down() {
		if(!$this->db->table_exists($this->tableName)){
			$this->dbforge->drop_table($this->tableName);
		}
	}
}
