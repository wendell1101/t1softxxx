...
class Migration_update_logged_template_on_static_sites_20151120 extends CI_Migration {

	private $tableName = 'static_sites';

	public function up() {
		$tmpl = <<<EOD
<div class="login-member fn-left"><i class="icon icon-member"></i><a href="javascript:void(0)" class='_player_username'><%- default_prefix_for_username+playerName %></a></div>
	...
	<a href="javascript:void(0)" class='_player_messages'><%- langText.header_messages %> (<%- ui.messageCount %>)</a></div>
...
<div class="login-member fn-left"><i class="icon icon-member"></i><a href="javascript:void(0)" class='_player_username'><%- default_prefix_for_username+playerName %></a></div>
	...
	<a href="javascript:void(0)" class='_player_memcashier'><%- langText.header_memcashier %></a>
	<a href="javascript:void(0)" class='_player_report'><%- langText.header_report %></a>
	<a href="javascript:void(0)" class='_player_messages'><%- langText.header_messages %></a></div>
	<a class="ui-btn ui-btn-logout fn-left" href="<%- ui.logoutUrl %>" target="<%- ui.logoutIframeName %>"><%- langText.button_logout %></a>
	<iframe name="<%- ui.logoutIframeName %>" id="<%- ui.logoutIframeName %>" width="0" height="0" border="0" style="display:none;border:0px;width:0px;height:0px;"></iframe>
EOD;
		$data = array('logged_template' => $tmpl);
