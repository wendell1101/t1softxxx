...
class CI_DB_active_record extends CI_DB_driver {
		...
		foreach ((array) $from as $val) {
			if (strpos($val, ',') !== FALSE) {
				foreach (explode(',', $val) as $v) {
					...
					$this->ar_from[] = $this->_protect_identifiers($v, TRUE, FALSE, FALSE);

					if ($this->ar_caching === TRUE) {
						$this->ar_cache_from[] = $this->_protect_identifiers($v, TRUE, FALSE, FALSE);
	...
	public function join($table, $cond, $type = '') {
		...
		if (preg_match('/([\w\.]+)([\W\s]+)(.+)/', $cond, $match)) {
			$match[1] = $this->_protect_identifiers($match[1], TRUE, FALSE);
			$match[3] = $this->_protect_identifiers($match[3], TRUE, FALSE);
	 ...
	 * @return	object
	 */
	public function get($table = '', $limit = null, $offset = null, $_sqlmode=null) {
		if ($table != '') {
			$this->_track_aliases($table);
			$this->from($table);
		}
		...
		$result = $this->query($sql, false, true, $_sqlmode);
		$this->_reset_select();
		return $result;
	}

	public function get_unbuffered_mysql($table = '', $limit = null, $offset = null) {

		return $this->get($table, $limit, $offset, MYSQLI_USE_RESULT);
	}
