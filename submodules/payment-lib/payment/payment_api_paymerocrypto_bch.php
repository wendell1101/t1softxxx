 ...
 */
class Payment_api_paymerocrypto_bch extends Abstract_payment_api_paymerocrypto {

    public function getPlatformCode() {
        return PAYMEROCRYPTO_BCH_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paymerocrypto_bch';
    }

    protected function getCoin() {
        return $this->getSystemInfo('currency', self::COIN_BCH);
    }
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        if (isset($cryptoRate->status)) {
            if ($cryptoRate->status == self::RETURN_SUCCESS_CODE) {
                ...
                $this->utils->debug_log('=====================get cryptoRate bch success rate', $exchangeRate,$rate);
            } else {
                $this->utils->debug_log('=====================get cryptoRate bch fail');
            }
        }else{
            $this->utils->debug_log('=====================not get cryptoRate bch');
        }
        ...
        #----Create Address----
            ...
            $this->CI->utils->debug_log('=====================paymerocrypto bch generatePaymentUrlForm params', $params);

            $response = $this->submitPostForm($create_address_url, $params, true, $secure_id);
            $response = json_decode($response, true);
            $this->CI->utils->debug_log('=====================paymerocrypto bch response', $response);
            ...
            if($response){
                ...
                if($response['status'] == self::RETURN_SUCCESS_CODE){
                    ...
                    $this->utils->debug_log('=====================paymerocrypto bch deposit_notes', $deposit_notes);
    ...
    protected function handlePaymentFormResponse($handle) {
        ...
        $data['Bitcoin Cash']    = $handle['crypto'];
        $data['Rate']            = $handle['current_rate'];
        $this->CI->utils->debug_log("=====================paymerocrypto bch handlePaymentFormResponse params", $data);
