 ...
 */
abstract class Abstract_payment_api_hadespay_ewallet extends Abstract_payment_api {
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        $params['additionalInfo'] = array(
            ...
            "successRedirectUrl" => $this->getSystemInfo('successRedirectUrl'),
        );
        $this->configParams($params, $order->direct_pay_extra_info);


        $this->CI->utils->debug_log('=====================hadespay_ewallet generatePaymentUrlForm params', $params);
    ...
    protected function processPaymentUrlFormRedirect($params) {
        ...
        $this->CI->utils->debug_log('=====================hadespay_ewallet processPaymentUrlFormRedirect response', $response);

        if(isset($response['responseCode']) && $response['responseCode'] == self::RESPONSE_SUCCESS) {
            return array(
                'success' => true,
                'type' => self::REDIRECT_TYPE_URL,
                'url' => $response['responseData']['eWallet']['url']
            );
        }
    ...
    private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        $this->CI->utils->debug_log("=====================hadespay_ewallet callbackFrom $source params", $params);


        if(empty($params)){
            $raw_post_data = file_get_contents('php://input', 'r');
            $params = json_decode($raw_post_data, true);
            $this->CI->utils->debug_log("=====================hadespay_ewallet json_decode params", $params);
        }
    ...
    private function checkCallbackOrder($order, $fields, &$processed = false) {
        ...
        //         $this->writePaymentErrorLog("=====================hadespay_ewallet checkCallbackOrder Missing parameter: [$f]", $fields);
        //         return false;
        //     }
        // }
        ...
        //     $this->writePaymentErrorLog('=====================hadespay_ewallet checkCallbackOrder Signature Error', $fields);
        //     return false;
        // }
        ...
        if ($fields['responseData']['status'] != self::CALLBACK_SUCCESS) {
            $this->writePaymentErrorLog("======================hadespay_ewallet checkCallbackOrder Payment status is not success", $fields);
            return false;
        }

        if ($fields['responseData']['amount'] != $this->convertAmountToCurrency($order->amount)) {
            $this->writePaymentErrorLog("======================hadespay_ewallet checkCallbackOrder Payment amount is wrong, expected [$order->amount]", $fields);
            return false;
        }

        if ($fields['responseData']['merchantRef'] != $order->secure_id) {
            $this->writePaymentErrorLog("======================hadespay_ewallet checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }
    ...
    public function sign($params) {
        ...
        $this->CI->utils->debug_log('=====================hadespay_ewallet sign signStr', $signStr);
    ...
    protected function processCurl($params, $url) {
        ...
        curl_close($ch);
        $this->CI->utils->debug_log('=====================hadespay_ewallet', $key, $token, $signature, $params);
        ...
        $this->CI->utils->debug_log('=====================hadespay_ewallet processCurl decoded response', $response);
        return $response;
    }
}