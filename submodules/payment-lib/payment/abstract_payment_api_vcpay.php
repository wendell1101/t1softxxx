 ...
 * * VCPAY_GPC_BANKCARD_PAYMENT_API, ID: 5255
 *
 * Required Fields:
 * * URL
 * * Account
 ...
 */
abstract class Abstract_payment_api_vcpay extends Abstract_payment_api {
    const TYPE_SELL = '1';
    const TYPE_BUY = '2';
    const CRYPTO_GPC = 'GPC';
    ...
    const RETURN_SUCCESS_CODE = '{"result":"SUCCESS"}';
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        $params['count']  = $this->convertAmountToCurrency($amount);
        $params['phone'] = $this->getSystemInfo('account');
        $params['user_num'] = $order->secure_id;
    ...
    protected function processPaymentUrlFormUrl($params) {
        $response = $this->submitPostForm($this->getSystemInfo('url'), $params, false, $params['user_num']);
        $response = json_decode($response, true);
        $this->CI->utils->debug_log('=====================vcpay processPaymentUrlFormQRCode response', $response);
        // exit;

        if($response['status'] == self::RESULT_CODE_SUCCESS && isset($response['trade_url']) && isset($response['trx_sn'])) {
            return array(
                'success' => true,
                'type' => self::REDIRECT_TYPE_URL,
                'url' => $response['trade_url'].'?trx_sn='.$response['trx_sn'],
            );
        }
            ...
            return array(
                ...
                'message' => 'status: '.$response['status'].'=> '.$response['msg']
            );
        }
        else {
            return array(
    ...
    private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        if ($orderStatus == Sale_order::STATUS_BROWSER_CALLBACK || $orderStatus == Sale_order::STATUS_SETTLED) {
            ...
            $this->CI->sale_order->updateExternalInfo($order->id, $params['trx_sn'], '', null, null, $response_result_id);
            if ($source == 'browser') {
    ...
    private function checkCallbackOrder($order, $fields, &$processed = false) {
        ...
        if ($fields['count'] != $this->convertAmountToCurrency($order->amount)) {
