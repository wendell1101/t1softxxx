 ...
 * PGTIANCI
 *
 * PGTIANCI_PAYMENT_API, 5827
 * PGTIANCI_QRCODE_PAYMENT_API, 5828
 * PGTIANCI_MOMO_PAYMENT_API, 5829
 * PGTIANCI_ZALOPAY_PAYMENT_API, 5830
 *
 * Required Fields:
 * * URL
 * * Key
 ...
 * @copyright 2022 tot
 */
abstract class Abstract_payment_api_pgtianci extends Abstract_payment_api {
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        $this->CI->utils->debug_log("=====================pgtianci generatePaymentUrlForm", $params);
    ...
    protected function processPaymentUrlFormRedirect($params) {
        ...
        if (empty($response)) {
            ...
            $this->CI->utils->debug_log("=====================pgtianci raw_post_data", $raw_post_data);
            $response = json_decode($raw_post_data, true);
            $this->CI->utils->debug_log("=====================pgtianci json_decode params", $params);
        }
        $this->CI->utils->debug_log('=====================pgtianci processCurl decoded response', $response);
    ...
    protected function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        $this->CI->utils->debug_log("=====================pgtianci callbackFrom $source params", $params);
    ...
    private function checkCallbackOrder($order, $fields, &$processed = false) {
        ...
        foreach ($requiredFields as $f) {
            if (!array_key_exists($f, $fields)) {
                $this->writePaymentErrorLog("=====================pgtianci checkCallbackOrder field missing '$f'");
                return false;
            }
        }
        ...
        if ($fields['state'] != self::STATUS_SUCCESSFUL) {
            $this->writePaymentErrorLog("=====================pgtianci checkCallbackOrder Payment status is not success", $fields);
            return false;
        }

        // Check amount (cash)
        if ($fields['amount'] != $this->convertAmountToCurrency($order->amount)) {
            $this->writePaymentErrorLog('=====================pgtianci checkCallbackOrder amount mismatch', [ 'expected' => $order->amount, 'returned' => $cb_result['amount'] ], $fields);
            return false;
        }

        // Check order ID (customer_order_no)
        if ($fields['out_trade_no'] != $order->secure_id) {
            $this->writePaymentErrorLog('=====================pgtianci checkCallbackOrder order ID mismatch', [ 'expected' => $order->secure_id, 'returned' => $cb_result['orderId'] ], $fields);
            return false;
        }
    ...
    protected function processCurl($params) {
        ...
        $this->CI->utils->debug_log('=====================pgtianci processCurl response', $content);
