 ...
 */
abstract class Abstract_payment_api_fly2pay extends Abstract_payment_api {
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        $this->CI->utils->debug_log("=====================fly2pay generatePaymentUrlForm params", $params);
    ...
    protected function processPaymentUrlFormPost($params) {
        ...
        $this->CI->utils->debug_log("=====================fly2pay processPaymentUrlFormPost response", $response);
        ...
        if($response['errCode'] == self::RESULT_CODE_SUCCESS && $response['status'] == self::RESULT_MSG_SUCCESS) {
            ...
            return array(
                ...
                'message' => '['.$response['errCode'].']: '.$response['error']
            );
        }
        else {
            return array(
    ...
    private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        $this->CI->utils->debug_log("=====================fly2pay callbackFrom $source params", $params);

        if($source == 'server' ){
            if(empty($params)){
                $raw_post_data = file_get_contents('php://input', 'r');
                $this->CI->utils->debug_log("=====================fly2pay raw_post_data", $raw_post_data);
    ...
    private function checkCallbackOrder($order, $fields, &$processed = false) {
        ...
        foreach ($requiredFields as $f) {
            if (!array_key_exists($f, $fields)) {
                $this->writePaymentErrorLog("=====================fly2pay checkCallbackOrder Missing parameter: [$f]", $fields);
                return false;
            }
        }
        ...
        if (!$this->validateSign($fields)) {
            $this->writePaymentErrorLog("======================fly2pay checkCallbackOrder Signature Error", $fields);
            return false;
        }
        ...
        if ($fields['deposit_amount'] != $check_amount) {
            $this->writePaymentErrorLog("======================fly2pay checkCallbackOrder Payment amount is wrong, expected <= ". $check_amount, $fields);
            return false;
        }

        if ($fields['order_id'] != $order->secure_id) {
            $this->writePaymentErrorLog("======================fly2pay checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }
