 ...
 * BITGO
 *
 * * BITGO_ETH_WITHDRAWAL_PAYMENT_API, ID: 5135
 * *
 * Required Fields:
 * * URL
 *
 ...
 */
class Payment_api_bitgo_eth_withdrawal extends Abstract_payment_api_bitgo {
    public function getPlatformCode() {
        return BITGO_ETH_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'bitgo_eth_withdrawal';
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId, $data=null) {
        ...
        if(!array_key_exists($bank, $this->getBankInfo())) {
            $this->CI->utils->debug_log('======================================bitgo_eth withdrawal bank whose bankTypeId=[$bank] is not supported by bitgo_eth');
            return array('success' => false, 'message' => 'Bank not supported by bitgo_eth');
        }
        ...
        #----Unlock----
            ...
            $this->CI->utils->debug_log('=====================bitgo_eth submitWithdrawRequest expires', $expires);
        ...
        #----Send Transaction----
            ...
            if ($this->getSystemInfo("use_realtime_currency")) {
               ...
               $params['amount'] = $this->currencyFormat($order['extra_info'], self::WEI);
            }
            ...
            $this->CI->utils->debug_log('======================================bitgo_eth submitWithdrawRequest decoded Result', $decodedResult);

        return $decodedResult;
    }

    public function decodeResult($result) {
        $this->utils->debug_log("=========================bitgo_eth decodeResult json decoded", $result);

        if(isset($result['error'])){
            $message = "Bitgo eth response failed. [".$result['name']."]: ".$result['error'];
            return array('success' => false, 'message' => $message);
        }
        else if(isset($result['transfer']['txid'])) {
            $txid    = $result['transfer']['txid'];
            $ether = abs($result['transfer']['value']);
            $message = "Bitgo eth withdrawal successful. Txid: ". $txid .", Transfered: ".$ether."/wei";
    ...
    public function getBankInfo() {
        $bankInfo = array();
        $bankInfoArr = $this->getSystemInfo("bitgo_eth_bank_info");
        if(!empty($bankInfoArr)) {
            foreach($bankInfoArr as $system_bank_type_id => $bankInfoItem) {
                $bankInfo[$system_bank_type_id] = $bankInfoItem;
            }
            $this->utils->debug_log("==================getting bitgo_eth bank info from extra_info: ", $bankInfo);
        } else {
            $bankInfo = array(
                '47'  => 'ETH'
            );
            $this->utils->debug_log("=======================getting bitgo_eth bank info from code: ", $bankInfo);
        }
        return $bankInfo;
    }
}