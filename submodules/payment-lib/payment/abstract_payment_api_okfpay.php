 ...
 */
abstract class Abstract_payment_api_okfpay extends Abstract_payment_api {
	const RETURN_SUCCESS_CODE = 'success';
	const RETURN_FAILED_CODE = 'failed';
	const OPSTAT_SUCCESS = '2';
	const VERSION = '1.0';
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$params['partner'] = $this->getSystemInfo("okfpay_partner");
		$params['orderid'] = strtolower($order->secure_id); # Documentation section 3.3 mention all parameter should be in lower case
		$params['payamount'] = $this->convertAmountToCurrency($amount);
		$params['payip'] = $this->getClientIP();
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		...
		$requiredFields = array(
			'version', 'partner', 'orderid', 'payamount', 'opstate', 'sign'
		);
		...
		if (
			...
			$this->convertAmountToCurrency(floatval($fields['payamount']))
		) {
	...
	public function getBankListInfoFallback() {
		return array(
			...
			array('label' => '渤海银行', 'value' => 'BOHAI'),
			array('label' => '上海农商', 'value' => 'SHNS'),
			array('label' => '银联支付', 'value' => 'UNION'),
			array('label' => '支付宝', 'value' => 'ALIPAY'),
			array('label' => '财付通', 'value' => 'TENPAY'),
			array('label' => '微信支付', 'value' => 'WECHAT'),
		);
	}
	...
	private function prepareSign($data) {
		$array = array();
		$keys = array('version', 'partner', 'orderid', 'payamount', 'payip', 'notifyurl', 'returnurl', 'paytype', 'remark');
		foreach ($keys as $key) {
			array_push($array, $key . '=' . (isset($data[$key]) ? $data[$key] : ''));
		}
	...
	public function sign($data) {
		...
		$signature = MD5($dataStr . "&key=" . $this->getSystemInfo('key'));
		return $signature;
	}

	public function prepareVerify($data) {
		$array = array();
		$keys = array('version', 'partner', 'orderid', 'payamount', 'opstate', 'orderno', 'okfpaytime', 'message', 'paytype', 'remark');
		foreach ($keys as $key) {
			array_push($array, $key . '=' . (isset($data[$key]) ? $data[$key] : ''));
		}
		return implode($array, '&');
