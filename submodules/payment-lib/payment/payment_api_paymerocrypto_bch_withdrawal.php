 ...
 */
class Payment_api_paymerocrypto_bch_withdrawal extends Abstract_payment_api_paymerocrypto {
    public function getPlatformCode() {
        return PAYMEROCRYPTO_BCH_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paymerocrypto_bch_withdrawal';
    }

    protected function getCoin() {
        return $this->getSystemInfo('coin', self::COIN_BCH);;
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId, $data=null) {
        ...
        if(!array_key_exists($bank, $this->getBankInfo())) {
            $this->CI->utils->debug_log('======================================paymerocrypto_bch withdrawal bank whose bankTypeId=[$bank] is not supported by paymerocrypto_bch');
            return array('success' => false, 'message' => 'Bank not supported by paymerocrypto_bch');
        }
        ...
        #https://service-api.paymero.io/v1/crypto/withdrawal
            ...
            $this->CI->utils->debug_log('=====================paymerocrypto_bch withdrawal generatePaymentUrlForm params', $params, $withdrawal_url);

            list($response, $response_result) = $this->submitPostForm($withdrawal_url, $params, true, $transId, true);

            $response = json_decode($response, true);

            $this->CI->utils->debug_log('=====================paymerocrypto_bch withdrawal response', $response);
            ...
            $this->CI->utils->debug_log('======================================paymerocrypto_bch withdrawal submitWithdrawRequest decoded Result', $decodedResult);
    ...
    public function decodeResult($result) {
        ...
        $this->utils->debug_log("=========================paymerocrypto_bch withdrawal decodeResult", $result);
    ...
    public function getBankInfo() {
        $bankInfo = array();
        $bankInfoArr = $this->getSystemInfo("paymerocrypto_bch_bank_info");
        if(!empty($bankInfoArr)) {
            foreach($bankInfoArr as $system_bank_type_id => $bankInfoItem) {
                $bankInfo[$system_bank_type_id] = $bankInfoItem;
            }
            $this->utils->debug_log("==================getting withdrawal paymerocrypto_bch bank info from extra_info: ", $bankInfo);
        } else {
            $bankInfo = array(
                '227'  => 'BCH',
            );
            $this->utils->debug_log("=======================getting withdrawal paymerocrypto_bch bank info from code: ", $bankInfo);
        }
        return $bankInfo;
    }
