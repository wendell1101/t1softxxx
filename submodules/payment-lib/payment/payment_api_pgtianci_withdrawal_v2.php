<?php
require_once dirname(__FILE__) . '/abstract_payment_api_pgtianci_v2.php';
/**
 * PGTIANCI_WITHDRAWAL_V2
 *
 ...
 *
 * Field Values:
 * * URL: https://tianciv990901.com/api/payment
 * * Account: ## User ID ##
 * * Key: ## Secret Key ##
 *
 *
 ...
 */
class Payment_api_pgtianci_withdrawal_v2 extends Abstract_payment_api_pgtianci_v2 {
    const STATUS_SUCCESSFUL = 'completed';
    const RETURN_SUCCESS_CODE = 'ok';

    public function getPlatformCode() {
        return PGTIANCI_WITHDRAWAL_V2_PAYMENT_API;
    }

    public function getPrefix() {
        return 'pgtianci_withdrawal_v2';
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId) {
        ...
        if(!array_key_exists($bank, $bankInfo)) {
            $this->utils->error_log("========================pgtianci_v2 withdrawal bank whose bankTypeId=[$bank] is not supported by pgtianci");
        ...
        if (empty($response)) {
            ...
            $this->CI->utils->debug_log("=====================pgtianci_v2 withdrawal raw_post_data", $raw_post_data);
            $response = json_decode($raw_post_data, true);
            $this->CI->utils->debug_log("=====================pgtianci_v2 withdrawal json_decode params", $params);
        }

        $decodedResult = $this->decodeResult($response);

        $this->CI->utils->debug_log('======================================pgtianci_v2 submitWithdrawRequest url: ', $url );
        $this->CI->utils->debug_log('======================================pgtianci_v2 submitWithdrawRequest response', $response);
        $this->CI->utils->debug_log('======================================pgtianci_v2 submitWithdrawRequest decoded Result', $decodedResult);
    ...
    public function getWithdrawParams($bank, $accNum, $name, $amount, $transId) {
        ...
        $this->utils->debug_log('=========================pgtianci_v2 getWithdrawParams params', $params);

        return $params;
    }

    public function decodeResult($resp) {
        if (!isset($resp['success']) || !isset($resp['data'])) {
            return [ 'success' => false, 'message' => lang('Invalid API response') ];
        }
        else {
            if ($resp['success'] == true) {
                return [ 'success' => true, 'message' => 'pgtianci_v2 withdrawal request successful' ];
            }

            $state_text = isset($resp['data']['state']) ? (": state = " . $resp['data']['state']) : null;
            return [ 'success' => false, 'message' => "pgtianci_v2 withdrawal request failed{$state_text}" ];
        }
    ...
    public function callbackFromServer($transId, $params) {
        ...
        if (empty($params)) {
            $raw_post_data = file_get_contents('php://input', 'r');
            $this->CI->utils->debug_log("=====================pgtianci_v2 callbackFromServer raw_post_data", $raw_post_data);
            $params = json_decode($raw_post_data, true);
            $this->CI->utils->debug_log("=====================pgtianci_v2 callbackFromServer json_decode params", $params);
        }

        $result = array('success' => false, 'message' => 'Payment failed');

        $this->CI->utils->debug_log('=========================pgtianci_v2 callbackFromServer transId', $transId);
        $this->CI->utils->debug_log("=========================pgtianci_v2 callbackFromServer params", $params);
    ...
    private function checkCallbackOrder($order, $fields) {
        ...
        foreach ($requiredFields as $f) {
            if (!array_key_exists($f, $fields)) {
                $this->writePaymentErrorLog("======================pgtianci_v2 withdrawal checkCallbackOrder missing parameter: [$f]", $fields);
                return false;
            }
        }

        if (!$this->validateSign($fields)) {
            $this->writePaymentErrorLog('=========================pgtianci_v2 withdrawal checkCallback signature Error', $fields);
            return false;
        }

        if ($fields['state'] != self::STATUS_SUCCESSFUL) {
            $this->writePaymentErrorLog("======================pgtianci_v2 withdrawal checkCallbackOrder Payment status is not success", $fields);
            return false;
        }

        if ($fields['amount'] != $this->convertAmountToCurrency($order['amount'])) {
            $this->writePaymentErrorLog('=========================pgtianci_v2 withdrawal checkCallbackOrder payment amount is wrong, expected =>'. $order['amount'], $fields);
            return false;
        }

        if ($fields['out_trade_no'] != $order['transactionCode']) {
            $this->writePaymentErrorLog('=========================pgtianci_v2 withdrawal checkCallbackOrder order IDs do not match, expected =>'. $order['transactionCode'], $fields);
            return false;
        }
