 ...
 */
class Payment_api_coinspaid_eth extends Abstract_payment_api_coinspaid {

    public function getPlatformCode() {
        return COINSPAID_ETH_PAYMENT_API;
    }

    public function getPrefix() {
        return 'coinspaid_eth';
    }
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        #----Create Address----
            ...
            $this->CI->utils->debug_log('=====================coinspaid eth generatePaymentUrlForm params', $params);

            $response = $this->submitPostForm($create_address_url, $params, true, $secure_id);
            $response = json_decode($response, true);
            $this->CI->utils->debug_log('=====================coinspaid eth response', $response);

            if($response){
                if(isset($response['data'])){
                    $cryptoRate = json_decode($this->getCryptoRate($params), true);
                    $this->CI->utils->debug_log('=====================coinspaid eth cryptoRate', $cryptoRate);
                    if (!empty($cryptoRate)) {
                    ...
                    $this->CI->utils->debug_log('=====================coinspaid eth calculateCrypto', $calculateCrypto);
                    ...
                    $this->utils->debug_log('=====================coinspaid eth deposit_notes', $deposit_notes);
    ...
    protected function handlePaymentFormResponse($handle) {
        ...
        $this->CI->utils->debug_log("=====================coinspaid eth handlePaymentFormResponse params", $data);
