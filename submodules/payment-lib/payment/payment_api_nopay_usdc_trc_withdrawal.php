 ...
 */
class Payment_api_nopay_usdt_erc_withdrawal extends Abstract_payment_api_nopay {
    public function getPlatformCode() {
        return NOPAY_USDC_TRC_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'nopay_usdc_trc_withdrawal';
    }
    ...
    public function callbackFromServer($transId, $params) {
        ...
        if ($this->getSystemInfo("allow_auto_decline") == true && $params['state'] != self::WITHDRAWAL_CALLBACK_STATE) {
            $msg = sprintf('nopay withdrawal failed: [%s]');
        ...
        if ($params['state'] == self::WITHDRAWAL_CALLBACK_STATE) {
            $msg = sprintf('nopay withdrawal success: trade ID [%s]');
    ...
    private function checkCallbackOrder($order, $fields) {
        ...
        if ($fields['state'] != self::WITHDRAWAL_CALLBACK_STATE) {
