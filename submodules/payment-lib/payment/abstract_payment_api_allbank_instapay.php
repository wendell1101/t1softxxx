 ...
 */
abstract class Abstract_payment_api_allbank_instapay extends Abstract_payment_api {
	const ALLBANK_INSTAPAY = 'IPAYPTR';
	const ALLBANK_SOA = 'ACCOUNT-SOA';
	const REPONSE_CODE_SUCCESS = '0';
	const RESULT_CODE_SUCCESS = 'ACTC';
	...
	public function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        if($source == 'server'){
			...
			if(substr($params['sender_reference_id'] , 0, 1) == 'W'){
                $result = $this->isWithdrawal($params, $params['sender_reference_id']);
                $this->CI->utils->debug_log('=======================allbank callbackFrom sender_reference_id', $params['sender_reference_id']);
                return $result;
            }
	...
	#For Withdrawal
    public function isWithdrawal($params, $transId){
        ...
        if($params['status'] == self::RESULT_CODE_SUCCESS) {
            $msg = sprintf('allbank withdrawal was successful: trade ID [%s]', $params['sender_reference_id']);
            $this->CI->wallet_model->withdrawalAPIReturnSuccess($transId, $msg);
            $result['message'] = self::RETURN_SUCCESS_CODE;
            $result['success'] = true;
        }
        else {
            $msg = sprintf('allbank withdrawal was not success: [%s]', $params['status']);
    ...
    private function checkCallbackWithdrawalOrder($order, $fields, &$processed = false) {
		$requiredFields = array('payment_datetime', 'bank_reference', 'sender_reference_id', 'status');

        $this->CI->utils->debug_log("=========================allbank checkCallback detailData", $fields);

        foreach ($requiredFields as $f) {
           if (!array_key_exists($f, $fields)) {
                $this->writePaymentErrorLog("=======================allbank withdrawal checkCallbackOrder missing parameter: [$f]", $fields);
                return false;
            }
        }
        ...
        if ($fields['status'] != self::RESULT_CODE_SUCCESS) {
			...
			$this->writePaymentErrorLog("=====================allbank checkCallbackOrder was not successful, resultCode is [$resultCode]", $fields);
			return false;
		}

		//check callback amount
		$allow_callback_amount_check = $this->getSystemInfo('allow_callback_amount_check', true);
		$acctno = $order['extra_info'];
		if ($allow_callback_amount_check) {
			if (!$this->checkDepositAmount($fields['bank_reference'], $this->convertAmountToCurrency($order['amount']), $acctno, $order['transactionCode'])) {
    ...
    public function getOrderIdFromParameters($flds) {
        ...
        if (isset($flds['sender_reference_id'])) {
            ...
            if(substr($flds['sender_reference_id'], 0, 1) == 'D'){
                $order = $this->CI->sale_order->getSaleOrderBySecureId($flds['sender_reference_id']);
            	return $order->id;
            }else{
				$transId = null;
		        $walletAccount = $this->CI->wallet_model->getWalletAccountByTransactionCode($flds['sender_reference_id']);
				if(!empty($walletAccount)){
					$transId = $walletAccount['transactionCode'];
				}else{
					$this->utils->debug_log('====================================allbank callbackOrder transId is empty when getOrderIdFromParameters', $flds);
				}
				return $transId;
            }
        } else {
