 ...
 */
class Payment_api_nyypay_withdrawal extends Abstract_payment_api_nyypay {

    public function getPlatformCode() {
        return NYYPAY_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'nyypay_withdrawal';
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId) {
        ...
        if(!array_key_exists($bank, $this->getBankInfo())) {
            $this->utils->error_log("========================nyypay submitWithdrawRequest bank whose bankTypeId=[$bank] is not supported by nyypay");
            return array('success' => false, 'message' => 'Bank not supported by nyypay');
        }
        ...
        $this->CI->utils->debug_log('======================================nyypay submitWithdrawRequest url: ', $url );
        $this->CI->utils->debug_log('======================================nyypay submitWithdrawRequest response', $response);
        $this->CI->utils->debug_log('======================================nyypay submitWithdrawRequest decoded Result', $decodedResult);
    ...
    public function getWithdrawParams($bank, $accNum, $name, $amount, $transId) {
        ...
        $params["method"] = self::PAYWAY_WITHDRAWAL;
        $params['signType'] = 'MD5';
        ...
        $params['accountBank'] = $bankCode;
        $params['notifyUrl'] = $this->getNotifyUrl($transId);
        $params['sign'] = $this->sign($params);

        $this->utils->debug_log('=========================nyypay getWithdrawParams params', $params);
        return $params;
    }
    ...
    public function decodeResult($resultString) {
        ...
        if($decodeResult['code'] == self::RESULT_STATUS_SUCCESS) {
            $message = 'NYYpay withdrawal request success.';
            ...
            if(empty($decodeResult['code'])) {
                ...
                $message = "NYYpay withdrawal request failed: [".$decodeResult['code']."] ".$decodeResult['message'];
            }
    ...
    public function callbackFromServer($transId, $params) {
        ...
        if (empty($params)) {
            ...
            $this->CI->utils->debug_log("=====================nyypay callbackFromServer raw_post_data", $raw_post_data);
            $params = json_decode($raw_post_data, true);
            $this->CI->utils->debug_log("=====================nyypay callbackFromServer json_decode params", $params);
        }

        $result = array('success' => false, 'message' => 'Payment failed');

        $this->CI->utils->debug_log('=========================nyypay callbackFromServer transId', $transId);
        $this->CI->utils->debug_log("=========================nyypay callbackFromServer params", $params);
        ...
        if ($params['orderStatus'] == self::CALLBACK_STATUS_SUCCESS) {
            $msg = sprintf('nyypay withdrawal success: trade ID [%s]', $params['merchantOrderNo']);
            ...
            $msg = sprintf('nyypay withdrawal payment was not successful: [%s]', $params['orderStatus']);
    ...
    private function checkCallbackOrder($order, $fields) {
        ...
        foreach ($requiredFields as $f) {
            if (!array_key_exists($f, $fields)) {
                $this->writePaymentErrorLog("======================nyypay withdrawal checkCallbackOrder missing parameter: [$f]", $fields);
                return false;
            }
        }

        if (!$this->validateSign($fields)) {
            $this->writePaymentErrorLog('=========================nyypay withdrawal checkCallback signature Error', $fields);
            return false;
        }

        if ($fields['orderStatus'] != self::CALLBACK_STATUS_SUCCESS) {
            $this->writePaymentErrorLog("======================nyypay withdrawal checkCallbackOrder Payment status is not success", $fields);
            return false;
        }

        if ($fields['orderAmount'] != $this->convertAmountToCurrency($order['amount'])) {
            $this->writePaymentErrorLog('=========================nyypay withdrawal checkCallbackOrder payment amount is wrong, expected =>'. $order['amount'], $fields);
            return false;
        }

        if ($fields['merchantOrderNo'] != $order['transactionCode']) {
            $this->writePaymentErrorLog('=========================nyypay withdrawal checkCallbackOrder order IDs do not match, expected =>'. $order['transactionCode'], $fields);
            return false;
        }
    ...
    public function getBankInfo() {
        ...
        if(!empty($bankInfoArr)) {
            ...
            $this->utils->debug_log("==================getting nyypay bank info from extra_info: ", $bankInfo);
        } else  {
            $bankInfo = array(
                ...
                '77' => array('name' => 'NGAN HANG TMCP XUAT NHAP KHAU VIET NAM (EXIMBANK)', 'code' => 'EXIM'),
