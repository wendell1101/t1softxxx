 ...
 */
class Payment_api_paymerocrypto_bsv_withdrawal extends Abstract_payment_api_paymerocrypto {
    public function getPlatformCode() {
        return PAYMEROCRYPTO_BSV_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paymerocrypto_bsv_withdrawal';
    }

    protected function getCoin() {
        return $this->getSystemInfo('coin', self::COIN_BSV);;
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId, $data=null) {
        ...
        if(!array_key_exists($bank, $this->getBankInfo())) {
            $this->CI->utils->debug_log('======================================paymerocrypto_bsv withdrawal bank whose bankTypeId=[$bank] is not supported by paymerocrypto_bsv');
            return array('success' => false, 'message' => 'Bank not supported by paymerocrypto_bsv');
        }
        ...
        #https://service-api.paymero.io/v1/crypto/withdrawal
            ...
            $this->CI->utils->debug_log('=====================paymerocrypto_bsv withdrawal generatePaymentUrlForm params', $params, $withdrawal_url);

            list($response, $response_result) = $this->submitPostForm($withdrawal_url, $params, true, $transId, true);

            $response = json_decode($response, true);

            $this->CI->utils->debug_log('=====================paymerocrypto_bsv withdrawal response', $response);
            ...
            $this->CI->utils->debug_log('======================================paymerocrypto_bsv withdrawal submitWithdrawRequest decoded Result', $decodedResult);
    ...
    public function decodeResult($result) {
        ...
        $this->utils->debug_log("=========================paymerocrypto_bsv withdrawal decodeResult", $result);
    ...
    public function getBankInfo() {
        $bankInfo = array();
        $bankInfoArr = $this->getSystemInfo("paymerocrypto_bsv_bank_info");
        if(!empty($bankInfoArr)) {
            foreach($bankInfoArr as $system_bank_type_id => $bankInfoItem) {
                $bankInfo[$system_bank_type_id] = $bankInfoItem;
            }
            $this->utils->debug_log("==================getting withdrawal paymerocrypto_bsv bank info from extra_info: ", $bankInfo);
        } else {
            $bankInfo = array(
                '227'  => 'BSV',
            );
            $this->utils->debug_log("=======================getting withdrawal paymerocrypto_bsv bank info from code: ", $bankInfo);
        }
        return $bankInfo;
    }
