 ...
 */
class Payment_api_coinspaid_eth_withdrawal extends Abstract_payment_api_coinspaid {
    const CALLBACK_TYPE_SEND = "withdrawal_exchange";

    public function getPlatformCode() {
        return COINSPAID_ETH_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'coinspaid_eth_withdrawal';
    }

    protected function getCoin() {
        return $this->getSystemInfo('currency', self::COIN_eth);;
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId, $data=null) {
        ...
        if(!array_key_exists($bank, $this->getBankInfo())) {
            $this->CI->utils->debug_log('======================================coinspaid_eth withdrawal bank whose bankTypeId=[$bank] is not supported by coinspaid_eth');
            return array('success' => false, 'message' => 'Bank not supported by coinspaid_eth');
        }
        ...
        $this->CI->utils->debug_log('=====================coinspaid_eth withdrawal generatePaymentUrlForm params', $params, $withdrawal_url);
        ...
        $this->CI->utils->debug_log('=====================coinspaid_eth withdrawal response', $response);
        ...
        $this->CI->utils->debug_log('========================coinspaid_eth withdrawal submitWithdrawRequest decoded Result', $decodedResult);
        ...
        $this->CI->utils->debug_log('=========================coinspaid_eth getWithdrawParams params', $params);
        return $params;
    }
    ...
    public function decodeResult($result) {
        ...
        $this->utils->debug_log("=========================coinspaid_eth withdrawal decodeResult", $result);
        ...
        if (isset($result['data'])) {
            if($result['data']['status'] == self::WITHDRAWAL_RESULT_CODE){
                ...
                $message = "coinspaid eth withdrawal successful. Txid: ". $txid .", Transfered: ".$bitcoin.", sender_amount: ".$sender_amount;

                return array('success' => true, 'message' => $message);
            }else {
                $message = "coinspaid eth response failed. [".json_encode($result)."]: ";
    ...
    public function getBankInfo() {
        ...
        if(!empty($bankInfoArr)) {
            ...
            $this->utils->debug_log("==================getting withdrawal coinspaid_eth bank info from extra_info: ", $bankInfo);
        } else {
            $bankInfo = array(
                '42'  => 'eth',
            );
            $this->utils->debug_log("==================getting withdrawal coinspaid_eth bank info from code: ", $bankInfo);
        }
        return $bankInfo;
    }
}