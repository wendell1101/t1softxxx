...
class Payment_api_mobao_unionpay extends Abstract_payment_api_mobao {

	public function getPlatformCode() {
		return MOBAO_UNIONPAY_PAYMENT_API;
	}

	public function getPrefix() {
		return 'mobao_unionpay';
	}
	...
	protected function configParams(&$params, $direct_pay_extra_info) {
		if($this->utils->is_mobile()) {
			...
			if($this->getSystemInfo('mobile_scan_qrcode')){
				$params['apiName'] = self::MOBAO_APINAME_UNIONPAY;
				$params['customerIP'] = $this->getClientIp();
			}
			else{
				$params['apiName'] = self::MOBAO_APINAME_UNIONPAY_WAP;
			}
		}
		else{
		    $params['apiName'] = self::MOBAO_APINAME_UNIONPAY;
