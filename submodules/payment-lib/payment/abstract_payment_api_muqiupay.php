 ...
 */
abstract class Abstract_payment_api_muqiupay extends Abstract_payment_api {
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log("=====================muqiupay generatePaymentUrlForm", $params);
    ...
    protected function processPaymentUrlFormPost($params) {
        ...
        $this->CI->utils->debug_log('=====================muqiupay processPaymentUrlFormURL received response', $response);
        $response = json_decode($response,true);
        $this->CI->utils->debug_log('=====================muqiupay processPaymentUrlFormURL json to array', $response);
	...
	private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        if($source == 'server'){
            $this->CI->utils->debug_log('=======================muqiupay callbackFromServer server callbackFrom', $params);
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		...
		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $fields)) {
				$this->writePaymentErrorLog("=====================muqiupay missing parameter: [$f]", $fields);
				return false;
			}
		}

		if ($this->convertAmountToCurrency($order->amount) != $fields['amount']) {
			$this->writePaymentErrorLog("=====================muqiupay Payment amounts do not match, expected [$order->amount]", $fields);
			return false;
		}

        if ($fields['orderno'] != $order->secure_id) {
            $this->writePaymentErrorLog("=====================muqiupay checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }
        # is signature authentic?
        if (!$this->verifySignature($fields)) {
            $this->writePaymentErrorLog('=====================muqiupay checkCallbackOrder verify signature Error', $fields);
            return false;
        }
