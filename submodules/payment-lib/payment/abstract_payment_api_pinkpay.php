 ...
 */
abstract class abstract_payment_api_pinkpay extends Abstract_payment_api {
    const BUSICODE_BANK      = '1012';
    const BUSICODE_QRCODE    = '1013';
    const BUSICODE_TRUEPAY    = '1045';
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        $this->CI->utils->debug_log('=====================PINKPAY generatePaymentUrlForm params', $params);
    ...
    protected function processPaymentUrlFormRedirect($params) {
        ...
        $this->CI->utils->debug_log('=====================PINKPAY processPaymentUrlFormQRCode response', $response);
    ...
    private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        $this->CI->utils->debug_log("=====================PINKPAY callbackFrom $source params", $params);

        if($source == 'server'){
            if (empty($params)) {
                $raw_post_data = file_get_contents('php://input', 'r');
                $this->CI->utils->debug_log("=====================PINKPAY raw_post_data", $raw_post_data);
                $params = json_decode($raw_post_data, true);
                $this->CI->utils->debug_log("=====================PINKPAY json_decode params", $params);
        ...
        if ($orderStatus == Sale_order::STATUS_BROWSER_CALLBACK || $orderStatus == Sale_order::STATUS_SETTLED) {
            ...
            $this->CI->sale_order->updateExternalInfo($order->id, $order->secure_id, '', null, null, $response_result_id);
            if ($source == 'browser') {
    ...
    private function checkCallbackOrder($order, $fields, &$processed = false) {
        ...
        foreach ($requiredFields as $f) {
            if (!array_key_exists($f, $fields)) {
                $this->writePaymentErrorLog("=====================PINKPAY checkCallbackOrder Missing parameter: [$f]", $fields);
                return false;
            }
        }
        ...
        if (!$this->validateSign($fields)) {
            $this->writePaymentErrorLog('=====================PINKPAY checkCallbackOrder Signature Error', $fields);
            return false;
        }
        ...
        if ($result_params['amount'] != $this->convertAmountToCurrency($order->amount)) {
            $this->writePaymentErrorLog("======================PINKPAY checkCallbackOrder Payment amount is wrong, expected [$order->amount]", $fields);
            return false;
        }

        if ($result_params['orderid'] != $order->secure_id) {
            $this->writePaymentErrorLog("======================PINKPAY checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }
