 ...
 */
class Payment_api_aipay_weixin extends Abstract_payment_api_aipay {

	public function getPlatformCode() {
		return AIPAY_WEIXIN_PAYMENT_API;
	}
	...
	protected function configParams(&$params, $direct_pay_extra_info) {
        if($this->CI->utils->is_mobile()) {
        	$params['pay_mode'] = self::PAY_MODE_H5;
           	$params['bank_code'] = self::SCANTYPE_WEIXIN_H5;
        }
        else {
        	$params['pay_mode'] = self::PAY_MODE_QRCODE;
            $params['bank_code'] = self::SCANTYPE_WEIXIN;
        }
	}
