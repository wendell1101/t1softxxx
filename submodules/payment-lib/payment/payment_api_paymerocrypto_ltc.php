 ...
 */
class Payment_api_paymerocrypto_ltc extends Abstract_payment_api_paymerocrypto {

    public function getPlatformCode() {
        return PAYMEROCRYPTO_LTC_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paymerocrypto_ltc';
    }

    protected function getCoin() {
        return $this->getSystemInfo('currency', self::COIN_LTC);
    }
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        if (isset($cryptoRate->status)) {
            if ($cryptoRate->status == self::RETURN_SUCCESS_CODE) {
                ...
                $this->utils->debug_log('=====================get cryptoRate ltc success rate', $exchangeRate,$rate);
            } else {
                $this->utils->debug_log('=====================get cryptoRate ltc fail');
            }
        }else{
            $this->utils->debug_log('=====================not get cryptoRate ltc');
        }
        ...
        #----Create Address----
            ...
            $this->CI->utils->debug_log('=====================paymerocrypto ltc generatePaymentUrlForm params', $params);

            $response = $this->submitPostForm($create_address_url, $params, true, $secure_id);
            $response = json_decode($response, true);
            $this->CI->utils->debug_log('=====================paymerocrypto ltc response', $response);
            ...
            if($response){
                ...
                if($response['status'] == self::RETURN_SUCCESS_CODE){
                    ...
                    $this->utils->debug_log('=====================paymerocrypto ltc deposit_notes', $deposit_notes);
    ...
    protected function handlePaymentFormResponse($handle) {
        ...
        $data['Litecoin']        = $handle['crypto'];
        $data['Rate']            = $handle['current_rate'];
        $this->CI->utils->debug_log("=====================paymerocrypto ltc handlePaymentFormResponse params", $data);
