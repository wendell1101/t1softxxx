 ...
 * * BOHAIPAY_ALIPAY_PAYMENT_API, ID: 536
 * * BOHAIPAY_QUICKPAY_PAYMENT_API, ID: 537
 * * BOHAIPAY_JDPAY_PAYMENT_API, ID: 538
 * * BOHAIPAY_UNIONPAY_PAYMENT_API, ID: 539
 * * BOHAIPAY_WEIXIN_WAP_PAYMENT_API, ID: 540
 * * BOHAIPAY_QQPAY_WAP_PAYMENT_API, ID: 541
 * * BOHAIPAY_ALIPAY_WAP_PAYMENT_API, ID: 542
 * * BOHAIPAY_QUICKPAY_WAP_PAYMENT_API, ID: 543
 * * BOHAIPAY_JDPAY_WAP_PAYMENT_API, ID: 544
 *
 * Required Fields:
 * * URL
 * * Account
 ...
 */
abstract class Abstract_payment_api_bohaipay extends Abstract_payment_api {

	const P_CHANNEL_ALIPAY = '992';
	const P_CHANNEL_ALIPAY_WAP = '1006';
	const P_CHANNEL_WEIXIN = '1004';
	const P_CHANNEL_WEIXIN_WAP = '1005';
	const P_CHANNEL_QQPAY = '1593';
    const P_CHANNEL_QQPAY_WAP = '1594';
    const P_CHANNEL_QUICKPAY = '2087';
    const P_CHANNEL_QUICKPAY_WAP = '2088';
    const P_CHANNEL_JDPAY = '1008';
    const P_CHANNEL_JDPAY_WAP = '1009';
	const P_CHANNEL_UNIONPAY = '1007';
	...
	public function generatePaymentUrlForm($orderid, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log("=====================bohaipay generatePaymentUrlForm", $params);
	...
	private function callbackFrom($source, $orderid, $params, $response_result_id) {
		...
		$this->CI->utils->debug_log("=====================bohaipay raw_post_data", $raw_post_data);
		$this->CI->utils->debug_log("=====================bohaipay params", $params);
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		...
		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $fields)) {
				$this->writePaymentErrorLog("=====================bohaipay checkCallbackOrder missing parameter: [$f]", $fields);
				return false;
			}
		}
		...
		if (!$this->validateSign($fields)) {
			$this->writePaymentErrorLog('=====================bohaipay checkCallbackOrder signature Error', $fields);
			return false;
		}
		...
		if ($fields['opstate'] != self::P_ERRORCODE_SUCCESS) {
			$this->writePaymentErrorLog('=====================bohaipay checkCallbackOrder payment was not successful', $fields);
			return false;
		}
		...
		if (
			...
			$this->writePaymentErrorLog("=====================bohaipay checkCallbackOrder payment amounts do not match, expected [$order->amount]", $fields);
			return false;
		}
