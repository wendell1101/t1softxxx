 ...
 */
abstract class Abstract_payment_api_mgppay extends Abstract_payment_api {

   	const CHANNEL_TYPE_ONLINE = 1;
   	const CHANNEL_TYPE_MOMO   = 2;
   	const CHANNEL_TYPE_ZALO   = 3;
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log("=====================mgppay generatePaymentUrlForm", $params);
	...
	protected function processPaymentUrlFormURL($params) {
        ...
        $this->CI->utils->debug_log('=====================mgppay processPaymentUrlFormURL response json to array', $decode_data);
	...
	private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        if($source == 'server'){
            $this->CI->utils->debug_log('=======================mgppay callbackFromServer server callbackFrom', $params);
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		...
		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $fields)) {
                $this->writePaymentErrorLog("=====================mgppay missing parameter: [$f]", $fields);
				return false;
			}
		}
		# is signature authentic?
		if (!$this->validateSign($fields)) {
			$this->writePaymentErrorLog('=====================mgppay checkCallbackOrder Signature Error', $fields);
			return false;
		}
		...
		if ($fields['status'] != self::CALLBACK_SUCCESS_CODE) {
			...
			$this->writePaymentErrorLog("=====================mgppay Payment was not successful, merchantNo is [$payStatus]", $fields);
			return false;
		}
		...
		if ($fields['bizAmt'] != $this->convertAmountToCurrency($order->amount)) {
			if ($this->getSystemInfo('allow_callback_amount_diff')) {
				...
				$this->writePaymentErrorLog("=====================mgppay Payment amounts do not match, expected [$order->amount]", $fields);
                return false;
            }
        }
        if ($fields['orderNo'] != $order->secure_id) {
            $this->writePaymentErrorLog("========================mgppay checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
		}
