 ...
 */
class Payment_api_hcpay_alipay extends Abstract_payment_api_hcpay {

    public function getPlatformCode() {
        return HCPAY_ALIPAY_PAYMENT_API;
    }
    ...
    protected function configParams(&$params, $direct_pay_extra_info) {
        if($this->CI->utils->is_mobile()) {
            $params['pay_passcode'] = $this->getSystemInfo('passcode_mobile', self::PASSCODE_ALIPAY_H5);
        }
        else {
            $params['pay_passcode'] = $this->getSystemInfo('passcode_pc', self::PASSCODE_ALIPAY);
        }
    }
