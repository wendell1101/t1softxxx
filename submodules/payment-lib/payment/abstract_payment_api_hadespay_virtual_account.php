 ...
 */
abstract class Abstract_payment_api_hadespay_virtual_account extends Abstract_payment_api {
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        $this->CI->utils->debug_log('=====================hadespay_virtual_account generatePaymentUrlForm params', $params);
    ...
    protected function processPaymentUrlFormRedirect($params) {
        ...
        $this->CI->utils->debug_log('=====================hadespay_virtual_account processPaymentUrlFormRedirect response', $response);

        if(isset($response['responseCode']) && $response['responseCode'] == self::RESPONSE_SUCCESS) {
            return array(
                'success' => true,
                'type' => self::REDIRECT_TYPE_STATIC,
                'data' => [$response['responseData']['virtualAccount']['vaNumber']],
            );
        }
    ...
    private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        $this->CI->utils->debug_log("=====================hadespay_virtual_account callbackFrom $source params", $params);


        if(empty($params)){
            $raw_post_data = file_get_contents('php://input', 'r');
            $params = json_decode($raw_post_data, true);
            $this->CI->utils->debug_log("=====================hadespay_virtual_account json_decode params", $params);
        }
    ...
    private function checkCallbackOrder($order, $fields, &$processed = false) {
        ...
        //         $this->writePaymentErrorLog("=====================hadespay_virtual_account checkCallbackOrder Missing parameter: [$f]", $fields);
        //         return false;
        //     }
        // }
        ...
        //     $this->writePaymentErrorLog('=====================hadespay_virtual_account checkCallbackOrder Signature Error', $fields);
        //     return false;
        // }
        ...
        if ($fields['responseData']['status'] != self::CALLBACK_SUCCESS) {
            $this->writePaymentErrorLog("======================hadespay_virtual_account checkCallbackOrder Payment status is not success", $fields);
            return false;
        }

        if ($fields['responseData']['amount'] != $this->convertAmountToCurrency($order->amount)) {
            $this->writePaymentErrorLog("======================hadespay_virtual_account checkCallbackOrder Payment amount is wrong, expected [$order->amount]", $fields);
            return false;
        }

        if ($fields['responseData']['merchantRef'] != $order->secure_id) {
            $this->writePaymentErrorLog("======================hadespay_virtual_account checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }
    ...
    public function sign($params) {
        ...
        $this->CI->utils->debug_log('=====================hadespay_virtual_account sign signStr', $signStr);
    ...
    protected function processCurl($params, $url) {
        ...
        curl_close($ch);
        $this->CI->utils->debug_log('=====================hadespay_virtual_account', $key, $token, $signature, $params);
        ...
        $this->CI->utils->debug_log('=====================hadespay_virtual_account processCurl decoded response', $response);
        return $response;
    }
}