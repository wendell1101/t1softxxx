 ...
 */
class Payment_api_paybus_sunpay_banktransfer extends Abstract_payment_api_paybus {

    public function getPlatformCode() {
        return PAYBUS_SUNPAY_BANKTRANSFER_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paybus_sunpay_banktransfer';
    }
		...
		if(!empty($direct_pay_extra_info)) {
	        ...
	        if(!empty($extraInfo['field_required_bank_code'])&&$bankCodeUse){
                $params['channel_input'] = json_decode(json_encode([
                    self::CHANNEL_SUNPAY_BANKTRANSFER => $channel_input
                ]));
	        }
		}
    }
