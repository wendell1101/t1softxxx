 ...
 * *  xinshengpay
 * *
 * * 'XINSHENGPAY_PAYMENT_API', ID 6087
 * *
 * Required Fields:
 *
 * * URL
 ...
 */
abstract class Abstract_payment_api_xinshengpay extends Abstract_payment_api {
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$params['store_id']	  	= $this->getSystemInfo("account");
		...
		$this->CI->utils->debug_log("=====================xinshengpay generatePaymentUrlForm", $params);
	...
	protected function processPaymentUrlFormURL($params) {
        ...
        $this->CI->utils->debug_log('=====================xinshengpay processPaymentUrlFormURL response', $response);

        if(isset($response['http_status_code']) && $response['http_status_code'] == self::RESULT_CODE_SUCCESS) {
            return array(
                'success' => true,
	...
	private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        if($source == 'server'){
            $this->CI->utils->debug_log('=======================xinshengpay callbackFromServer server callbackFrom', $params);
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		$requiredFields = array('order_no','money','status','sign');

		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $fields)) {
				$this->writePaymentErrorLog("=====================xinshengpay missing parameter: [$f]", $fields);
				return false;
			}
		}
        ...
        if (!$this->verifySignature($fields)) {
            $this->writePaymentErrorLog('=======================xinshengpay checkCallbackOrder verify signature Error', $fields);
            return false;
        }
		...
		if ( $this->convertAmountToCurrency($order->amount) != floatval( $fields['money'] )
			...
			$this->writePaymentErrorLog("=====================xinshengpay Payment amounts do not match, expected [$order->amount]", $fields);
			return false;
		}

        if ($fields['order_no'] != $order->secure_id) {
            $this->writePaymentErrorLog("========================xinshengpay checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }
