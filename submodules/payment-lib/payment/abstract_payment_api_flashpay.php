 ...
 */
abstract class Abstract_payment_api_flashpay extends Abstract_payment_api {
	const RETURN_SUCCESS_CODE = 'opstate=0';
	const RETURN_FAILED_CODE = 'opstate!=0';
	const OPSTAT_SUCCESS = '0';

	const TYPE_ALIPAY    = "992";
	const TYPE_ALIPAY_H5 = "1101";
	const TYPE_WEIXIN    = "1004";
	const TYPE_WEIXIN_H5 = "1100";
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log('=====================flashpay generatePaymentUrlForm params', $params);
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		...
		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $fields)) {
				$this->writePaymentErrorLog("======================flashpay checkCallbackOrder Missing parameter: [$f]", $fields);
				return false;
			}
		}
		...
		if (!$this->verify($fields, $fields['sign'])) {
			$this->writePaymentErrorLog('======================flashpay checkCallbackOrder Signature Error', $fields);
			return false;
		}
		...
		if ($fields['opstate'] != self::OPSTAT_SUCCESS) {
			$this->writePaymentErrorLog('======================flashpay checkCallbackOrder Payment was not successful', $fields);
			return false;
		}
		...
		if (
			...
			$this->writePaymentErrorLog("======================flashpay checkCallbackOrder Payment amounts do not match, expected [$order->amount]", $fields);
			return false;
		}

		if ($fields['orderid'] != $order->secure_id) {
			$this->writePaymentErrorLog("======================flashpay checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
			return false;
		}
