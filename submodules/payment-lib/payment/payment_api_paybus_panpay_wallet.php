 ...
 */
class Payment_api_paybus_panpay_wallet extends Abstract_payment_api_paybus {

    public function getPlatformCode() {
        return PAYBUS_PANPAY_WALLET_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paybus_panpay_wallet';
    }
		...
		if(!empty($direct_pay_extra_info)) {
	        ...
	        if(!empty($extraInfo['field_required_bank_code'])&&$bankCodeUse){
                $params['channel_input'] = json_decode(json_encode([
                    self::CHANNEL_PANPAY_WALLET => array(
    ...
    protected function getBankListInfoFallback() {
        ...
        if(empty($banklist)){
            $banklist=array(
                array('label' => "MoMo", 'value' => 'MoMo'),
                array('label' => "ViettelPay", 'value' => 'ViettelPay'),
                array('label' => "VNPAY", 'value' => 'VNPAY'),
                array('label' => "ZaloPay", 'value' => 'ZaloPay')       
            );
        }
        return $banklist;
    }
