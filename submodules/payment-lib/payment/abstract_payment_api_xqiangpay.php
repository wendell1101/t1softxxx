 ...
 * *  XQIANGPAY_ALIPAY_H5_PAYMENT_API, ID: 496
 * * XQIANGPAY_QQPAY_H5_PAYMENT_API, ID: 497
 * * XQIANGPAY_WEIXIN_H5_PAYMENT_API, ID: 498
 *
 * Required Fields:
 *
 * * URL
...
abstract class Abstract_payment_api_xqiangpay extends Abstract_payment_api {
	//const URL   = "http://test.xqiangpay.net/website/pay.htm";
	const STATECODE = '2';
	const PAYTYPE_ALIPAY = 'ZFB';
	const ORGCODE_ALIPAY = 'zfb';
	const PAYTYPE_ALIPAY_H5 = 'ZFB_H5';
	const ORGCODE_ALIPAY_H5 = 'zfb_h5';
	const PAYTYPE_WEIXIN = 'WX';
	const ORGCODE_WEIXIN = 'wx';
	const PAYTYPE_WEIXIN_H5 = 'WX_H5';
	const ORGCODE_WEIXIN_H5 = 'wx_h5';
	const PAYTYPE_QQPAY = 'QQ';
	const ORGCODE_QQPAY = 'qq';
	const PAYTYPE_QQPAY_H5 = 'QQ_H5';
	const ORGCODE_QQPAY_H5 = 'qq_h5';
	const RETURN_QRCODE_MODE = 'true';
	const PAYTYPE_UNIONPAY = 'B2C_SCAN';
	const ORGCODE_UNIONPAY = 'b2c_scan';
	//
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$params['order_secure_id'] = $order->secure_id;
	...
	private function sign($data){
        if(array_key_exists("pureQr",$data)) {
            unset($data['pureQr']);
        }
    ...
    protected function processPaymentUrlFormQRCode($params) {
        $this->CI->utils->debug_log('=====================xqiangpay scan url', $this->getSystemInfo('url'));
        $order_secure_id = $params['order_secure_id'];
        unset($params['order_secure_id']);
        $response = $this->submitPostForm($this->getSystemInfo('url'), $params, false, $order_secure_id);
        $this->CI->utils->debug_log('========================================xqiangpay received response', $response);

        $decode_data = json_decode($response,true);
        $this->CI->utils->debug_log('========================================xqiangpay response[1] json to array', $decode_data);
        ...
        if(!empty($decode_data['codeUrl']) && isset($decode_data['codeUrl'])) {
            return array(
                'success' => true,
                'type' => self::REDIRECT_TYPE_QRCODE,
                'base64_url' => $decode_data['codeUrl'],
            );
        } else{
            return array(
                'success' => false,
                'type' => self::REDIRECT_TYPE_ERROR, # will be redirected to a view for error display
                'message' => $decode_data['error'],//lang('Invalidte API response')
            );
        }
    }
	...
	private function callbackFrom($source, $orderId, $params, $response_result_id) {
		...
		if ($orderStatus == Sale_order::STATUS_BROWSER_CALLBACK || $orderStatus == Sale_order::STATUS_SETTLED) {
			...
			$this->CI->sale_order->updateExternalInfo($order->id,
				$params['orderNo'], 'Third Party Payment (No Bank Order Number)',
