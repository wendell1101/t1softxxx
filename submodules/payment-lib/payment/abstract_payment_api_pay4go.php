 ...
 */
abstract class Abstract_payment_api_pay4go extends Abstract_payment_api {
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log("====================pay4go generatePaymentUrlForm", $params);
    ...
    private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        $this->CI->utils->debug_log("=====================pay4go callbackFrom $source params", $params);

        if($source == 'server'){
            if (empty($params)) {
                $raw_post_data = file_get_contents('php://input', 'r');
                $this->CI->utils->debug_log("=====================pay4go raw_post_data", $raw_post_data);
                $params = json_decode($raw_post_data,true);
                $this->CI->utils->debug_log("=====================pay4go json_decode data", $params);
            }
	...
	private function checkCallbackOrder($order, $params, &$processed = false) {
        ...
        $this->CI->utils->debug_log("=====================pay4go json_decode params", $params);

		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $params)) {
				$this->writePaymentErrorLog("=====================pay4go missing parameter: [$f]", $params);
				return false;
			}
		}
        ...
        if (!$this->validateSign($params)) {
            $this->writePaymentErrorLog("=====================pay4go checkCallbackOrder verify signature Error", $params);
            return false;
        }
        ...
        if ($params['Amount'] != $this->convertAmountToCurrency($order->amount)) {
            $this->writePaymentErrorLog("=====================pay4go checkCallbackOrder Payment amount do not match, expected [$amount]", $params);
            return false;
        }

        if ($params['MerchantInvoiceId'] != $order->secure_id) {
            $this->writePaymentErrorLog("=====================pay4go checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $params);
            return false;
        }

        if ($params['Status'] != self::PAY_RESULT_SUCCESS) {
            $this->writePaymentErrorLog("======================pay4go checkCallbackOrder Payment status is not success", $params);
            return false;
        }
    ...
    public function processCurl($params) {
        ...
        $this->CI->utils->debug_log('=====================pay4go processCurl response', $response);
        $response = json_decode($response, true);

        $this->CI->utils->debug_log('=====================pay4go processCurl decoded response', $response);
        return $response;
    }
