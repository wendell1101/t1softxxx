 ...
 */
class Payment_api_paymerocrypto_eth extends Abstract_payment_api_paymerocrypto {

    public function getPlatformCode() {
        return PAYMEROCRYPTO_ETH_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paymerocrypto_eth';
    }
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        if (isset($cryptoRate->status)) {
            if ($cryptoRate->status == self::RETURN_SUCCESS_CODE) {
                ...
                $this->utils->debug_log('=====================get cryptoRate eth success rate', $exchangeRate,$rate);
            } else {
                $this->utils->debug_log('=====================get cryptoRate eth fail');
            }
        }else{
            $this->utils->debug_log('=====================not get cryptoRate eth');
        }
        ...
        #----Create Address----
            ...
            $this->CI->utils->debug_log('=====================paymerocrypto eth generatePaymentUrlForm params', $params);

            $response = $this->submitPostForm($create_address_url, $params, true, $secure_id);
            $response = json_decode($response, true);
            $this->CI->utils->debug_log('=====================paymerocrypto eth response', $response);
            ...
            if($response){
                ...
                if($response['status'] == self::RETURN_SUCCESS_CODE){
                    ...
                    $this->utils->debug_log('=====================paymerocrypto eth deposit_notes', $deposit_notes);
    ...
    protected function handlePaymentFormResponse($handle) {
        ...
        $data['Ethereum']        = $handle['crypto'];
        $data['Rate']            = $handle['current_rate'];
        $this->CI->utils->debug_log("=====================paymerocrypto eth handlePaymentFormResponse params", $data);
