 ...
 * DIFFUSEPAY
 *
 * * 'DIFFUSEPAY_ALIPAY_H5_PAYMENT_API', 5110;
 * * 'DIFFUSEPAY_WEIXIN_PAYMENT_API', 5146;
 * * 'DIFFUSEPAY_QUICKPAY_PAYMENT_API', 5147;
 *
 * Required Fields:
 *
 * * URL
 ...
 */
abstract class Abstract_payment_api_diffusepay extends Abstract_payment_api {

    const PAY_BANKCODE_ONLINE_BANK = '1206'; //網銀支付
    const PAY_BANKCODE_ALIPAY = '1216'; //支付宝 PC/h5
    const PAY_BANKCODE_WEIXIN = '1202'; //微信
    const PAY_BANKCODE_UNIONAPAY = '1217'; //銀聯扫码
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log("=====================diffusepay generatePaymentUrlForm", $params);
    ...
    protected function processPaymentUrlFormQRCode($params) {
        ...
        $this->CI->utils->debug_log('========================================diffusepay processPaymentUrlFormQRcode response json to array', $decode_data);
	...
	private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        if($source == 'server'){
            $this->CI->utils->debug_log('=======================diffusepay callbackFromServer server callbackFrom', $params);
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		...
		$requiredFields = array('memberid', 'orderid','amount','datetime','returncode','sign');

		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $fields)) {
				$this->writePaymentErrorLog("=====================diffusepay missing parameter: [$f]", $fields);
				return false;
			}
		}
		...
		if ($fields['returncode'] != self::PAY_RESULT_SUCCESS) {
			...
			$this->writePaymentErrorLog("=====================diffusepay checkCallbackOrder Payment was not successful, payStatus is [$payStatus]", $fields);
			return false;
		}

		if ( $this->convertAmountToCurrency($order->amount) != floatval( $fields['amount'] )
		) {
			$this->writePaymentErrorLog("=====================diffusepay checkCallbackOrder Payment amounts do not match, expected [$order->amount]", $fields);
			return false;
		}

        if ($fields['orderid'] != $order->secure_id) {
            $this->writePaymentErrorLog("=====================diffusepay checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }

        # is signature authentic?
        if (!$this->verifySignature($fields)) {
            $this->writePaymentErrorLog('=====================diffusepay checkCallbackOrder verify signature Error', $fields);
            return false;
        }
