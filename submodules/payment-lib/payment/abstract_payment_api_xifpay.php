 ...
 * XIFPAY 喜付
 *
 * * XIFPAY_PAYMENT_API, ID: 564
 * * XIFPAY_ALIPAY_PAYMENT_API, ID: 565
 * * XIFPAY_WEIXIN_PAYMENT_API, ID: 566
 * * XIFPAY_QQPAY_PAYMENT_API, ID: 567
 * * XIFPAY_UNIONPAY_PAYMENT_API, ID: 568
 * * XIFPAY_JDPAY_PAYMENT_API, ID: 569
 * * XIFPAY_BDPAY_PAYMENT_API, ID: 570
 * * XIFPAY_UNIONQRPAY_PAYMENT_API, ID: 571
 * * XIFPAY_WITHDRAWAL_PAYMENT_API, ID: 572
 *
 * Required Fields:
 *
 * * URL
 ...
 */
abstract class Abstract_payment_api_xifpay extends Abstract_payment_api {
	const DEFAULTNANK_ALIPAY = 'ALIPAY';
	const DEFAULTNANK_WEIXIN = 'WXPAY';
	const DEFAULTNANK_QQPAY = 'QQPAY';
	const DEFAULTNANK_JDPAY = 'JDPAY';
	const DEFAULTNANK_UNIONPAY = 'UNIONPAY';
    const DEFAULTNANK_UNIONQRPAY = 'UNIONQRPAY';
    const DEFAULTNANK_BDPAY = 'BDPAY';
    const DEFAULTNANK_QUICKPAY = 'QUICKPAY';

	const RETURN_SUCCESS_CODE = 'succ';
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log('======================================xifpay generatePaymentUrlForm: ', $params);
	...
	protected function processPaymentUrlFormPost($params) {
		...
		$this->CI->utils->debug_log('======================================xifpay url: ', $url );
	...
	protected function processPaymentUrlFormQRCode($params) {
		...
		$this->CI->utils->debug_log('=====================xifpay scan url', $url);
		$response = $this->submitPostForm($url, $params, false, $params['orderNo']);
		$response = json_decode($response, true);

		$this->CI->utils->debug_log('=====================xifpay response', $response);
	...
	private function callbackFrom($source, $orderId, $params, $response_result_id) {
		...
		if (!$order) {
			$this->utils->error_log("=======================xifpay callback order ID [$orderId] not found.");
			return $result;
		}
		if($source == 'server' ){
			$callbackValid = false;
	...
	private function checkCallbackOrder($order, $fields, &$callbackValid) {
		...
		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $fields)) {
				$this->writePaymentErrorLog("=======================xifpay checkCallbackOrder missing parameter: [$f]", $fields);
				return false;
			}
		}

		if($this->ignore_callback_sign){
			$this->CI->utils->debug_log('=======================xifpay checkCallbackOrder ignore callback sign', $fields, $order, $this->validateSign($fields));

		}else{
			# is signature authentic?
			if (!$this->validateSign($fields)) {
				$this->writePaymentErrorLog('=======================xifpay checkCallbackOrder signature Error', $fields);
				return false;
			}
		}
		...
		if ($fields['order_no'] != $order->secure_id) {
			$this->writePaymentErrorLog("=======================xifpay checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
			return false;
		}

		if ($fields['trade_status'] != self::TRADE_STATUS_SUCCESS) {
			$this->writePaymentErrorLog('=======================xifpay checkCallbackOrder payment was not successful', $fields);
			return false;
		}

		if ($this->convertAmountToCurrency($order->amount) != $fields['total_fee']) {
			$this->writePaymentErrorLog("=======================xifpay checkCallbackOrder payment amounts do not match, expected [$order->amount]", $fields);
			return false;
		}
	...
	protected function getBankListInfoFallback() {
		return array(
			array('value' => 'CMB', 'label' => '招商银⾏'),
			array('value' => 'ICBC', 'label' => '工商银⾏'),
