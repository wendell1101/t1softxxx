 ...
 * Field Values:
 *
 * * URL: http://sapi.yc888paypay.com/Payment_Dfpay_add.html
 *
 * @category Payment
 * @copyright 2013-2022 tot
 */
class Payment_api_yc888pay_withdrawal extends Abstract_payment_api_yc888pay {
    const RETURN_STATUS_SUCCESS = '1';

    public function getPlatformCode() {
        return YC888PAY_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'yc888pay_withdrawal';
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId) {
        ...
        if(!array_key_exists($bank, $this->getBankInfo())) {
            $this->utils->error_log("========================yc888pay submitWithdrawRequest bank whose bankTypeId=[$bank] is not supported by yc888pay");
        ...
        $this->CI->utils->debug_log('======================================yc888pay submitWithdrawRequest url: ', $url );
        $this->CI->utils->debug_log('======================================yc888pay submitWithdrawRequest params: ', $params);
        $this->CI->utils->debug_log('======================================yc888pay submitWithdrawRequest response ', $response);
        $this->CI->utils->debug_log('======================================yc888pay submitWithdrawRequest decoded Result', $decodedResult);
    ...
    public function getWithdrawParams($bank, $accNum, $name, $amount, $transId) {
        ...
        if(!array_key_exists($bank, $bankInfo)) {
            $this->utils->error_log("========================yc888pay withdrawal bank whose bankTypeId=[$bank] is not supported by yc888pay");
            return array('success' => false, 'message' => 'Bank not supported by yc888pay');
            $bank = '无';
        }
    ...
    public function decodeResult($resultString, $queryAPI = false) {
        ...
        if(isset($result['status'])) {
            ...
            if($returnCode == self::RETURN_STATUS_SUCCESS) {
                $message = "yc888pay withdrawal response successful, transId: ". $result['orderno']. ", msg: ". $returnDesc;
                return array('success' => true, 'message' => $message);
            }
            $message = "yc888pay withdrawal response failed. [".$returnCode."]: ".$returnDesc;
    ...
    public function callbackFromServer($transId, $params) {
        ...
        if (empty($params)) {
            ...
            $this->CI->utils->debug_log("=====================yc888pay callbackFromServer raw_post_data", $raw_post_data);
            $params = json_decode($raw_post_data, true);
        }

        $this->utils->debug_log("=========================yc888pay checkCallback params", $params);
    ...
    private function checkCallbackOrder($order, $fields) {
        ...
        foreach ($requiredFields as $f) {
            if (!array_key_exists($f, $fields)) {
                $this->writePaymentErrorLog("======================yc888pay withdrawal checkCallbackOrder missing parameter: [$f]", $fields);
                return false;
            }
        }

        if (!$this->verifySignature($fields)) {
            $this->writePaymentErrorLog('=========================yc888pay withdrawal checkCallback signature Error', $fields);
            return false;
        }

        if ($this->convertAmountToCurrency($order['amount']) != $this->convertAmountToCurrency(floatval($fields['amount'])) ) {
            $this->writePaymentErrorLog("======================yc888pay withdrawal checkCallbackOrder payment amount is wrong, expected <= ". $order['amount'], $fields);
            return false;
        }

        if ($fields['orderno'] != $order['transactionCode']) {
            $this->writePaymentErrorLog("======================yc888pay withdrawal checkCallbackOrder order IDs do not match, expected ".$order['transactionCode'], $fields);
            return false;
        }
