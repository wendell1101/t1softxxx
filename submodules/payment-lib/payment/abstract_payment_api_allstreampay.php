 ...
 * ALLSTREAMPAY_WEIXIN_PAYMENT_API, ID: 252
 *
 * Required Fields:
 * * URL
 * * Extra Info:
 ...
 */
abstract class Abstract_payment_api_allstreampay extends Abstract_payment_api
{
	...
	public function getSecretInfoList() {
		$secretsInfo = array('live_key', 'live_secret', 'sandbox_key', 'sandbox_secret', 'allstreampay_server_pub_key', 'allstreampay_priv_key');
		return $secretsInfo;
	}
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null)
	{
		...
		$this->CI->utils->debug_log("=====================allstreampay generatePaymentUrlForm", $params);
	...
	private function checkCallbackOrder($order, $fields, &$callbackValid) {
		...
		if (!$this->validateSign($fields['encParam'], $fields['sign'])) {
			$this->writePaymentErrorLog('==========================allstreampay Signature Error', $fields);
			return false;
		}
		...
		if ($res['respCode'] != self::RESULT_CODE_SUCCESS) {
			$this->writePaymentErrorLog('allstreampay Payment was not successful', $fields);
			return false;
		}
	...
	/**
	 * get signature and encoded params by algorithm of allstreampay
	 *
	 * @param $params
	 * @return array
	 */
	...
	public function getServerPubKeyStr() {
		$allstreampay_pub_key = $this->getSystemInfo('allstreampay_server_pub_key');
		$pub_key = '-----BEGIN PUBLIC KEY-----' . PHP_EOL . chunk_split($allstreampay_pub_key, 64, PHP_EOL) . '-----END PUBLIC KEY-----' . PHP_EOL;
	...
	private function getPrivKeyStr() {
		...
		$allstreampay_priv_key = $this->getSystemInfo('allstreampay_priv_key');
		$priv_key = '-----BEGIN RSA PRIVATE KEY-----' . PHP_EOL . chunk_split($allstreampay_priv_key, 64, PHP_EOL) . '-----END RSA PRIVATE KEY-----' . PHP_EOL;
