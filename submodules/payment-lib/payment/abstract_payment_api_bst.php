 ...
 * BST
 *
 * * BST_ALIPAY_PAYMENT_API, ID 5957
 * Required Fields:
 *
 * * URL
 * * Account - Merchant ID
 ...
 */
abstract class Abstract_payment_api_bst extends Abstract_payment_api {
	...
	public function getSecretInfoList() {
		$secretsInfo = array('live_key', 'live_secret', 'sandbox_key', 'sandbox_secret', 'bst_pub_key', 'bst_priv_key');
		return $secretsInfo;
	}
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log("=====================bst generatePaymentUrlForm", $params);
    ...
    protected function processPaymentUrlFormPost($params) {
        ...
        $this->utils->debug_log('=====================BST response_token', $response_token);
	...
	private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        if($source == 'server'){
            ...
            $this->CI->utils->debug_log('=======================bst callbackFromServer server callbackFrom', $params);
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		...
		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $fields)) {
				$this->writePaymentErrorLog("=====================bst missing parameter: [$f]", $fields);
				return false;
			}
		}
		if ( $this->convertAmountToCurrency($order->amount) != floatval( $fields['amount'] ) ) {
			$this->writePaymentErrorLog("=====================bst Payment amounts do not match, expected [$order->amount]", $fields);
			return false;
		}

        if ($fields['client_order_id'] != $order->secure_id) {
            $this->writePaymentErrorLog("=====================bst checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }
        # is signature authentic?
        if (!$this->verifySignature($fields)) {
            $this->writePaymentErrorLog('=====================bst checkCallbackOrder verify signature Error', $fields);
            return false;
        }
        ...
        if ($fields['status'] != self::CALLBACK_STATUS_SUCCESS) {
			$this->writePaymentErrorLog('=======================bst checkCallbackOrder payment was not successful', $fields);
			return false;
		}
    ...
    public function processCurl($params, $response_token) {
        ...
        $this->CI->utils->debug_log('=====================BST processCurl response', $response);
        $response = json_decode($response, true);

        $this->CI->utils->debug_log('=====================BST processCurl decoded response', $response);
        return $response;
    }

    # Returns public key given by gateway
	public function getPubKey() {
		$bst_pub_key = $this->getSystemInfo('bst_pub_key');

		$pub_key = '-----BEGIN PUBLIC KEY-----' . PHP_EOL . chunk_split($bst_pub_key, 64, PHP_EOL) . '-----END PUBLIC KEY-----' . PHP_EOL;
		return openssl_get_publickey($pub_key);
	}

	# Returns the private key generated by merchant
	public function getPrivKey() {
		$bst_priv_key = $this->getSystemInfo('bst_priv_key');

		$priv_key = '-----BEGIN RSA PRIVATE KEY-----' . PHP_EOL . chunk_split($bst_priv_key, 64, PHP_EOL) . '-----END RSA PRIVATE KEY-----' . PHP_EOL;
