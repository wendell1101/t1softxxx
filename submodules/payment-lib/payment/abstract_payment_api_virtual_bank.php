 ...
 */
abstract class Abstract_payment_api_virtual_bank extends Abstract_payment_api {
...
const RETURN_FAILED_CODE = "FAIL";
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        $this->CI->utils->debug_log("=====================virtual bank generatePaymentUrlForm", $params);
    ...
    protected function processPaymentUrlFormPost($params) {
        ...
        $this->CI->utils->debug_log('========================================virtual bank processPaymentUrlFormPost response json to array', $response);
        ...
        if( isset($response['status']) && $response['status'] == self::REPONSE_CODE_SUCCESS && !empty($response['trx_id']) ){
            ...
            if(isset($response['data']) && !empty($response['data'])){
                $this->CI->utils->debug_log('========================================virtual bank response data', $response['data']);
                return array(
                    'success' => true,
                    'type' => self::REDIRECT_TYPE_STATIC,
                    'data' => [$response['data']],
                    'note' => $this->getSystemInfo('note')
                );
            }else{
                return array(
                    'success' => false,
    ...
    private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        $this->CI->utils->debug_log("=====================virtual bank callbackFrom $source params", $params);
    ...
    private function checkCallbackOrder($order, $fields, &$processed)
    {
        ...
        foreach ($requiredFields as $f) {
            if (!array_key_exists($f, $fields)) {
                    $this->writePaymentErrorLog("=========================virtual bank checkCallbackOrder missing parameter: [$f]", $fields);
                    return false;
            }
        }
        ...
        if (($fields['status'] != self::ORDER_STATUS_SUCCESS) ) {
            $this->writePaymentErrorLog("=========================virtual bank checkCallbackOrder returncode was not successful", $fields);
           return false;
        }
        ...
        if ($fields['amount'] != $this->convertAmountToCurrency($order->amount)) {
            ...
            if($this->getSystemInfo('allow_callback_amount_diff')){
                $this->CI->utils->debug_log('=====================virtual bank amount not match expected [$order->amount]');
                ...
                $this->writePaymentErrorLog("======================virtual bank checkCallbackOrder Payment amount is wrong, expected [$order->amount]", $fields);
                return false;
            }
        }
        ...
        if($submitPostCheckStatus['status'] != self::ORDER_STATUS_SUCCESS){
            $this->writePaymentErrorLog("=========================virtual bank checkCallbackOrder submitPostCheckStatus failed", $submitPostCheckStatus['status']);
            return false;
        }
    ...
    public function getOrderIdFromParameters($flds) {
        if(empty($flds)){
            ...
            $this->utils->debug_log('======virtual bank getOrderIdFromParameters raw_post flds ' , $flds);
        }
        ...
        if(isset($flds['trx_id'])) {
            ...
            $this->utils->debug_log('=====================virtual bank getOrderIdFromParameters cannot get ref_no', $flds);
            return;
        }
        ...
        //     }
        //     $this->utils->debug_log('=====================virtual bank target_table', $target_table);

        //     $order_id=$this->getOrderIdFormResp($flds['trx_id'], $target_table);
        //     if(empty($order_id)){
        //         $this->utils->debug_log('=====================virtual bank callbackOrder cannot get any order_id when getOrderIdFromParameters', $flds);
        //         return;
        //     }
        //     return $order_id;
        // }
	...
	protected function submitPostCheckStatus($url, $params) {
        $this->CI->utils->debug_log("=====================virtual bank submitPostCheckStatus==============", $url, $params);
		...
		try {
			...
			$this->CI->utils->debug_log('========================= virtual bank submitPostForm curl content ', $response_result_content['status']);
