 ...
 */
class Payment_api_coinspaid_usdtt extends Abstract_payment_api_coinspaid {

    public function getPlatformCode() {
        return COINSPAID_USDTT_PAYMENT_API;
    }

    public function getPrefix() {
        return 'coinspaid_usdtt';
    }
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        #----Create Address----
            ...
            $this->CI->utils->debug_log('=====================coinspaid usdtt generatePaymentUrlForm params', $params);

            $response = $this->submitPostForm($create_address_url, $params, true, $secure_id);
            $response = json_decode($response, true);
            $this->CI->utils->debug_log('=====================coinspaid usdtt response', $response);

            if($response){
                if(isset($response['data'])){
                    $cryptoRate = json_decode($this->getCryptoRate($params), true);
                    $this->CI->utils->debug_log('=====================coinspaid usdtt cryptoRate', $cryptoRate);
                    if (!empty($cryptoRate)) {
                    ...
                    $this->CI->utils->debug_log('=====================coinspaid usdtt calculateCrypto', $calculateCrypto);
                    ...
                    $this->utils->debug_log('=====================coinspaid usdtt deposit_notes', $deposit_notes);
    ...
    protected function handlePaymentFormResponse($handle) {
        ...
        $data['Tether USD (TRC20)'] = $handle['crypto'];
        $data['Rate']               = '1 '. $this->getCoin() .' ~ ' .$handle['current_rate'] . ' ' .$this->targetCurrency;
        $data['Address qrcode']     = $handle['qrcode'];
        $this->CI->utils->debug_log("=====================coinspaid usdtt handlePaymentFormResponse params", $data);
