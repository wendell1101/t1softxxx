 ...
 * * 'STONPAY_UNIONPAY_PAYMENT_API', ID 5086
 * * 'STONPAY_WITHDRAWAL_PAYMENT_API', ID 5087
 * * 'STONPAY_UNIONPAY_H5_PAYMENT_API', ID 5090
 * * 'STONPAY_ONLINEBANK_H5_PAYMENT_API', ID 5091
 * * 客戶要求 參數相同也要拆分PC 與 H5
 * Required Fields:
 *
 * * URL
 * * Account - Merchant ID
 ...
 */
abstract class Abstract_payment_api_stonpay extends Abstract_payment_api {

	const PTYPE_ONLINEBANK = 'B2C001'; //网关支付
    const PTYPE_ALIPAY	   = 'AL0001'; //支付宝
    const PTYPE_ALIPAY_H5  = 'AL0003'; //支付宝H5
	const PTYPE_UNIONPAY   = 'UN0001'; //銀聯扫码
    const PTYPE_QUICKPAY   = 'GQ0001'; //网关快捷
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log("=====================stonpay generatePaymentUrlForm", $params);
    ...
    protected function processPaymentUrlFormQRCode($params) {
        ...
        $this->CI->utils->debug_log('=====================stonpay processPaymentUrlFormQRcode received response', $response);
        $this->CI->utils->debug_log('=====================stonpay processPaymentUrlFormQRcode response[1] json to array', $decode_data);
        $msg = lang('Invalidte API response');

		if(!empty($decode_data['payMessage']) && ($decode_data['resultCode'] == self::REQUEST_SUCCESS)) {
			return array(
	                'success' => true,
	                'type' => self::REDIRECT_TYPE_QRCODE,
	                'url' => $decode_data['payMessage'],
            	);
        }else {
	...
	private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        $this->CI->utils->debug_log('=======================stonpay callbackFrom in Function callbackFrom', $params);
        ...
        if($source == 'server'){
            $this->CI->utils->debug_log('=======================stonpay callbackFromServer server callbackFrom', $params);
		...
		if ($orderStatus == Sale_order::STATUS_BROWSER_CALLBACK || $orderStatus == Sale_order::STATUS_SETTLED) {
            ...
            $this->CI->sale_order->updateExternalInfo($order->id,
                $params['outTradeNo'], 'Third Party Payment (No Bank Order Number)', # no info available
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		...
		$requiredFields = array('appid','outTradeNo','orderAmount','pType','tradeStatus','successTime','orderTime','trxNo','sign');

		foreach ($requiredFields as $f) {
			if (!array_key_exists($f, $fields)) {
				$this->writePaymentErrorLog("=====================stonpay missing parameter: [$f]", $fields);
				return false;
			}
		}

		if ($fields['tradeStatus'] != self::PAY_RESULT_SUCCESS) {
			$payStatus = $fields['tradeStatus'];
			$this->writePaymentErrorLog("=====================stonpay Payment was not successful, payStatus is [$payStatus]", $fields);
			return false;
		}

		if ( $this->convertAmountToCurrency($order->amount) != floatval( $fields['orderAmount'] )
		) {
			$this->writePaymentErrorLog("=====================stonpay Payment amounts do not match, expected [$order->amount]", $fields);
			return false;
		}

        if ($fields['outTradeNo'] != $order->secure_id) {
            $this->writePaymentErrorLog("========================stonpay checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }

        # is signature authentic?
        if (!$this->verifySignature($fields)) {
            $this->writePaymentErrorLog('=======================stonpay checkCallbackOrder verify signature Error', $fields);
            return false;
        }
    ...
    private function createSignStr($params) {
		...
		foreach ($params as $key => $value) {
			if(is_null($value) || is_null($value) || $key == 'sign' || $key == 'mobile'){
				continue;
			}
			$signStr .= $key."=".$value."&";
		}
