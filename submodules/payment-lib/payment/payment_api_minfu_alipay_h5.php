<?php
require_once dirname(__FILE__) . '/abstract_payment_api_minfu.php';
/**
 * MINFU 民付支付
 * *
 * * MINFU_ALIPAY_H5_PAYMENT_API, ID: 5173
 *
 * Required Fields:
 * * URL
 * * Account
 * * Key
 *
 * Field Values:
 * * URL: http://47.110.56.150/gateway/index/checkpoint.do
 * * Account: ## Merchant ID ##
 * * Key: ## Secret Key ##
 *
 *
 * @category Payment
 * @copyright 2013-2022 tot
 */
class Payment_api_minfu_alipay_h5 extends Abstract_payment_api_minfu {

    public function getPlatformCode() {
        return MINFU_ALIPAY_H5_PAYMENT_API;
    }

    public function getPrefix() {
        return 'minfu_alipay_h5';
    }

    protected function configParams(&$params, $direct_pay_extra_info) {
        if($this->getSystemInfo('thoroughfare_cloudpay')){
            $params['thoroughfare'] = self::THOROUGHFARE_CLOUDPAY;
            $params['method'] = self::METHOD_ALIPAY;
            $params['content_type'] = self::CONTENTTYPE_SCAN;
        }else{
            $params['content_type'] = self::CONTENTTYPE_H5;
            $params['thoroughfare'] = self::THOROUGHFARE_ALIPAY;
        }

    }

    # Hide bank selection drop-down
    public function getPlayerInputInfo() {
        /*
            1.
            Valid [float_amount_limit] pattern

            pattern: "float_amount_limit": "(A|B|C|D|E|F|...)"

            A: limit amount 1
            B: limit amount 2
            C: limit amount 3

            example: "float_amount_limit": "(1|21|51)"

            2.
            show [float_amount_limit_msg]  when amount is incorrect
        */
        $type = $this->getSystemInfo('float_amount_limit')? 'float_amount_limit' : 'float_amount' ;
        $float_amount_limit_msg = $this->getSystemInfo('float_amount_limit_msg')?$this->getSystemInfo('float_amount_limit_msg'):'請輸入上方金額';
        if($type == 'float_amount_limit'){
            return array(
                array('name' => 'deposit_amount', 'type' => $type, 'label_lang' => 'cashier.09', 'float_amount_limit' => $this->getSystemInfo('float_amount_limit'), 'float_amount_limit_msg' => $float_amount_limit_msg),
            );
        }else{
            return array(
                array('name' => 'deposit_amount', 'type' => $type, 'label_lang' => 'cashier.09'),
            );
        }
    }

    protected function processPaymentUrlForm($params) {
        if($this->getSystemInfo('thoroughfare_cloudpay')){
            return $this->processPaymentUrlFormPost($params);
        }
        return $this->processPaymentUrlFormRedirect($params);
    }
}
