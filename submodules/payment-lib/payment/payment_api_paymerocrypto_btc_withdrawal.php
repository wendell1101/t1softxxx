 ...
 */
class Payment_api_paymerocrypto_btc_withdrawal extends Abstract_payment_api_paymerocrypto {
    public function getPlatformCode() {
        return PAYMEROCRYPTO_BTC_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paymerocrypto_btc_withdrawal';
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId, $data=null) {
        ...
        if(!array_key_exists($bank, $this->getBankInfo())) {
            $this->CI->utils->debug_log('======================================paymerocrypto_btc withdrawal bank whose bankTypeId=[$bank] is not supported by paymerocrypto_btc');
            return array('success' => false, 'message' => 'Bank not supported by paymerocrypto_btc');
        }
        ...
        #https://service-api.paymero.io/v1/crypto/withdrawal
            ...
            $this->CI->utils->debug_log('=====================paymerocrypto_btc withdrawal generatePaymentUrlForm params', $params, $withdrawal_url);

            list($response, $response_result) = $this->submitPostForm($withdrawal_url, $params, true, $transId, true);

            $response = json_decode($response, true);

            $this->CI->utils->debug_log('=====================paymerocrypto_btc withdrawal response', $response);
            ...
            $this->CI->utils->debug_log('======================================paymerocrypto_btc withdrawal submitWithdrawRequest decoded Result', $decodedResult);
    ...
    public function decodeResult($result) {
        ...
        $this->utils->debug_log("=========================paymerocrypto_btc withdrawal decodeResult", $result);
    ...
    public function getBankInfo() {
        $bankInfo = array();
        $bankInfoArr = $this->getSystemInfo("paymerocrypto_btc_bank_info");
        if(!empty($bankInfoArr)) {
            foreach($bankInfoArr as $system_bank_type_id => $bankInfoItem) {
                $bankInfo[$system_bank_type_id] = $bankInfoItem;
            }
            $this->utils->debug_log("==================getting withdrawal paymerocrypto_btc bank info from extra_info: ", $bankInfo);
        } else {
            $bankInfo = array(
                '227'  => 'BTC',
            );
            $this->utils->debug_log("=======================getting withdrawal paymerocrypto_btc bank info from code: ", $bankInfo);
        }
        return $bankInfo;
    }
