 ...
 * * 'YONKOPAY_MOMO_PAYMENT_API', ID 5983
 * Required Fields:
 *
 * * URL
 * * Account - Merchant ID
 ...
 */
abstract class Abstract_payment_api_yonkopay extends Abstract_payment_api {
	const PAYWAY_ONLINEBANK	   = '907';
	const PAYWAY_BANKCARD  	   = '908';
	const PAYWAY_QRCODE  	   = '909';
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$this->CI->utils->debug_log("=====================YONKOPAY generatePaymentUrlForm", $params);
	...
	private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        if($source == 'server'){
            $this->CI->utils->debug_log('=======================YONKOPAY callbackFromServer server callbackFrom', $params);
	...
	private function checkCallbackOrder($order, $fields, &$processed = false) {
		...
		foreach ($requiredFields as $f) {
			if($f == 'status' || $f == 'sign'){
				if (!array_key_exists($f, $fields)) {
					$this->writePaymentErrorLog("=====================YONKOPAY missing parameter: [$f]", $fields);
					return false;
				}
			}else{
		...
		if ($fields['status'] != self::RESULT_STATUS_SUCCESS) {
			$this->writePaymentErrorLog("=====================YONKOPAY Payment was not successful", $fields);
			return false;
		}

		if ($this->convertAmountToCurrency($order->amount) != $resultData['amount']) {
			$this->writePaymentErrorLog("=====================YONKOPAY Payment amounts do not match, expected [$order->amount]", $fields);
			return false;
		}

        if ($resultData['orderid'] != $order->secure_id) {
            $this->writePaymentErrorLog("=====================YONKOPAY checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }

        # is signature authentic?
        if (!$this->verifySignature($fields)) {
            $this->writePaymentErrorLog('=====================YONKOPAY checkCallbackOrder verify signature Error', $fields);
            return false;
        }
