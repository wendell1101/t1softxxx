 ...
 */
abstract class Abstract_payment_api_v8pay extends Abstract_payment_api {
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        if(isset($playerdetail[0])){
            $firstName = !empty($playerdetail[0]['firstName'])  ? $playerdetail[0]['firstName']  : "no firstName";
            $lastName  = !empty($playerdetail[0]['lastName'])   ? $playerdetail[0]['lastName']   : "no lastName";
            $pixNumber = !empty($playerdetail[0]['pix_number']) ? $playerdetail[0]['pix_number'] : "none";
        }
        ...
        $params['paid_name']    = $firstName . " " . $lastName;
        $params['paid_account'] = $pixNumber;
        $this->configParams($params, $order->direct_pay_extra_info);
        $this->CI->utils->debug_log("=====================v8pay generatePaymentUrlForm params", $params);
    ...
    protected function processPaymentUrlFormPost($params) {
        ...
        $this->CI->utils->debug_log("=====================v8pay processPaymentUrlFormPost response", $response);
    ...
    private function callbackFrom($source, $orderId, $params, $response_result_id) {
        ...
        $this->CI->utils->debug_log("=====================v8pay callbackFrom $source params", $params);

        if($source == 'server' ){
            if(empty($params)){
                $raw_post_data = file_get_contents('php://input', 'r');
                $this->CI->utils->debug_log("=====================v8pay raw_post_data", $raw_post_data);
                $params = json_decode($raw_post_data, true);
                $this->CI->utils->debug_log("=====================v8pay raw_post_data json_decode params", $params);
            }
            else {
                $params = json_decode($params['requestParams'], true);
                $this->CI->utils->debug_log("=====================v8pay json_decode params", $params);
            }
        }
    ...
    private function checkCallbackOrder($order, $fields, &$processed = false) {
        ...
        foreach ($requiredFields as $f) {
            if (!array_key_exists($f, $fields)) {
                $this->writePaymentErrorLog("=====================v8pay checkCallbackOrder Missing parameter: [$f]", $fields);
                return false;
            }
        }
        ...
        if (!$this->validateSign($fields)) {
            $this->writePaymentErrorLog("======================v8pay checkCallbackOrder Signature Error", $fields);
            return false;
        }
        ...
        if ($fields['amount'] != $check_amount) {
            $this->writePaymentErrorLog("======================v8pay checkCallbackOrder Payment amount is wrong, expected <= ". $check_amount, $fields);
            return false;
        }

        if ($fields['out_trade_no'] != $order->secure_id) {
            $this->writePaymentErrorLog("======================v8pay checkCallbackOrder order IDs do not match, expected [$order->secure_id]", $fields);
            return false;
        }
    ...
    protected function processCurl($url, $token, $params, $secure_id, $post = true) {
        ...
        $this->CI->utils->debug_log('=====================v8pay processCurl url', $url, 'params', $params , 'response', $response, 'errCode', $errCode, 'error', $error, 'statusCode', $statusCode);
