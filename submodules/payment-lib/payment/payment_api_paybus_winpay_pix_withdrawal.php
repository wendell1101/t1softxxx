 ...
 * PAYBUS_WITHDRAWAL
 *
 * * PAYBUS_HPAY_WITHDRAWAL_PAYMENT_API, ID: 6380
 *
 * Field Values:
 ...
 */
class Payment_api_paybus_winpay_pix_withdrawal extends Abstract_payment_api_paybus {

    public function getPlatformCode() {
        return PAYBUS_WINPAY_PIX_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paybus_winpay_pix_withdrawal';
    }
    ...
    public function getWithdrawParams($bank, $accNum, $name, $amount, $transId) {
        ...
        $playerDetail=$this->getPlayerInfoByTransactionCode($transId);
        $this->utils->debug_log("===============================paybus Get playerBankDetails using [$bank] + [$accNum]", $playerDetail);
        ...
        $fullName = !empty($playerDetail['firstName'])?$this->generateFullName([$playerDetail['firstName'], $playerDetail['lastName']]):"";
        $channelInput = [
            self::CHANNEL_WINPAY_PIX => [
                'cardType' => $bankCode,
                "cpf" => $playerDetail['cpfNumber'],
                "userName" => $fullName,
                "walletId" => isset($walletKey[$bankCode])? $playerDetail[$walletKey[$bankCode]]: "",
            ]
        ];
        
        if ($bankCode == 'PHONE') {
            unset($channelInput[self::CHANNEL_WINPAY_PIX]['cpf']);
        }
