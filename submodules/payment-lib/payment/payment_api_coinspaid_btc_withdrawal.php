 ...
 */
class Payment_api_coinspaid_btc_withdrawal extends Abstract_payment_api_coinspaid {
    const CALLBACK_TYPE_SEND = "withdrawal_exchange";

    public function getPlatformCode() {
        return COINSPAID_BTC_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'coinspaid_btc_withdrawal';
    }

    protected function getCoin() {
        return $this->getSystemInfo('currency', self::COIN_BTC);;
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId, $data=null) {
        ...
        if(!array_key_exists($bank, $this->getBankInfo())) {
            $this->CI->utils->debug_log('======================================coinspaid_btc withdrawal bank whose bankTypeId=[$bank] is not supported by coinspaid_btc');
            return array('success' => false, 'message' => 'Bank not supported by coinspaid_btc');
        }
        ...
        $this->CI->utils->debug_log('=====================coinspaid_btc withdrawal generatePaymentUrlForm params', $params, $withdrawal_url);
        ...
        $this->CI->utils->debug_log('=====================coinspaid_btc withdrawal response', $response);
        ...
        $this->CI->utils->debug_log('========================coinspaid_btc withdrawal submitWithdrawRequest decoded Result', $decodedResult);
        ...
        $this->CI->utils->debug_log('=========================coinspaid_btc getWithdrawParams params', $params);
        return $params;
    }
    ...
    public function decodeResult($result) {
        ...
        $this->utils->debug_log("=========================coinspaid_btc withdrawal decodeResult", $result);
        ...
        if (isset($result['data'])) {
            if($result['data']['status'] == self::WITHDRAWAL_RESULT_CODE){
                ...
                $message = "coinspaid btc withdrawal successful. Txid: ". $txid .", Transfered: ".$bitcoin.", sender_amount: ".$sender_amount;

                return array('success' => true, 'message' => $message);
            }else {
                $message = "coinspaid btc response failed. [".json_encode($result)."]: ";
    ...
    public function getBankInfo() {
        ...
        if(!empty($bankInfoArr)) {
            ...
            $this->utils->debug_log("==================getting withdrawal coinspaid_btc bank info from extra_info: ", $bankInfo);
        } else {
            $bankInfo = array(
                '42'  => 'BTC',
            );
            $this->utils->debug_log("==================getting withdrawal coinspaid_btc bank info from code: ", $bankInfo);
        }
        return $bankInfo;
    }
}