 ...
 */
class Payment_api_paymerocrypto_btc extends Abstract_payment_api_paymerocrypto {

    public function getPlatformCode() {
        return PAYMEROCRYPTO_BTC_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paymerocrypto_btc';
    }
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        if (isset($cryptoRate->status)) {
            if ($cryptoRate->status == self::RETURN_SUCCESS_CODE) {
                ...
                $this->utils->debug_log('=====================get cryptoRate btc success rate', $exchangeRate,$rate);
            } else {
                $this->utils->debug_log('=====================get cryptoRate btc fail');
            }
        }else{
            $this->utils->debug_log('=====================not get cryptoRate btc');
        }
        ...
        #----Create Address----
            ...
            $this->CI->utils->debug_log('=====================paymerocrypto btc generatePaymentUrlForm params', $params);

            $response = $this->submitPostForm($create_address_url, $params, true, $secure_id);
            $response = json_decode($response, true);
            $this->CI->utils->debug_log('=====================paymerocrypto btc response', $response);
            ...
            if($response){
                ...
                if($response['status'] == self::RETURN_SUCCESS_CODE){
                    ...
                    $this->utils->debug_log('=====================paymerocrypto btc deposit_notes', $deposit_notes);
    ...
    protected function handlePaymentFormResponse($handle) {
        ...
        $this->CI->utils->debug_log("=====================paymerocrypto btc handlePaymentFormResponse params", $data);
