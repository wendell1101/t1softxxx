 ...
 * * YEEPAY_WEIXIN_PAYMENT_API, ID: 213
 *
 * Required Fields:
 *
 * * URL
 ...
 */
abstract class Abstract_payment_api_yeepay extends Abstract_payment_api {
	...
	public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
		...
		$params['phone'] = $this->getSystemInfo("phone");
		$params['amount'] = $this->convertAmountToCurrency($amount);
		$params['notifyUrl'] = $this->getNotifyUrl($orderId);
		$params['returnUrl'] = $this->getReturnUrl($orderId);

		$params['orderNo'] = $order->secure_id;
		$params['paycode'] = '123456';  //temp
	...
	protected function processPaymentUrlQRCode($params) {
		$resultString = $this->submitPostForm($this->getSystemInfo('url'), $params, false, $params['orderNo']);
	...
	private function checkCallbackOrder($order, $fields, &$callbackValid) {
		...
		$requiredFields = array(
			'code', 'message', 'qRcodeURL', 'orderId', 'orderNo'
		);
	...
	private function validateSign($params) {
		...
		return strcasecmp($md5Sign, $params['message']) === 0;
	}
