 ...
 *
 * Field Values:
 * * URL: https://pay2-open.kyriandev.com/payment/withdraw
 * * Account: ## User ID ##
 * * Key: ## Secret Key ##
 *
 *
 ...
 */
class Payment_api_paybus_mockpay1_pix_withdrawal extends Abstract_payment_api_paybus {

    public function getPlatformCode() {
        return PAYBUS_MOCKPAY1_PIX_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'paybus_mockpay1_pix_withdrawal_withdrawal';
    }
    ...
    public function getWithdrawParams($bank, $accNum, $name, $amount, $transId) {
        ...
        if(!empty($playerBankDetails)){
            ...
            $pixNumber = (isset($playerDetails[0]) && !empty($playerDetails[0]['pix_number']))? $playerDetails[0]['pix_number'] : '';
            ...
            $dialing_code = (isset($playerDetails[0]) && !empty($playerDetails[0]['dialing_code']))? $playerDetails[0]['dialing_code'] : '';
        }
        ...
        $accArr = array(
            "CPF"   => $pixNumber,
            "EMAIL" => $email,
            "PHONE" => $dialing_code.$phone
        );

        $withdraw_pix_type = $this->getSystemInfo("withdraw_pix_type", "CPF");

        $params = array();
        $params['client_id']     = $transId;
        $params['amount']        = (double)$this->convertAmountToCurrency($amount);
        $params['channel_input'] = json_decode(json_encode([self::CHANNEL_MOCKPAY1_PIX => ['type' => $withdraw_pix_type, 'identifier' => $accArr[$withdraw_pix_type]]]));
