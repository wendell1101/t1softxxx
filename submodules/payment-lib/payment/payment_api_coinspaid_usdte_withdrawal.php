 ...
 */
class Payment_api_coinspaid_usdte_withdrawal extends Abstract_payment_api_coinspaid {
    const CALLBACK_TYPE_SEND = "withdrawal_exchange";

    public function getPlatformCode() {
        return COINSPAID_USDTE_WITHDRAWAL_PAYMENT_API;
    }

    public function getPrefix() {
        return 'coinspaid_usdte_withdrawal';
    }
    ...
    public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId, $data=null) {
        ...
        if(!array_key_exists($bank, $this->getBankInfo())) {
            $this->CI->utils->debug_log('======================================coinspaid_usdte withdrawal bank whose bankTypeId=[$bank] is not supported by coinspaid_usdte');
            return array('success' => false, 'message' => 'Bank not supported by coinspaid_usdte');
        }
        ...
        $this->CI->utils->debug_log('=====================coinspaid_usdte withdrawal generatePaymentUrlForm params', $params, $withdrawal_url);
        ...
        $this->CI->utils->debug_log('=====================coinspaid_usdte withdrawal response', $response);
        ...
        $this->CI->utils->debug_log('========================coinspaid_usdte withdrawal submitWithdrawRequest decoded Result', $decodedResult);
        ...
        $this->CI->utils->debug_log('=========================coinspaid_usdte getWithdrawParams params', $params);
        return $params;
    }
    ...
    public function decodeResult($result) {
        ...
        $this->utils->debug_log("=========================coinspaid_usdte withdrawal decodeResult", $result);
        ...
        if (isset($result['data'])) {
            if($result['data']['status'] == self::WITHDRAWAL_RESULT_CODE){
                ...
                $message = "coinspaid usdte withdrawal successful. Txid: ". $txid .", Transfered: ".$bitcoin.", sender_amount: ".$sender_amount;

                return array('success' => true, 'message' => $message);
            }else {
                $message = "coinspaid usdte response failed. [".json_encode($result)."]: ";
    ...
    public function getBankInfo() {
        ...
        if(!empty($bankInfoArr)) {
            ...
            $this->utils->debug_log("==================getting withdrawal coinspaid_usdte bank info from extra_info: ", $bankInfo);
        } else {
            $bankInfo = array(
                '42'  => 'usdte',
            );
            $this->utils->debug_log("==================getting withdrawal coinspaid_usdte bank info from code: ", $bankInfo);
        }
        return $bankInfo;
    }
}