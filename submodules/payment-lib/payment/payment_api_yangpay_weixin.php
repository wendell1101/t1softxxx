 ...
 */
class Payment_api_yangpay_weixin extends Abstract_payment_api_yangpay {
	...
	public function getPlatformCode() {
		...
		// return YANGPAY_WEIXIN_PAYMENT_API;
...
$this->CI->utils->debug_log("=====================YANGPAY configParams.parent::pay_productname", parent::getSystemInfo("pay_productname"));
	 ...
	 * @return string The XML content source code.
	 */
	public function genXML(){
		...
		/// fixed fields,
		$xml->addChild('note', 'NULL'); // if needed for itdesk, then description it.
		...
		$xml->addChild('live_key', '## Key ##'); // "## Key ##" for itdesk setup.
		$xml->addChild('live_secret', 'NULL');
		$xml->addChild('sandbox_key', 'NULL');
		$xml->addChild('sandbox_secret', 'NULL');
		$xml->addChild('live_mode', 1);  // fixed, 1. for payment provider online.
		$xml->addChild('second_url', 'NULL');
		$xml->addChild('sandbox_account', 'NULL');
		$xml->addChild('live_account', '## Merchant ID ##'); // "## Merchant ID ##" for itdesk setup.

		// depence by payment and billing channel
		// <system_code>yangpay_unionpay</system_code> // orig
		$system_code = array();
