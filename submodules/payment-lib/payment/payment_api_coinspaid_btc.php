 ...
 */
class Payment_api_coinspaid_btc extends Abstract_payment_api_coinspaid {

    public function getPlatformCode() {
        return COINSPAID_BTC_PAYMENT_API;
    }

    public function getPrefix() {
        return 'coinspaid_btc';
    }
    ...
    public function generatePaymentUrlForm($orderId, $playerId, $amount, $orderDateTime, $playerPromoId = null, $enabledSecondUrl = true, $bankId = null) {
        ...
        #----Create Address----
            ...
            $this->CI->utils->debug_log('=====================coinspaid btc generatePaymentUrlForm params', $params);

            $response = $this->submitPostForm($create_address_url, $params, true, $secure_id);
            $response = json_decode($response, true);
            $this->CI->utils->debug_log('=====================coinspaid btc response', $response);

            if($response){
                if(isset($response['data'])){
                    $cryptoRate = json_decode($this->getCryptoRate($params), true);
                    $this->CI->utils->debug_log('=====================coinspaid btc cryptoRate', $cryptoRate);
                    if (!empty($cryptoRate)) {
                    ...
                    $this->CI->utils->debug_log('=====================coinspaid btc calculateCrypto', $calculateCrypto);
                    ...
                    $this->utils->debug_log('=====================coinspaid btc deposit_notes', $deposit_notes);
    ...
    protected function handlePaymentFormResponse($handle) {
        ...
        $this->CI->utils->debug_log("=====================coinspaid btc handlePaymentFormResponse params", $data);
