 ...
 */
class Payment_api_xifpay_withdrawal extends Abstract_payment_api_xifpay {
	const CALLBACK_STATUS_SUCCESS = 1;

	public function getPlatformCode() {
		return XIFPAY_WITHDRAWAL_PAYMENT_API;
	}

	public function getPrefix() {
		return 'xifpay_withdrawal';
	}
	...
	public function getWithdrawParams($bank, $accNum, $name, $amount, $transId) {
		...
		$batchContentString = "1";
		foreach ($batchContent as $value) {
  			$batchContentString .= ',' . $value;
		}

		$params['batchContent'] = $batchContentString;
	...
	public function submitWithdrawRequest($bank, $accNum, $name, $amount, $transId) {
		...
		if(!array_key_exists($bank, $bankInfo)) {
			$this->utils->error_log("========================xifpay withdrawal bank whose bankTypeId=[$bank] is not supported by xifpay");
			return array('success' => false, 'message' => 'Bank not supported by xifpay');
			$bank = '无';
		}
		...
		$this->CI->utils->debug_log('======================================xifpay withdrawal url: ', $url );
		...
		$this->submitPreprocess($params, $content, $url, $response, array('errCode' => $errCode, 'error' => $error, 'statusCode' => $statusCode), $transId);

		$decodedResult = $this->decodeResult($response);
		$this->utils->debug_log("===============================xifpay withdrawal decoded Result", $decodedResult);
	...
	public function decodeResult($resultString, $queryAPI = false) {
		...
		if($result['respCode'] == 'S0001') {
			$message = 'xifpay payment response successful, respCode: '.$result['respCode'];
			...
			if($queryAPI) {	//checkWithdrawStatus result
				...
				$this->utils->debug_log("=========================xifpay checkWithdrawStatus batchContentArr", $batchContentArr);
				...
				switch ($tradeFeedbackcode) {
					case 'null':
						$message = 'XIFPAY_WITHDRAWAL(喜付代付) 处理中, 商户订单号: ['. $tradeCustorder .']';
						break;

					case '成功':
						$success = true;
						$message = 'XIFPAY_WITHDRAWAL(喜付代付) 成功, 商户订单号: ['. $tradeCustorder .']';
						break;

					case '失败':
						$message = 'XIFPAY_WITHDRAWAL(喜付代付) 失敗, 原因: ['. $tradeReason .'], 商户订单号: ['. $tradeCustorder .']';
						break;

					default:
						$message = 'xifpay payment check withdraw status and got an unknown tradeFeedbackcode, tradeFeedbackcode: '.$tradeFeedbackcode;
						break;
				}
			...
			$this->errMsg = 'xifpay payment failed for unknown reason';
		}
	...
	public function getBankInfo() {
        ...
        if(!empty($bankInfoArr)) {
            ...
            $this->utils->debug_log("=========================xifpay bank info from extra_info: ", $bankInfo);
        } else {
			$bankInfo = array(
			...
			$this->utils->debug_log("=======================getting xifpay bank info from code: ", $bankInfo);
		}
		return $bankInfo;
	}
