 ...
 */
class Game_api_v2_sports extends Game_api_v2_commmon_ipm {
    ...
    public function queryPlayerBalance($playerName) {
        ...
        $params = array(
            ...
            "ProductWallet"     => self::PRODUCT_CODE_IM_SPORTSBOOK
        );
    ...
    public function depositToGame($playerName, $amount, $transfer_secure_id=null) {
        ...
        $data = array(
                ...
                "product_code" => self::PRODUCT_CODE_IM_SPORTSBOOK,
            );
    ...
    public function processResultForSyncOriginalGameLogs($params) {
        // $this->CI->load->model(array('ipm_v2_game_logs', 'player_model'));
        ...
        if(!empty($rows)){
            ...
            foreach ($rows as $record) {
                ...
                if ($update_type=='update') {
                    $this->CI->original_game_logs_model->updateRowsToOriginal('ipm_v2_game_logs', $record);
                } else {
                    unset($record['id']);
                    $this->CI->original_game_logs_model->insertRowsToOriginal('ipm_v2_game_logs', $record);
                }
                $dataCount++;
                unset($data);
            }
    ...
    public function syncLostAndFound($token) {
        $this->CI->load->model(array('game_logs','ipm_v2_game_logs'));
        ...
        $result = $this->CI->ipm_v2_game_logs->getOpenLogs($queryStartDate,$queryEndDate);
        // if(!empty($result)) {
        ...
        if(!empty($creationDate)) {
            ...
            if(count($creationDate) > 1) {
                $this->CI->utils->loopDateTimeStartEnd($startDate, $endDate, '+10 minutes', function($startDate, $endDate) {
                    ...
                    $resultByRange = $this->CI->ipm_v2_game_logs->getOpenLogs($startDate,$endDate);
    ...
    public function queryOriginalGameLogs($dateFrom, $dateTo, $use_bet_time){
        $sqlTime='`ipm_v2_game_logs`.`LastUpdatedDate` >= ?
          AND `ipm_v2_game_logs`.`LastUpdatedDate` <= ?';
        if($use_bet_time){
            $sqlTime='`ipm_v2_game_logs`.`WagerCreationDateTime` >= ?
          AND `ipm_v2_game_logs`.`WagerCreationDateTime` <= ?';
        }

        $sql = <<<EOD
        SELECT 
          `ipm_v2_game_logs`.`id` AS sync_index,
          `ipm_v2_game_logs`.`PlayerId` AS gameUsername,
          `ipm_v2_game_logs`.`external_uniqueid`,
          `ipm_v2_game_logs`.`WagerCreationDateTime` as bet_time,
          `ipm_v2_game_logs`.`LastUpdatedDate` AS game_date,
          `ipm_v2_game_logs`.`GameID` AS game_code,
          `ipm_v2_game_logs`.`response_result_id`,
          `ipm_v2_game_logs`.`PayoutAmount` AS payout_amount,
          `ipm_v2_game_logs`.`StakeAmount` AS bet_amount,
          `ipm_v2_game_logs`.`WinLoss` AS result_amount,
          `ipm_v2_game_logs`.`Platform` AS platform,
          `ipm_v2_game_logs`.`isSettled` AS is_settled,
          `ipm_v2_game_logs`.`isConfirmed` AS is_confirmed,
          `ipm_v2_game_logs`.`isCancelled` AS is_cancelled,
          `ipm_v2_game_logs`.`BetId` AS RoundID,
          `ipm_v2_game_logs`.`SportsName` AS sports_name,
          `ipm_v2_game_logs`.`DetailItems` AS bet_details,
          `ipm_v2_game_logs`.`md5_sum`, 
        ...
        FROM
          ...
          LEFT JOIN `game_description` 
            ON ipm_v2_game_logs.GameID = game_description.external_game_id 
          ...
          JOIN `game_provider_auth` 
            ON (
              `ipm_v2_game_logs`.`PlayerId` = `game_provider_auth`.`login_name` 
...
EOD;
    ...
    public function getGameRecordsStatus($row) {
        ...
        if($row['is_settled'] == self::SETTLED && $row['is_confirmed'] == self::Confirmed && $row['is_cancelled'] == self::NOT_CANCEL){
            ...
            if($row['is_confirmed'] == self::Confirmed && $row['is_cancelled'] == self::CANCEL){//confirmed but canceled
                $status = Game_logs::STATUS_CANCELLED;
            }
        }
        return $status;
