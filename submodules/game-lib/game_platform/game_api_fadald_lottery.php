 ...
 */
class Game_api_fadald_lottery extends Abstract_game_api {
    ...
    const ORIGINAL_LOGS_TABLE_NAME = "fadald_lottery_game_logs";
    ...
    public function processResultBoolean($responseResultId, $resultJson, $playerName) {
        ...
        if(empty($resultJson['success'])) {
            ...
            $this->CI->utils->debug_log("==========FADA LD Lottery API GOT ERROR=============", $resultJson['msg'], $playerName);
            return false;
        }
        return $success;
    }
    ...
    public function convertGameOddAmountToServerOddAmount($amount) {
        return $amount / 1000;      // new update OGP-9925
    }
    ...
    public function _continueSync($startDate, $endDate, $page = 1) {
        ...
        $this->CI->utils->debug_log('=====> FADA Lottery syncLdLotteryOriginalGames result ' . json_encode($result) . "for page {$page}");
    ...
    */
    public function takeSleep(){
        ...
        if($this->interval_seconds_on_sync_original>0){
            $this->CI->utils->info_log('FADA sleeping '. $this->interval_seconds_on_sync_original .' seconds for sync original,please wait');
    ...
    public function rebuildOriginalLogs(&$records, $responseResultId) {
        ...
        foreach ($records as $key => $record) {
            $logs = array(
                ...
                'odds'                  => $this->convertGameOddAmountToServerOddAmount($record['Odds']),
                ...
                //extra info from SB,
                'external_uniqueid'     => (string) $record['OrderNo'],
            ...
            foreach ($data as $record) {
                if ($queryType == 'update') {
                    ...
                    if($this->use_insert_ignore){
                        $this->CI->original_game_logs_model->insertIgnoreRowsToOriginal(self::ORIGINAL_LOGS_TABLE_NAME, $record);
                    }else {
    ...
    public function syncRecords($gameRecords, $responseResultId) {
        ...
        if (!empty($gameRecords)) {
            ...
            foreach ($gameRecords as $row) {
                    ...
                    ))
                {
                    ...
                    $data['odds'] =  $this->convertGameOddAmountToServerOddAmount($row['Odds']);
        ...
        $existingRecord = $this->CI->fadald_lottery_game_logs->getExistingBetIds($externalUniqueIds);

        # Update data
        if (!empty($existingRecord)) {
            foreach ($existingRecord as $rec) {
                $this->CI->fadald_lottery_game_logs->updateGameLog($rec['id'], $map[$rec['order_no']]);
        ...
        if (!empty($map)) {
            foreach ($map as $key => $row) {
                $this->CI->fadald_lottery_game_logs->insertGameLogs($row);
                $count++;
            }
        }
    ...
    public function queryForwardGame($playerName, $extra=null) {
        ...
        if($result['success']) {
            $language = !empty($extra['fixed_language']) ? $extra['fixed_language'] :  $this->getGameLanguage($extra['language']);
...
EOD;
    ...
    public function makeParamsForInsertOrUpdateGameLogsRow(array $row){
     ...
     public function prepareOriginalRows(array &$rows){
        ...
        $this->CI->db->select('round_key, odds, win_lose,payout_amount,bet_amount, play_value, cmd as bet_placed, winloss_amount AS result_amount')
            ->from('fadald_lottery_game_logs')->where_in('round_key', $roundKeyList);
        $qry=$this->CI->db->get();
    ...
    public function preprocessOriginalRowForGameLogs(array &$row){
        ...
        // $gameResult = $this->getByRoundIdAndGameUsername($row['round_id'], $row['player_username']);
        ...
        //     );
        // } else {
        //     $bet_details = array('bet_details' => $this->getBetDetailsWithSameRoundId($gameResult));
        // }
    ...
    // public function getByRoundIdAndGameUsername($roundId, $gameUsername){
    ...
    private function getGameDescriptionInfo($row, $unknownGame) {
        ...
        $extra = array('game_code' => $external_game_id,'game_name' => $row['game']);
    ...
    public function isDemoMode($extra){
        $extra = is_null($extra)?array():$extra;
        if(array_key_exists('game_mode', $extra)){
            return in_array(strtolower($extra["game_mode"]), self::TRIAL_MODE_LIST);
        }else{
            return false;
        }
    }
