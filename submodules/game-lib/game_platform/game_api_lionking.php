...
class Game_api_lionking extends Abstract_game_api {
    ...
    public function __construct() {
        parent::__construct();
        $this->original_game_logs_table = 'lionking_game_logs';
    ...
    public function processResultBoolean($responseResultId, $resultArr, $extra = null) {
        ...
        if(!$success) {
            ...
            $this->CI->utils->debug_log('LIONKING API got error processResultBoolean', $responseResultId, 'processResultBoolean_result', $resultArr);
        }
	...
	public function queryForwardGame($playerName, $extra) {
        ...
        $usercode = strtoupper($gameUsername);
        $password = $this->agent_secret_key;
        $scheme = '';
        $packageName = '';
		$timestamp = strval($this->CI->utils->getTimestampNow());
        ...
        $responseSchemeData = json_encode($params);
        $baseStr = base64_encode($responseSchemeData);
        ...
        $this->CI->utils->debug_log(__METHOD__, 'GetLoginTokenApp_result', $result);
...
FROM
    ...
    LEFT JOIN game_description ON ogl.gameName = game_description.game_code AND game_description.game_platform_id = ?
