 ...
 */
class Game_api_common_isb extends Abstract_game_api {
	...
	public function __construct() {
		...
		$this->use_extrainfo_currency = $this->getSystemInfo('use_extrainfo_currency',false);
		...
		$this->allow_fullscreen_in_mobile = $this->getSystemInfo('allow_fullscreen_in_mobile',true);
		$this->allow_fullscreen_in_desktop = $this->getSystemInfo('allow_fullscreen_in_desktop',false);
		$this->table_betlimit = $this->getSystemInfo('table_betlimit',6);
	...
	const SIMPLE_MD5_FIELDS_FOR_ORIGINAL=['playerid', 'status', 'type','transactionid','amount'];
	...
	public function generateUrl($apiName, $params) {
		$this->CI->utils->debug_log('>==== ISB-JSON-PARAMS => ', json_encode($params));
    ...
    private function getPlayerISBCurrency($username){
		...
		if(!is_null($playerId) && !$this->use_extrainfo_currency){
	...
	public function createPlayer($playerName, $playerId, $password, $email = null, $extra = null) {
		...
		$params = array(
			...
			"action" => array(
				"command" => "addPlayer",
				"parameters" => array(
					"real" => $this->getSystemInfo('isb_createplayer_demo_mode_flag'),
				),
			),
		);
    ...
    protected function isErrorCode($apiName, $params, $statusCode, $errCode, $error) {
        ...
        $isErr=$errCode || intval($statusCode, 10) >= 400;

    	if($apiName==self::API_depositToGame && $statusCode==504){
    		//504 means success
    		$isErr=false;
    	}
	...
	public function processResultForDepositToGame($params) {
		...
		if($this->enabled_504_deposit_eq_success){
    ...
    public function queryForwardGame($playerName, $param) {
        ...
        if (empty($playerName)) {
        	$url = $this->isb_game_url . "/" . $this->isb_api_license_id . "/" . $param['game_code'] . "?lang=" . $language . '&cur=' . $this->isb_currency . '&mode=' . $game_mode . "&table=" . $this->table_betlimit;
        } else {
        	$playerName = $this->getGameUsernameByPlayerUsername($playerName);
	        $data = $this->getPlayerISBToken($playerName);

	        if (!empty($data['result'])) {
	            $url = $this->isb_game_url . "/" . $this->isb_api_license_id . "/" . $param['game_code'] . "?lang=" . $language . '&cur=' . $this->getPlayerISBCurrency($playerName) . '&mode=' . $game_mode . "&table=" . $this->table_betlimit;
		...
		$lobby_url = (@$param['extra']['is_mobile_flag']||@$param['is_mobile'])
        ...
        # add this param for mobile
        if (@$param['extra']['is_mobile_flag'] || @$param['is_mobile']) {
        	$url .= "&allowFullScreen=".$this->allow_fullscreen_in_mobile;
    	}else{
    		if($this->allow_fullscreen_in_desktop){
    			$url .= "&allowFullScreen=".$this->allow_fullscreen_in_desktop;
    		}
    	}
        ...
        return array(
            ...
            'url' => $url,
            'iframeName' => "ISB API",
            'is_redirect' => (@$param['extra']['is_mobile_flag']||@$param['is_mobile'])?true:false
        );
    }
  ...
  //                   $extra
	...
	public function makeParamsForInsertOrUpdateGameLogsRow(array $row){
        ...
        $row['after_balance'] = $this->gameAmountToDB($this->convertAmountToSbeMoney($row['after_balance']));
