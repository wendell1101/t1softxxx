...
/**
	...
	* API docs: http://mucho.oriental-game.com:8058
	*
...
class Game_api_og_v2 extends Abstract_game_api {
	const POST = 'POST';
	const GET = 'GET';
	...
	public function __construct() {
		...
		$this->URI_MAP = array(
			...
			self::API_updatePlayerInfo => '/game-providers/1/setBetLimit',
			self::API_queryPlayerBalance => '/game-providers/1/balance',
			self::API_isPlayerExist => '/game-providers/1/balance',
	        self::API_depositToGame => '/game-providers/1/balance',
	        self::API_withdrawFromGame => '/game-providers/1/balance',
	        self::API_queryTransaction => '/Transfer',
	        self::API_login => '/game-providers/1/games/oglive/key',
	        self::API_queryForwardGame => '/game-providers/1/play',
	...
	protected function getHttpHeaders($params){
		...
		if(!$this->data_api){
			if($this->currentAPI == self::API_generateToken){
				...
				$this->CI->utils->debug_log('OG_V2_API (getHttpHeaders)', $this->method);
				return $headers;
			}			

			$clone = clone $this;
			$headers['X-Token'] =  $clone->getAvailableApiToken();
			$this->CI->utils->debug_log('OG_V2_API (getHttpHeaders) clone:', $clone->method);
		}
	...
	public function generateUrl($apiName, $params) {
		...
		$this->CI->utils->debug_log('OG_V2_API (generateUrl) :', $this->method, $url);
	...
	protected function customHttpCall($ch, $params) {		
		...
		$this->utils->debug_log('OG V2 REQUEST FIELD: ', $this->method, http_build_query($params));
	}
	...
	public function processResultBoolean($responseResultId, $resultArr, $playerName = null,$is_querytransaction= false) {
		...
		if (!$success) {
			...
			$this->CI->utils->debug_log('OG_V2_API got error ', $responseResultId, 'playerName', $playerName, 'result', $resultArr);
		}
		return $success;
	}
	...
	public function createPlayer($playerName, $playerId, $password, $email = null, $extra = null) {
		...
		$this->CI->utils->debug_log('OG_V2_API (createPlayer) :', $params, $this->method);
	...
	public function processResultForCreatePlayer($params){
		...
		if($success){
	        ...
	        $this->updatePlayerInfo($playerName, []);
	        $result['exists'] = true;
		}
	...
	public function updatePlayerInfo($playerName, $infos) {
		...
		$params = array(
			...
			'limit' => array(
				'video' => $this->limitvideo,
				'roulette' => $this->limitroulette
			)
		);
	...
	public function processResultForUpdatePlayerInfo($params){
		...
		if($success){
			$result["message"] = lang("Successfully Update Setting");
		}
    ...
    public function processResultForIsPlayerExist($params){
		...
		if($success){
			...
			if(@$resultArr['data']['message'] == 'User not found.'){
				$result['exists'] = false;
			}else{
				$result['exists'] = null;
			}
	...
	private function rebuildGameRecords(&$gameRecords,$extra){
        ...
        foreach($gameRecords as $index => $record) {
        	...
        	$gammeusername = explode("_",$record['membername']);
			$new_gameRecords[$index]['gammeusername'] = isset($gammeusername[1])?$gammeusername[1]:reset($gammeusername);
			// preg_match_all('/\d+/', $record['bettingdate'], $bettingdate);
			// $newbettingdate = $bettingdate[0][0]/1000;
			// $new_gameRecords[$index]['bettingdate'] = date('Y-m-d H:i:s',$newbettingdate);
        ...
        if(!empty($rows)){
            ...
            foreach ($rows as $record) {
                if ($update_type=='update') {
                    $this->CI->original_game_logs_model->updateRowsToOriginal('og_v2_game_logs', $record);
                } else {
                    unset($record['id']);
                    $this->CI->original_game_logs_model->insertRowsToOriginal('og_v2_game_logs', $record);
                }
                $dataCount++;
                unset($data);
            }
    ...
    public function queryOriginalGameLogs($dateFrom, $dateTo, $use_bet_time){
        $sqlTime='`og_v2`.`updated_at` >= ?
          AND `og_v2`.`updated_at` <= ?';
        if($use_bet_time){
            $sqlTime='`og_v2`.`bettingdate` >= ?
          AND `og_v2`.`bettingdate` <= ?';
        }

        $sql = <<<EOD
			SELECT
				og_v2.id as sync_index,
				og_v2.response_result_id,
				og_v2.bettingcode as round,
				og_v2.gammeusername as username,
				og_v2.bettingamount as bet_amount,
				og_v2.validbet as valid_bet,
				og_v2.balance as after_balance,
				og_v2.winloseamount as result_amount,
				og_v2.bettingdate as start_at,
				og_v2.bettingdate as end_at,
				og_v2.bettingdate as bet_at,
				og_v2.gameid as game_code,
				og_v2.gamename as game_name,
				og_v2.updated_at,
				og_v2.external_uniqueid,
				og_v2.md5_sum,
				game_provider_auth.player_id,
				gd.id as game_description_id,
				gd.game_name as game_description_name,
				gd.game_type_id
			FROM og_v2_game_logs as og_v2
			LEFT JOIN game_description as gd ON og_v2.gameid = gd.external_game_id AND gd.game_platform_id = ?
			LEFT JOIN game_type as gt ON gd.game_type_id = gt.id
			JOIN game_provider_auth ON og_v2.gammeusername = game_provider_auth.login_name
