...
class Abstract_game_api_common_won extends Abstract_game_api {
    const ORIGINAL_LOGS_TABLE_NAME = 'won_game_logs';
    ...
    const API_queryBetLimit = 'queryBetLimit';
    ...
    public function __construct(){
        ...
        $this->page_size = $this->getSystemInfo('page_size', self::PAGE_SIZE);
        ...
        $this->URI_MAP = array(
            ...
            self::API_queryBetLimit => '/api/quota/list',
        );
        ...
        $this->METHOD_MAP = array(
            ...
            self::API_queryBetLimit => self::GET
		);

        $this->bet_limit=$this->getSystemInfo('bet_limit', "21,26,60");
        $this->is_create = $this->getSystemInfo('is_create', 1);# 1: true, 0: false, default : 1
    ...
    protected function customHttpCall($ch, $params){
        ...
        $this->utils->debug_log('WON Request Field: ',http_build_query($params));
    }
    ...
    public function processResultBoolean($responseResultId, $resultArr, $statusCode){
        ...
        if (!$success) {
            ...
            $this->CI->utils->debug_log('WON GAME API got error: ', $responseResultId,'result', $resultArr, $statusCode);
        }
    ...
    public function createPlayer($playerName, $playerId, $password, $email = null, $extra = null){
		$this->CI->utils->debug_log('WON (createPlayer)', $playerName);
        ...
        if(!isset($extra['language'])){
            $extra['language'] = $this->CI->language_function->getCurrentLanguage();
        }
        ...
        $stringSignTemp = "appid=".$this->app_id."&clienttype=".$clienttype."&iscreate=".$isCreate."&language=".$language."&username=".$gameUsername.
        "&key=".$this->secret_key;

        //$this->CI->utils->debug_log('WON (createPlayer)', 'stringSignTemp', $stringSignTemp);

        $md5 = md5($stringSignTemp);

        //$this->CI->utils->debug_log('WON (createPlayer)', 'md5', $md5);

        //$params['sign'] = md5($stringSignTemp);
        $params['sign'] = $this->generateSign($params);

		//$this->CI->utils->debug_log('WON (createPlayer) :', $params);
    ...
    public function isPlayerExist($playerName){
        $this->CI->utils->debug_log('WON (isPlayerExist)', $playerName);
        ...
        $this->CI->utils->debug_log('WON (isPlayerExist) :', $params);
    ...
    public function queryPlayerBalance($playerName){
        $this->CI->utils->debug_log('WON (queryPlayerBalance)', $playerName);
        ...
        $this->CI->utils->debug_log('WON (queryPlayerBalance) :', $params);
    ...
    public function processResultForQueryPlayerBalance($params){
        ...
        if($success){
            if(isset($resultArr['balance'])){
                $result['balance'] = $this->gameAmountToDBTruncateNumber(($resultArr['balance']));
            }else{
                $success = false;
            }
        }
    ...
    public function depositToGame($playerName, $amount, $transfer_secure_id=null){
		$this->CI->utils->debug_log('WON (depositToGame)', $playerName);
        ...
        $this->CI->utils->debug_log('WON (depositToGame) :', $params);
    ...
    public function withdrawFromGame($playerName, $amount, $transfer_secure_id = null){
        $this->CI->utils->debug_log('WON (withdrawFromGame)', $playerName);
        ...
        $this->CI->utils->debug_log('WON (withdrawFromGame) :', $params);
    ...
    public function processResultForWithdrawFromGame($params){
        ...
        if(!isset($resultArr['balance'])){
            ...
            $this->CI->utils->error_log('WON ERROR WITHDRAWAL RESPONSE', 'resultArr', $resultArr);
        }
    ...
    public function queryForwardGame($playerName, $extra){
        $this->CI->utils->debug_log('Player Details: ',$extra,$playerName);
        $gameUsername = $this->getGameUsernameByPlayerUsername($playerName);
        ...
        $gameCode = !empty($extra['game_code'])?(int)$extra['game_code']:null;
    ...
    public function syncOriginalGameLogs($token){
        $this->CI->utils->debug_log('WON (syncOriginalGameLogs)', $token);

        $startDate = clone parent::getValueFromSyncInfo($token, 'dateTimeFrom');
        $endDate = clone parent::getValueFromSyncInfo($token, 'dateTimeTo');

        $this->CI->utils->info_log('<-------------------------PARAMS------------------------->', 'startDate', $startDate->format('Y-m-d H:i:s'), 
        ...
        $this->CI->utils->info_log('<-------------------------PARAMS------------------------->', 'startDateTime', $startDateTime->format('Y-m-d H:i:s'), 
        'endDateTime', $endDateTime->format('Y-m-d H:i:s'));

        $startDate = strtotime($startDateTime->format('Y-m-d H:i:s'));
        $endDate   = strtotime($endDateTime->format('Y-m-d H:i:s'));

        $this->CI->utils->info_log('<-------------------------PARAMS------------------------->', 'startDate', $startDate, 
        'endDate', $endDate);

        $this->CI->utils->debug_log('WON (timestamp)', $startDate, $endDate);
       ...
       $this->method = self::GET;
        ...
        for($currentPage=0;$currentPage<=$totalPage;$currentPage++){
           ...
           $this->CI->utils->info_log('gameRecords params', $params);
           $api_result = $this->callApi(self::API_syncGameRecords, $params, $context);

           $this->CI->utils->info_log('gameRecords api_result', $api_result);

            if (isset($api_result['totalPage'])) {
                $totalPage = isset($api_result['totalPage']) ? $api_result['totalPage'] : 0;
            }else{
                $this->CI->utils->error_log(__METHOD__.' Error sync gameRecords', 'params', $params);
            }

            $this->CI->utils->debug_log(__METHOD__.' gameRecords', 'currentPage: ',$currentPage, 'totalPage', 
            $totalPage, 'params',$params);
    ...
    public function processResultForSyncOriginalGameLogs($params){
        ...
        $total = isset($resultArr['total'])?$resultArr['total']:0;

        if($total>$this->page_size){
            $result['totalPage'] =  intval($total/$this->page_size);
        }else{
            $result['totalPage'] = 1;
        }

        //$this->CI->utils->debug_log(__METHOD__, 'gameRecords', $gameRecords);

        if($success && !empty($gameRecords)){
            $extra = ['response_result_id' => $responseResultId];

            $this->rebuildGameRecords($gameRecords, $extra);
            //$this->CI->utils->info_log('gameRecords bermar', $gameRecords);
        ...
        $this->CI->utils->debug_log('bermar result', $result);
    ...
    private function rebuildGameRecords(&$gameRecords, $extra){
        if(!empty($gameRecords)){
            foreach($gameRecords as $index => $gr){
                ...
                $data['betodds'] = isset($gr['betodd']) ? $gr['betodd'] : null;
    ...
    public function queryOriginalGameLogs($dateFrom, $dateTo, $use_bet_time){
        ...
        $this->CI->utils->debug_log('WON sqlTime ===>', $sqlTime);
        ...
        SELECT
	        ...
	        gd.game_type_id
        FROM won_game_logs as original
...
EOD;
    ...
    public function queryBetLimit($extra = NULL) {

        $context = array(
            'callback_obj'    => $this,
            'callback_method' => 'processResultForQueryBetLimit'
        );
        ...
        return $this->callApi(self::API_queryBetLimit, $params, $context);
    }

    public function processResultForQueryBetLimit($params) {
		$statusCode = $this->getStatusCodeFromParams($params);
		$resultArr = $this->getResultJsonFromParams($params);
		$responseResultId = $this->getResponseResultIdFromParams($params);
		$success = $this->processResultBoolean($responseResultId, $resultArr, $statusCode);
    ...
    private function updateOrInsertGameList($data, $queryType){
        ...
        if (!empty($data)) {
            $caption = [];
            if ($queryType == 'update') {
                $caption = "## UPDATE WON CASINO GAME LIST\n";
            }
            else {
                $caption = "## ADD NEW WON CASINO GAME LIST\n";
            }
