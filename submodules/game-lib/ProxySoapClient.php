...
class ProxySoapClient extends SoapClient {
	...
	private $soap_connection_timeout=null;
	private $error=null;
	...
	public function __construct($url, $options = null) {
		...
		if ($options) {
			...
			if (array_key_exists('soap_connection_timeout', $options)) {
				$this->soap_connection_timeout = $options['soap_connection_timeout'];
			}
		...
		if (empty($this->soap_connection_timeout)) {
			$this->soap_connection_timeout = $this->CI->config->item('default_connect_timeout');
		}
	...
	protected function getConnectTimeoutSecond() {
		return $this->soap_connection_timeout;
	}
	...
	protected function callCurl($url, $data, $action) {
        ...
        if($use_timeout_ms_on_curl){
        	curl_setopt($handle, CURLOPT_CONNECTTIMEOUT_MS, $this->getConnectTimeoutSecond()*1000);
    	}else{
