function() {
    ...
    // Constants 
    var getStatusUrl = window.location.hostname+'/whl_bti_service_api/status'; // line 52 
    var getRefreshUrl = window.location.hostname+'/whl_bti_service_api/refresh' //line 53 
    var URL_PARAM_NAME_TOKEN = "operatorToken"; // Do not edit 
    var COOKIE_PARAM_NAME_TOKEN = "operatorToken"; // Do not edit 
    var SESSION_PARAM_NAME_TOKEN = "operatorToken"; // Do not edit 
    // Init token 
    ...
    whl.prototype.status = function(callback) {
        ...
        if (strToken != "logout" && strToken != "") {
            ...
            xmlhttp.open("GET", getStatusUrl + '?stoken=' + strToken + '&r=' + new Date().getTime(), true);
            xmlhttp.send();
        } else {
            that.logout();
            UserInfo.logout();
    ...
    whl.prototype.refreshSession = function(callback) {
        ...
        if (strToken != "logout" && strToken != "") {
            ...
            xmlhttp.open("GET", getRefreshUrl + '?stoken=' + strToken + '&r=' + new Date().getTime(), true);
            xmlhttp.send();
        } else {
            that.logout();
            UserInfo.logout();
