...
;(function ($, document, window){
	...
	defaults = {
		...
		text:{
			finished: 'SIGN UP WITH LENSAWIN' ,
			next: 'NEXT',
			previous: 'GO BACK'
		}
	},
	...
	checkStatus = function(){
		...
		if(that.currentStep == stepsItems.length){
			...
			var	badge = $('ul.steps > li.completed .step-index .label'),
				disableSteps = $('ul.steps > span.label');
				

			$('#stepsIndex > ul > li').removeClass("completed").addClass("done");
			that.$element.find('.btn-next').removeClass(opts.btnClassDefault).addClass('final-step '+ opts.btnClassCompleted);
			$(stepText).hide();
			$(stepFour).show();				
			$('.step-four').replaceWith('<li class="completed" data-step="4"><span class="step-index"><b>SUMMARY</b></span></li>');
			$(badge).addClass('disabled').css("background-color", "#83b752");
			$('.steps > li[data-step="'+ 4 +'"]').css("opacity","1");
			$(disableSteps).addClass('disabled');
			...
			that.$element.find('.btn-prev').addClass('disabled hidden');
			$(stepText).show();		
			$('.steps > li[data-step="'+ 4 +'"]').css("opacity","0");
			$(badge).css("background-color", "#858584");	
		} 

		else if(that.currentStep == 2 || that.currentStep == 3 ){
			$(stepText).show();
			$('.steps > li[data-step="'+ 4 +'"]').css("opacity","0");
			$(badge).css("background-color", "#858584");
	...
	methods = {
		init: function (element, options) {
			...
			if(stepsBar.parent().hasClass('wizard')){
				...
				stepsBar.wrap('<div class="steps-index-container" id="stepsIndex"></div>');				
			} else {
			...
			if(opts.bottomButtons && !bottomActions.length){
				...
				html += '<div class="right-actions"><span class="'+ opts.btnClass +' '+ opts.btnClassDefault +' btn-next"><span class="next-text">'+ opts.text.next +'</span><span class="finished-text"><a href="javascript:void(0)">'+ opts.text.finished +'</a></span></div>';
				html += '</div>';
			...
			for(var i=0;i<stepsLi.length;i++){
				...
				step.empty().html('<span class="step-index"><span class="label">'+ (i+1) +'</span></span>'+ content );
			...
			this.$element.find('.steps > li:nth-child(4)').addClass('final');
		...
		next: function(checkStep,event){
			if(Register.validateSteps(this.currentStep)){
