...
if [ -z "$2" ]; then
    echo "usage: run_ci_cli.sh <controller> <function name> [parameters]"
    exit 1
fi
...
start_time=$(date "+%s")
echo $PHP_PATH $SHELL_DIR/ci_cli.php "cli/$1/$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"

$PHP_PATH $SHELL_DIR/ci_cli.php "cli/$1/$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
end_time=$(date "+%s")
