...
if [ -z "$1" ]; then
	echo "usage: sync_balance_for_one.sh <og_home> <game platform id> [player username]"
	exit 1
fi
...
#call sync records
nohup $PHP_PATH $OG_BASEPATH/public/index.php cli/sync_balance/sync_once "$2" "$3" 2>&1 >> $OG_BASEPATH/application/logs/sync_balance_service_$2.log &
