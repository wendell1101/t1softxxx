...
if [ -z "$1" ]; then
    echo "usage: debug_manually_sync_stream_t1_gamegateway.sh <strt date time> <lastDateTimeFileName>"
    exit 1
fi
...
start_time=$(date "+%s")
$PHP_PATH $SHELL_DIR/ci_cli.php "cli/sync_t1_gamegateway/sync_stream" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
end_time=$(date "+%s")
