...
if [ -z "$1" ]; then
    echo "usage: manually_sync_t1_gamegateway_stream.sh <date time from> <date time to> [player name] [queue_token]"
    exit 1
fi
...
start_time=$(date "+%s")
echo $PHP_PATH $SHELL_DIR/ci_cli.php "cli/command/manually_sync_t1_gamegateway_stream" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"

$PHP_PATH $SHELL_DIR/ci_cli.php "cli/command/manually_sync_t1_gamegateway_stream" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
end_time=$(date "+%s")
