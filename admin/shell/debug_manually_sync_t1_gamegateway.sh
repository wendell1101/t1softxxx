...
if [ -z "$1" ]; then
	echo "usage: debug_manually_sync_t1_gamegateway.sh <dateTimeFromStr> <dateTimeToStr> [playerName] [queue_token] [use_bet_time]"
	exit 1
fi
...
start_time=$(date "+%s")
$PHP_PATH $SHELL_DIR/ci_cli.php "cli/sync_t1_gamegateway/sync" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
end_time=$(date "+%s")
