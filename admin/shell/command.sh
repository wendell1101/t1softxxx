...
if [ -z "$1" ]; then
	echo "usage: command.sh <function name> [parameters]"
	exit 1
fi
...
start_time=$(date "+%s")
echo $PHP_PATH $SHELL_DIR/ci_cli.php "cli/command/$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"

$PHP_PATH $SHELL_DIR/ci_cli.php "cli/command/$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
end_time=$(date "+%s")
