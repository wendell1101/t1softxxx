...
class Lib_report {
    ...
    public function set_data($request, $columns, $table, $form_search = array(), $values = array(), $joins = array(), $group_by = array(), $having = array(), $distinct = true, $external_order=[],
                             $not_datatable = '', $countOnlyField='') {
    ...
    public function get_data($request, $columns, $table, $form_search = array(), $values = array(), $joins = array(), $group_by = array(), $having = array(),
            $distinct = true, $external_order=[], $not_datatable = '', $countOnlyField='') {

        $this->set_data($request, $columns, $table, $form_search, $values, $joins, $group_by, $having, $distinct, $external_order, $not_datatable, $countOnlyField);
        ...
        $this->filter($this->request, $this->columns, $this->form_search, $this->values);
                                ...
                                if ($is_export_csv) {
                                    ...
                                    if(!empty($this->export_token)){
                                        if(in_array($count_loop, $percentage_steps)){
                                            $done = true;
                                            $rlt=['success'=>false, 'is_export'=>true, 'written' => $count_loop, 'total_count' => $totalCount, 'progress' => ceil($count_loop/$totalCount * 100)];
                                            $this->ci->queue_result->updateResultRunning($this->export_token, $rlt, $state);
                                        }
                                        if(($count_loop) == $totalCount){
                                            $rlt=['success'=>true, 'is_export'=>true, 'filename'=>$csv_filename.'.csv','written' => $count_loop, 'total_count' => $totalCount, 'progress' => 100];
