...
/**
 * itniotech SMS API implementation
 * http://central.itniotech.com/index
 *
 * Config items:
 * * sms_api_itniotech_key
 * * sms_api_itniotech_account
 * * sms_api_itniotech_appId
 */
class Sms_api_itniotech extends Abstract_sms_api {
    const SUCCESS_CODE = 0;

    public function getUrl() {
        return $this->getParam('url') ? $this->getParam('url') : 'https://api.itniotech.com/sms/sendSms';
    }
    ...
    public function getFields($mobile, $content, $dialingCode) {
        ...
        $this->utils->debug_log("===============itniotech SMS fields", $fields);
        return $fields;
    }
    ...
    public function isSuccess($returnQueryString) {
        $response = json_decode($returnQueryString,true);
        $this->utils->debug_log("===============sms response itniotech ", $response);
    ...
    protected function getParam($name) {
        return $this->CI->config->item('sms_api_itniotech_'.$name);
    }
}
