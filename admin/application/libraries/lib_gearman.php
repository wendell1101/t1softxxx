...
class Lib_gearman {
	...
	public function __construct() {
        ...
        $config=&get_config();
		$this->gearman_host = $config['gearman_server'];
		$this->gearman_port = $config['gearman_port'];
	...
	public function is_supported() {
		if (!extension_loaded('gearman')) {
			raw_debug_log('error', 'The Gearman Extension must be loaded to use Gearman client.');
	...
	public function add_worker_function($identifier, $function_name) {
		...
		raw_debug_log('debug', "Gearman Library: Successfully added worker function with identifier $identifier with function $function_name");
	...
	public function do_job_background($function, $param, $priority = 'normal') {
		...
		raw_debug_log('debug', "Gearman Library: Performed task with function $function with parameter ".count($param));
	}
	...
	private function _auto_connect($object) {
		...
		foreach ($this->gearman_host as $key => $server) {
			if (!$object->addServer($server, $this->gearman_port[$key])) {
				...
				raw_debug_log('error', 'Gearman Library: Could not connect to the server named "' . $key . '"');
			} else {
				raw_debug_log('debug', 'Gearman Library: Successfully connected to the server named "' . $key . '"');
			}
		}
	}
