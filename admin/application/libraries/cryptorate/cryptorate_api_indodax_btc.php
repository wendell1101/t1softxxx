 ...
 *
 */
class Cryptorate_api_indodax_btc extends Abstract_cryptorate_api {
    ...
    public function convertCryptoCurrency($amount, $base, $target, $paymentType) {
        ...
        if($this->utils->getCurrentCurrency()['currency_code'] == 'IDR'){
            if($paymentType == 'deposit'){
                $rate = $this->getDecimalPlaceSetting($table['tickers']['btc_idr']['buy']);
            }else if($paymentType == 'withdrawal'){
                $rate = $this->getDecimalPlaceSetting($table['tickers']['btc_idr']['sell']);
            }
        ...
        if(!empty($cust_fix_rate) && isset($target_amount)){
            ...
            $this->utils->debug_log("Cryptorate_api_indodax_btc has error");
            return false;
        }
    }

    public function getParam($name) {
        return $this->CI->config->item('cryptorate_api_indodax_btc_'.$name);
    }
}
