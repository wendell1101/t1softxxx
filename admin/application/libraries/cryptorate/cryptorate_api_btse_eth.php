 ...
 *
 */
class Cryptorate_api_btse_eth extends Abstract_cryptorate_api {
    ...
    public function convertCryptoCurrency($amount, $base, $target, $paymentType) {
        ...
        foreach ($rate_list as $key => $value) {
            if($rate_list[$key]['symbol'] == "ETH-CNY"){
        ...
        if(!empty($cust_fix_rate) && isset($target_amount)){
            ...
            $this->utils->debug_log("Cryptorate_api_btse_eth has error");
            return false;
        }
    }

    protected function getParam($name) {
        return $this->CI->config->item('cryptorate_api_btse_eth_'.$name);
    }
}
