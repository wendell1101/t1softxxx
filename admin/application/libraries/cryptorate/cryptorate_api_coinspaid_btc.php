 ...
 *
 */
class Cryptorate_api_coinspaid_btc extends Abstract_cryptorate_api {
    ...
    public function convertCryptoCurrency($amount, $base, $target, $paymentType) {
        ...
        $this->CI->utils->debug_log('=====================coinspaid btc crypto params', $params, $amount, $base, $target, $paymentType);

        $cryptoRate = json_decode($this->getCryptoRate($params), true);
        $this->CI->utils->debug_log('=====================coinspaid btc crypto cryptoRate', $cryptoRate);
        if (!empty($cryptoRate) && !empty($cryptoRate['data'])) {
            $rate = $cryptoRate['data'][0]['rate_to'];
        }

        if(!empty($rate)){
            $cust_fix_rate = $this->getDecimalPlaceSetting($rate * $this->getCustFixRate(self::COIN_BTC, $paymentType));
            if(strtoupper($base) == self::COIN_BTC){
                $target_amount = $this->getDecimalPlaceSetting($amount / $cust_fix_rate);
            }elseif(strtoupper($base) == $currency){
                $target_amount = $this->getDecimalPlaceSetting($amount * $cust_fix_rate);
            }
        }else{
            $this->utils->debug_log("=====================coinspaid btc crypto getCryptoRate fail");
            return false;
        }

        $this->utils->debug_log("=====================coinspaid btc crypto rate and target amount", $cust_fix_rate, $target_amount);

        if(!empty($cust_fix_rate) && isset($target_amount)){
            return array($target_amount, $cust_fix_rate);
        }else{
            $this->utils->debug_log("=====================coinspaid btc crypto convertCryptoCurrency fail");
            return false;
        }
    }
    ...
    public function getHeaders($params){
        ...
        $this->utils->debug_log('=====================coinspaid btc crypto getHeaders', $headers);
        return $headers;
    }
    ...
    protected function getParam($name) {
        return $this->CI->config->item('cryptorate_api_coinspaid_btc_'.$name);
    }
}
