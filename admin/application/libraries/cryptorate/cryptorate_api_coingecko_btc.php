 ...
 *
 */
class Cryptorate_api_coingecko_btc extends Abstract_cryptorate_api {

    public function getUrl() {
        $defaultCurrency  = $this->utils->getCurrentCurrency()['currency_code'];
        return $this->getParam('url') ? $this->getParam('url') : 'https://api.coingecko.com/api/v3/simple/price?ids=Bitcoin&vs_currencies='.$defaultCurrency;
    }
    ...
    public function convertCryptoCurrency($amount, $base, $target, $paymentType) {
        ...
        if($this->utils->getCurrentCurrency()['currency_code'] == 'VND'){
            $rate = $this->getDecimalPlaceSetting($table['bitcoin']['vnd']);
        }else if($this->utils->getCurrentCurrency()['currency_code'] == 'CNY'){
            $rate = $this->getDecimalPlaceSetting($table['bitcoin']['cny']);
        }else if($this->utils->getCurrentCurrency()['currency_code'] == 'THB'){
            $rate = $this->getDecimalPlaceSetting($table['bitcoin']['thb']);
        }
        ...
        if(!empty($cust_fix_rate) && isset($target_amount)){
            ...
            $this->utils->debug_log("Cryptorate_api_coingecko_btc has error");
            return false;
        }
    }

    public function getParam($name) {
        return $this->CI->config->item('cryptorate_api_coingecko_btc_'.$name);
    }
}
