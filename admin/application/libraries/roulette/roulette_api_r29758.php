 ...
 */
class Roulette_api_r29758 extends Abstract_roulette_api
{
	const DAILY_SPIN_TIMES = 2;
	const CONSUMPTION = 500;
	const API_PREFIX = 'r29758';
	...
	public function generatePlayerRoulette($start_date, $end_date, $player_id, $promo_cms_id)
	{
		...
		$this->utils->debug_log(__METHOD__ . "generatePlayerRoulette params [$player_id]", ['promoRuleDescription' => $promoRuleDescription, 'hasAnyDeposit' => $hasAnyDeposit, 'promo_cms_id' => $promo_cms_id]);
	...
	public function calculateSpinTimes($type, $daily_times, $player_used_times, $amount, $consumption, $player_used_times_by_pid = null, $description = null)
	{
		...
		$any_deposit = $this->getCondictionItem($description, 'any_deposit');
		$daily_times = $this->utils->safeGetArray($description, 'daily_spin_limits');
		$this->utils->debug_log(__METHOD__, ['per_earn' => $per_earn, 'threshold' => $threshold, 'any_deposit' => $any_deposit, 'daily_times' => $daily_times]);

		$times_by_amount = floor(($amount / $consumption)) * $per_earn;

		if ($amount >= $threshold) {
			$times_by_amount += $any_deposit;
		}

		$total_times = $times_by_amount > $daily_times ? $daily_times : $times_by_amount;

		$this->utils->debug_log(__METHOD__, ['29758_total_times' => $total_times,'times_by_amount' => $times_by_amount]);
