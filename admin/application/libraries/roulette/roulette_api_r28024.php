 ...
 *
 * 1. 不論存款綁過優惠或轉完再綁都可計入
 * 2. 單筆存款 5000 - 9999 給3次小輪, 每日有2筆transaction可得到,當日最多可轉6輪
 * 3. 每日優惠,跨日重計
 * 4. 1/20 - 1/22三日
 * 5. 取款條件 bonus X5
 * 6. 每日只能存在一次大獎
 ...
 */
class Roulette_api_r28024 extends Abstract_roulette_api {

	const DAILY_SPIN_TIMES = 6;
	const CONSUMPTION = 5000;
	const MAX_AMOUNT_LIMIT = 9999;
	const API_PREFIX_R28024 = 'r28024';
	...
	public function getPlatformCode(){
		return self::R28024_API;
	}

	public function getPrefix() {
		return self::API_PREFIX_R28024;
	}

	public function getRouletteType($roulette_name){
		$type = '';
		switch ($roulette_name) {
			case self::API_PREFIX_R28024:
	...
	public function generatePlayerRoulette($start_date, $end_date, $player_id, $roulette_name){
		...
		// start pre check Premium wheel
		$player_used_times_by_pid = $this->countRouletteRecord($player_id, $description['pid'], $date_today_start, $date_now);
		$p_description = $this->rouletteDescription($description['pname']);
		$p_daily_times = $p_description['daily_spin_limits'];
		$times_by_amount = 0;
		$p_total_times = 0;

		$this->utils->debug_log(__METHOD__." p_description player_id {$player_id}", [
			'pid' => $description['pid'],
		...
		if ($player_used_times_by_pid >= $p_daily_times) {
			$p_deposit_list = $this->listDepositTransactions($player_id, $start_date, $end_date, $p_description['per_amount'], $p_description['max_amount_limit']);

			$per_transaction = $p_description['per_transaction'];
			if (!empty($p_deposit_list)) {
				$count = count($p_deposit_list);
				...
				$p_total_times = $times_by_amount > $p_daily_times ? $p_daily_times : $times_by_amount;
			}
		}
		// end check

		$spin_times_res = $this->calculateSpinTimes($this->getSpinCondition(), $daily_times, $player_used_times, $deposit_list, $consumption, $p_total_times, $description);
	...
	public function calculateSpinTimes($type, $daily_times, $player_used_times, $deposit_list, $consumption, $p_total_times = null, $description = null){
		$this->utils->debug_log(__METHOD__, 'start', [ 'type' => $type, 'daily_times' => $daily_times, 'player_used_times' => $player_used_times , 'p_total_times' => $p_total_times, 'deposit_list' => $deposit_list]);
