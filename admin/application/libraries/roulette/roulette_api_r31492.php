 ...
 */
class Roulette_api_r31492 extends Abstract_roulette_api
{
	const DAILY_SPIN_TIMES = PHP_INT_MAX;
	const CONSUMPTION = 300;
	const API_PREFIX = 'r31492';
	...
	{
		$threshold = self::CONSUMPTION;
		$spin_conditions = $this->utils->safeGetArray($description, 'spin_conditions', false);
		if ($spin_conditions) {
			$threshold = $this->utils->safeGetArray($spin_conditions, 'threshold', $threshold);
		}
		return $threshold;
	}
    ...
    public function getCondictionEarn($description = null)
	{
	...
	public function generatePlayerRoulette($start_date, $end_date, $player_id, $promo_cms_id)
	{
        ...
        $dailyTimes = $this->getDailySpinTimes();
		$promoRuleDescription = $this->getPromoRuleDescription($promo_cms_id);
        $hasAnyDeposit = $this->CI->transactions->hasAnyDeposit($player_id, $start_date, $end_date);
        list($anyDepositEarn, $depositEarn, $referralEarn) = $this->getCondictionEarn($promoRuleDescription);
		$spin_times_res = [
			"total_times" => 0,
			"used_times" => 0,
			"remain_times" => 0,
		...
		if (!$promoRuleDescription || !$hasAnyDeposit) {
            ...
            $spin_times_res['total_times'] += $anyDepositEarn;
        }

        $this->utils->debug_log(__METHOD__, "generatePlayerRoulette-$rname-hasAnyDeposit [$player_id]", [
			'hasAnyDeposit' => $hasAnyDeposit,
		...
		$threshold = $this->getConsumption($promoRuleDescription);

        if($deposit >= $threshold){
            $spin_times_res['total_times'] += $depositEarn;
        }

        $this->utils->debug_log(__METHOD__, "generatePlayerRoulette-$rname-total_deposit [$player_id]", [
        ...
        $referralBonusList = $this->CI->transactions->getReferralBonusList($player_id, $start_date, $end_date);
        $countRefList = count($referralBonusList);

        if($countRefList > 0){
            $spin_times_res['total_times'] += ($countRefList * $referralEarn);
        }

        $this->utils->debug_log(__METHOD__, "generatePlayerRoulette-$rname-referral [$player_id]", [
        ...
        $spin_times_res['total_times'] = $spin_times_res['total_times'] > $dailyTimes ? $dailyTimes : $spin_times_res['total_times'];
        $spin_times_res['used_times'] = $countPlayerPromoRecord;
        $remain = $spin_times_res['total_times'] - $spin_times_res['used_times'];
        $spin_times_res['remain_times'] = $remain > 0 ? $remain : $spin_times_res['remain_times'];
        $spin_times_res['threshold_amount'] = $threshold;
