...
class Sms_securimage
{
     ...
     * character set
     *
     * @see Sms_securimage::$charset charset property
     * @var int
     */
     ...
     *
     * @see Sms_securimage::SI_IMAGE_PNG SI_IMAGE_PNG
     * @see Sms_securimage::SI_IMAGE_JPEG SI_IMAGE_JPEG
     * @see Sms_securimage::SI_IMAGE_GIF SI_IMAGE_GIF
     * @var int
     */
     ...
     *
     * @var bool
     * @see Sms_securimage::$database_driver database_driver property
     * @deprecated 3.2RC4
     */
     ...
     *
     * @see Sms_securimage::$database_driver
     * @var bool
     */
     ...
     *
     * @see Sms_securimage::$database_file for SQLite
     * @var string
     */
     ...
     *
     * @see Sms_securimage::SI_CAPTCHA_STRING SI_CAPTCHA_STRING
     * @see Sms_securimage::SI_CAPTCHA_MATHEMATIC SI_CAPTCHA_MATHEMATIC
     * @see Sms_securimage::SI_CAPTCHA_WORDS SI_CAPTCHA_WORDS
     * @see Sms_securimage::$charset charset property
     * @see Sms_securimage::$wordlist_file wordlist_file property
     * @var int
     */
     ...
     *
     * @deprecated 3.2RC4
     * @see Sms_securimage::$database_file database_file property
     * @var string
     */
     ...
     *
     * @see Sms_securimage::$sox_binary_path sox_binary_path property
     ...
     * Static so that Sms_securimage::getCaptchaHtml() has access to this value.
     *
     * @since 3.6
     * @var string
     */
     ...
     *
     * @since 3.0.3
     * @see Sms_securimage::$audio_noise_path audio_noise_path property
     ...
     * used when calling Sms_securimage::show().
     *
    ...
    /**
     * Captcha code supplied by user [set from Sms_securimage::check()]
     *
     * @var string
     */
     ...
     *         'num_lines' => 5,
     *         'noise_level' => 3,
     *         'font_file' => Sms_securimage::getPath() . '/custom.ttf'
     *     );
     *
    ...
    public static function getCaptchaId($new = true, array $options = array())
    {
        if (is_null($new) || (bool)$new == true) {
            ...
            $si = new self($opts);
            Sms_securimage::$_captchaId = $id;
            $si->createCode();

            return $id;
        } else {
            return Sms_securimage::$_captchaId;
        }
    }
    ...
    public static function getCaptchaHtml($options = array())
    {
        ...
        if ($show_audio_btn) {
            ...
            if (is_executable(Sms_securimage::$lame_binary_path)) {
    ...
    public function getCode($array = false, $returnExisting = false)
    {
        ...
        if ($this->no_session != true) {
            // $this->CI->load->library('session');
            //read session
            $sms_securimage_code_value=$this->CI->session->userdata('sms_securimage_code_value');
            $sms_securimage_code_ctime=$this->CI->session->userdata('sms_securimage_code_ctime');
            $sms_securimage_code_disp=$this->CI->session->userdata('sms_securimage_code_disp');

            if (isset($sms_securimage_code_value[$this->namespace]) &&
                    trim($sms_securimage_code_value[$this->namespace]) != '') {
                if ($this->isCodeExpired(
                        $sms_securimage_code_ctime[$this->namespace]) == false) {
                    $code['code'] = $sms_securimage_code_value[$this->namespace];
                    $code['time'] = $sms_securimage_code_ctime[$this->namespace];
                    $code['display'] = $sms_securimage_code_disp[$this->namespace];
                }
            }
        }
    ...
    */
    ...
    protected function getAudibleCode()
    {
        ...
        if (preg_match('/(\d+) (\+|-|x) (\d+)/i', $code['display'], $eq)) {
            ...
            $letters = str_split($code['display']);
        }
     ...
     *
     * @see Sms_securimage::$charset  Charset option
     ...
     *
     * @see Sms_securimage::$correct_code 'correct_code' property
     */
    protected function validate()
    {
        ...
        if ($code != '') {
            ...
            if ((string)$code === (string)$code_entered) {
                ...
                if ($this->no_session != true) {
                    // $this->CI->load->library('session');
                    $this->CI->session->unset_userdata('sms_securimage_code_disp');
                    $this->CI->session->unset_userdata('sms_securimage_code_value');
                    $this->CI->session->unset_userdata('sms_securimage_code_ctime');
                    // $_SESSION['sms_securimage_code_disp'] [$this->namespace] = '';
                    // $_SESSION['sms_securimage_code_value'][$this->namespace] = '';
                    // $_SESSION['sms_securimage_code_ctime'][$this->namespace] = '';
                }
    ...
    protected function saveData()
    {
        if ($this->no_session != true) {
            ...
            // if (!empty($sms_securimage_code_value) && is_scalar($sms_securimage_code_value)) {
            //     // fix for migration from v2 - v3
            //     // unset($_SESSION['sms_securimage_code_value']);
            //     // unset($_SESSION['sms_securimage_code_ctime']);
            //     $this->CI->session->unset_userdata('sms_securimage_code_value');
            //     $this->CI->session->unset_userdata('sms_securimage_code_ctime');
            // }

            $sms_securimage_code_disp[$this->namespace] = $this->code_display;
            $sms_securimage_code_value[$this->namespace] = $this->code;
            $sms_securimage_code_ctime[$this->namespace] = time();

            $this->CI->session->set_userdata('sms_securimage_code_disp', $sms_securimage_code_disp);
            $this->CI->session->set_userdata('sms_securimage_code_value', $sms_securimage_code_value);
            $this->CI->session->set_userdata('sms_securimage_code_ctime', $sms_securimage_code_ctime);

            $this->CI->utils->debug_log('save captcha session', $this->CI->session->all_userdata());
        }
     ...
     *
     * @see Sms_securimage::$use_database Use database
     * @see Sms_securimage::$database_driver Database driver
     * @see Sms_securimage::$pdo_conn pdo_conn
        ...
        switch($this->database_driver) {
            ...
            case self::SI_DRIVER_PGSQL:
                if (empty($this->database_host)) {
                    throw new Exception('Sms_securimage::database_host is not set');
                } else if (empty($this->database_name)) {
                    throw new Exception('Sms_securimage::database_name is not set');
                }
    ...
    protected function getCodeFromDatabase()
    {
        ...
        if ($this->use_database == true && $this->pdo_conn) {
            if (Sms_securimage::$_captchaId !== null) {
                $query  = "SELECT * FROM {$this->database_table} WHERE id = ?";
                $stmt   = $this->pdo_conn->prepare($query);
                $result = $stmt->execute(array(Sms_securimage::$_captchaId));
            } else {
                $ip = $_SERVER['REMOTE_ADDR'];
                $ns = $this->namespace;
     ...
     *
     * @see Sms_securimage::$expiry_time expiry_time
