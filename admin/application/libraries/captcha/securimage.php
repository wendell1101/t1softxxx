...
class Securimage
{
     ...
     *     exit;
     */
    public function show($background_image = '', &$b64im = null)
    {
    ...
    public function getCode($array = false, $returnExisting = false)
    {
        ...
        if ($this->no_session != true) {
            ...
            $securimage_code_ctime=$this->CI->session->userdata('securimage_code_ctime');
            $securimage_code_disp=$this->CI->session->userdata('securimage_code_disp');

            if (isset($securimage_code_value[$this->namespace]) &&
                    trim($securimage_code_value[$this->namespace]) != '') {
                if ($this->isCodeExpired(
                        $securimage_code_ctime[$this->namespace]) == false) {
                    $code['code'] = $securimage_code_value[$this->namespace];
                    $code['time'] = $securimage_code_ctime[$this->namespace];
                    $code['display'] = $securimage_code_disp[$this->namespace];
                }
            }
        }
     ...
     */
    protected function doImage(&$b64im = null)
    {
    ...
    */
     ...
     *
     * @param null|array $b64imData If it is null, output directly to the browser.
     * If it is a empty array, the image information will assign the param, and will Not output to the browser.
     * the param's format as followings,
     * - $b64imData['code_display'] The attribute, "code_display" of self. It is the code of the captcha.
     * - $b64imData['code'] The attribute, "code_display" of self. It is the code of the captcha.
     * - $b64imData['token'] The part of the cache-key, the cache-key = self::cache_prefix+ token.
     * - $b64imData['contentType'] The Content Type of the captcha.
     * - $b64imData['b64im'] The image content of the captcha, after base64 encoded.
     */
    protected function output(&$b64imData = null)
    {
        if ($this->canSendHeaders() || $this->send_headers == false) {
            if ($this->send_headers) {
            ...
            $tmpFile = null; // output directly
            if( ! is_null($b64imData) ){
                $tmpFile = tempnam('/tmp','securimage');
                if( ! is_array($b64imData) ){
                    $b64imData = []; // reset to empty array
                }
            }
            ...
            switch ($this->image_type) {
                case self::SI_IMAGE_JPEG:
                    ...
                    if ($this->send_headers) header("Content-Type: ". $contentType);
                    imagejpeg($this->im, $tmpFile, 90);
                    break;
                case self::SI_IMAGE_GIF:
                    $contentType = 'image/gif';
                    if ($this->send_headers) header("Content-Type: ". $contentType);
                    imagegif($this->im, $tmpFile);
                    break;
                default:
                    $contentType = 'image/png';
                    if ($this->send_headers) header("Content-Type: ". $contentType);
                    imagepng($this->im, $tmpFile);
                    break;
            }
            ...
            try {
                if( ! empty($tmpFile) ){
                    ...
                    $cache_key = sprintf('%s-%s', self::cache_prefix, $token);
                    $jsonArr = [];
                    $jsonArr['code'] = $this->code;
                    $this->CI->utils->saveJsonToCache($cache_key, $jsonArr, $this->expiry_time);

                    $b64imData['code_display'] = $this->code_display;
                    $b64imData['code'] = $this->code;
                    $b64imData['token'] = $token;// from radis-cache with code
                    $b64imData['content_type'] = $contentType;
                    $b64imData['expiry_sec'] = $this->expiry_time;
                    $b64imData['b64im'] = base64_encode( file_get_contents($tmpFile) );
                    unlink($tmpFile);
                }
            } catch (Exception $e) {
                $this->CI->utils->error_log('convert to file/variable error:', $e);
			}
    ...
    protected function validate()
    {
        ...
        if ($code != '') {
            ...
            if ((string)$code === (string)$code_entered) {
                ...
                if ($this->no_session != true) {
                    // $this->CI->load->library('session');
                    $this->CI->session->unset_userdata('securimage_code_disp');
                    $this->CI->session->unset_userdata('securimage_code_value');
                    $this->CI->session->unset_userdata('securimage_code_ctime');
    ...
    protected function saveData()
    {
        if ($this->no_session != true) {
            ...
            // if (!empty($securimage_code_value) && is_scalar($securimage_code_value)) {
            ...
            $this->CI->session->set_userdata('securimage_code_disp', $securimage_code_disp);
            $this->CI->session->set_userdata('securimage_code_value', $securimage_code_value);
            $this->CI->session->set_userdata('securimage_code_ctime', $securimage_code_ctime);
