...
class Bidsopt_api extends Abstract_tracking_api
{

    public function init()
    {
        $this->_options = array_replace_recursive($this->_options, config_item('bidsopt'));
    }

    public function regPostBack($clickid, $player, $extra_info = null)
    {
        $username = $player->username;
        $post_back_data = array('click_id'=>$clickid, 'aff_sub1'=>$username);
        return $this->doPost($post_back_data, $player);
    }

    public function depositPostBack($clickid, $player, $deposit_order=null)
    {
        $this->utils->debug_log("============3rdparty_affiliate_network_first_deposit_postback:[Bidsopt_api]============");
