...
class Redgenn_seamless_transactions extends Base_game_logs_model {

	public $tableName = "redgenn_seamless_wallet_transactions";
	...
	public function flagBetTransactionSettled($tableName, $data){
		...
		->set('win_amount', $data['win'])
		->where("round_id", $data['round_id'])
		->where("trans_type", 'roundbet')
		;
    ...
    public function flagBetTransactionCancel($tableName,$data){
		...
		->where("transaction_id", $data['reference_id'])
		->where("trans_type", 'roundbet')
        ;
	...
	public function getTransaction($tableName, $where) {
		if(!empty($tableName)){
            $this->tableName = $tableName;
        }
		// $where = [
