...
class Fastspin_seamless_transactions extends Base_game_logs_model {

	public $tableName = "fastspin_seamless_wallet_transactions";
	...
	public function flagBetTransactionSettled($tableName, $data){
		...
		->where("transfer_id", $data['reference_id'])
		;
    ...
    public function flagBetTransactionCancel($tableName,$data){
		...
		$bet_transfer_id = isset($data['reference_id']) ? $data['reference_id'] : null;
		if(!$bet_transfer_id){
			return false;
		}
		$this->db
		->set('status', Game_logs::STATUS_CANCELLED)
		->where("transfer_id", $bet_transfer_id)
        ;
	...
	public function isTransactionExist($tableName, $data) {
        ...
        $qry = $this->db->get_where($this->tableName, array('transfer_id' => $data['transfer_id']));
    ...
    public function getExistingTransactionByReferenceId($tableName, $reference_id) {
		if(!empty($tableName)){
            $this->tableName = $tableName;
        }
		$where = [
