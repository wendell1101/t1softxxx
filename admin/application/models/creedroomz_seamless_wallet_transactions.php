...
class Creedroomz_seamless_wallet_transactions extends Base_game_logs_model {

	public $tableName = "creedroomz_seamless_wallet_transactions";
...
EOD;
	...
	public function updateTransactionStatus($uniqueId, $data){
		$status = isset($data['status']) ? $data['status'] : null;
...
EOD;
	...
	public function getPlayerCompleteDetailsByUsernameAndPassword($username, $password, $gamePlatformId) {
        $sql = <<<EOD
SELECT
...
WHERE gpa.login_name = ? AND gpa.password = ? AND gpa.game_provider_id = ?;

EOD;


        $params=[$username, $password, $gamePlatformId];
