...
class Endorphina_seamless_transactions extends Base_game_logs_model {

	public $tableName = "endorphina_seamless_wallet_transactions";
	...
	public function flagBetTransactionSettled($tableName, $data){
		...
		$bet_transaction_id = isset($data['bet_transaction_id']) ? $data['bet_transaction_id'] : null;
		if(!$bet_transaction_id){
			return false;
		}
		$this->db
		->set('status', Game_logs::STATUS_SETTLED)
		->set('win_amount', $data['win_amount'])
		->where("transaction_id", $bet_transaction_id)
    ...
    public function flagBetTransactionRefund($tableName,$data){
		if(!empty($tableName)){
            $this->tableName = $tableName;
        }
		...
		$this->db
		->set('status', Game_logs::STATUS_REFUND)
    ...
    public function getExistingTransactionByTransactionId($tableName, $transaction_id) {
		if(!empty($tableName)){
            $this->tableName = $tableName;
        }
