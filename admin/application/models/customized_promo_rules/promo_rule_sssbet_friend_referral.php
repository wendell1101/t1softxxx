...
condition:
{
    "class": "promo_rule_sssbet_friend_referral",
	"allowed_date": {
        "start": "2023-02-16",
        "end": "2023-02-25"
    },
    "bonus_settings": {
        "fix_amount" : {"min_bet": 1000, "bonus_amount": 10},
        "percentage" : {"min_bet": 1000, "bonus_amount": 0.005},
    }
}
 ...
 *
 */
class Promo_rule_sssbet_friend_referral extends Abstract_promo_rule{
	...
	public function getClassName(){
		return 'Promo_rule_sssbet_friend_referral';
	}
	...
	public function get_referred_list_by_player_id(){
		...
		$rows = $this->player_friend_referral->getPlayerReferralList($this->playerId);
	...
	private function checkCustomizeBounsCondition($description, &$extra_info, &$errorMessageLang){
        ...
        $referred_player_id_list = $this->get_referred_list_by_player_id();
        ...
        if( is_array($referred_player_id_list) && !empty($referred_player_id_list)){
        	foreach ($referred_player_id_list as $referred_player_id){
        		$referred_player_id_of_total_bet = $this->getPlayersBetByDate($referred_player_id, $fromDate, $toDate, null, $game_type);
