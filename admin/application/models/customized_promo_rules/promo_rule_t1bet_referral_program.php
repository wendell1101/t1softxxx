...
condition:
{
    "class": "promo_rule_t1bet_referral_program",
	"allowed_date": {
        "start": "2023-06-26",
        "end": "2023-06-30"
    },
    ...
    "bonus_settings": {
        "percentage" : {"min_bet": 1000, "bonus_amount": 0.001, "deposit_bouns": 0.01}
    }
}
 ...
 *
 */
class Promo_rule_t1bet_referral_program extends Abstract_promo_rule{
	...
	public function getClassName(){
		return 'Promo_rule_t1bet_referral_program';
	}
    ...
    public function getPlayersDepositByDate($invitedPlayerId, $fromDate, $toDate){
	    $this->load->model(['sale_order']);
	    return $this->sale_order->sumDepositRequestsByDate($invitedPlayerId, $fromDate, $toDate);
	}
	...
	private function checkCustomizeBounsCondition($description, &$extra_info, &$errorMessageLang){
		...
		$summary_total_deposit_on_referred_player = 0;
		...
		if($this->process_mock('today', $today)){
			...
			$betting_from = $today." 00:00:00";
			$betting_to = $today." 23:59:59";
		}
        ...
        if( is_array($referred_player_id_list) && !empty($referred_player_id_list)){
        	foreach ($referred_player_id_list as $referred_player_id){
				...
				$referred_player_id_of_total_bet = $this->getPlayersDepositByDate($referred_player_id, $betting_from, $betting_to);
				$summary_total_deposit_on_referred_player = $summary_total_deposit_on_referred_player + $referred_player_id_of_total_bet;
				$this->appendToDebugLog('==================deposit_bouns'.$summary_total_deposit_on_referred_player);
        	}
	            	...
	            	if(isset($list['min_bet']) && isset($list['bonus_amount'])){
		            	switch ($bonus_types) {
		            		...
		            		case 'percentage':
		            			if($summary_total_bet_on_referred_player > $list['min_bet']){
				                    $success = true;
				                    $bonus_amount = $summary_total_bet_on_referred_player * $list['bonus_amount'] + $summary_total_deposit_on_referred_player * $list['deposit_bouns'];
								}else{
				                	$errorMessageLang = 'promo.total_bet_not_met';
				                }
		            		break;
