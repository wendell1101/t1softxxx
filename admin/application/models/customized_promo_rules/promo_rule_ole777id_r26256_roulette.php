 ...
 * per_amount: 符合存款金額 >= 3000
 * roulette_exclusion_promo_list: 排除有綁定list內優惠的存款
 *
condition:
{
    "class": "promo_rule_ole777id_r26256_roulette",
    "allowed_date": {
        "start": "",
        "end": ""
    },
...
*/
class Promo_rule_ole777id_r26256_roulette extends Abstract_promo_rule
{
    ...
    {
        return 'Promo_rule_ole777id_r26256_roulette';
    }
    ...
    private function checkCustomizeBonusCondition($fromDate, $toDate, &$extra_info, $description, &$errorMessageLang, $dry_run = false)
    {
        ...
        try{
            ...
            if ($spin_conditions) {
                ...
                $per_amount = isset($spin_conditions['per_amount'])? $spin_conditions['per_amount'] : 3000;
                $per_earn = isset($spin_conditions['earn'])? $spin_conditions['earn'] : 1;

                switch ($verification_amount_type) {

                    case 'deposit':
                        $deposit_list = $this->callHelper('listDepositTransactions', [$this->playerId, $fromDate, $toDate, $per_amount]);
                        break;
