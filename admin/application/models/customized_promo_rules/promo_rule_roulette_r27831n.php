...
condition:
{
    "class": "promo_rule_roulette_r27831n",
    "allowed_date": {
        "start": "",
        "end": ""
    },
    ...
    "rouletteName": "r27831n",
    "cmsId":4
}
*/
class Promo_rule_roulette_r27831n extends Abstract_promo_rule
{
    ...
    {
        return 'Promo_rule_roulette_r27831n';
    }

    public function getRetentionTime($description)
	{
        $found_promo_setting = false;
        $retention_time = $this->utils->safeGetArray($description, 'retention_time', 0);
        if($retention_time != 0) {
		...
		if(!$found_promo_setting) {
            ...
            if($retention_time_config) {
                ...
                $retention_time = $this->utils->safeGetArray($retention_time_config, $config_key, 0) ?: $this->utils->safeGetArray($retention_time_config, 'default', 0);
    ...
    private function checkCustomizeBonusCondition($fromDate, $toDate, &$extra_info, $description, &$errorMessageLang, $dry_run = false)
    {
        ...
        try{
            ...
            if ($spin_conditions) {
                ...
                $per_amount = isset($spin_conditions['per_amount'])? $spin_conditions['per_amount'] : 5000;
