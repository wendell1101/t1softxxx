...
condition:
{
    "class": "promo_rule_sambabet_upgrade_level_bonus",
    "bonus_settings": {
 ...
 *
 */
class Promo_rule_sambabet_upgrade_level_bonus extends Abstract_promo_rule{
	...
	public function getClassName(){
		return 'Promo_rule_sambabet_upgrade_level_bonus';
	}
	...
	protected function runBonusConditionChecker($description, &$extra_info, $dry_run){
        ...
        if($isUpgrade){
            $upgarde_time = $gradeRecord['pgrm_end_time'];
            $release_date['start'] = $upgarde_time;
            $release_date['end'] = $endAt;
            $count_approved_promo = $this->callHelper('count_approved_promo', [$this->promorulesId, self::DATE_TYPE_CUSTOMIZE, $release_date]);
            $this->appendToDebugLog('count approved promo from upgrade time to now', ['from' => $upgarde_time, 'to' => $endAt]);
