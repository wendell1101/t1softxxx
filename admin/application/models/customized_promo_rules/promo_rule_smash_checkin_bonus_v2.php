...
condition:
{
    "class": "promo_rule_smash_checkin_bonus_v2",
    "allowed_date": {
        "start": "",
        "end": ""
    },
...
*
*/
class Promo_rule_smash_checkin_bonus_v2 extends Abstract_promo_rule{
	...
	public function getClassName(){
		return 'Promo_rule_smash_checkin_bonus_v2';
	}
	...
	protected function runBonusConditionChecker($description, &$extra_info, $dry_run){
		...
		$this->appendToDebugLog('runBonusConditionChecker',['description' => $description, 'application_day' => $application_day, 'today' => $today]);
	...
	private function checkCustomizeBonusCondition($bonus_settings, $fromDate, $toDate, &$extra_info, $description, &$errorMessageLang){
	    ...
	    $amountLimit = isset($description['amountLimit']) ? $description['amountLimit'] : 500;
	    $nowDate = $this->get_date_type(self::TO_TYPE_NOW);

	    $getPlayerBetByDate = $this->callHelper('getPlayerBetByDate',[$fromDate, $toDate]);
	    $this->appendToDebugLog('checkCustomizeBonusCondition',['getPlayerBetByDate' => $getPlayerBetByDate, 'fromDate' => $fromDate, 'toDate' => $toDate]);

	    if($getPlayerBetByDate < $amountLimit){
	    	$success=false;
			$errorMessageLang =  lang('notify.81');
	    	return $result = ['success' => $success, 'message' => $errorMessageLang, 'continue_process_after_script' => FALSE];
	    }

	    $fromDateHourStr = $this->utils->formatDateForMysql(new DateTime($fromDate));
		$toDateHourStr = $this->utils->formatDateForMysql(new DateTime($toDate));
	    // $depositListByDates = $this->callHelper('getPlayerDepositListByDates',[$this->playerId, $fromDate, $toDate, $amountLimit]);
	    $betListByDates = $this->callHelper('getPlayerBetListByDates', [$this->playerId, $fromDateHourStr, $toDateHourStr, $amountLimit]);
	    ...
	    $betListByDates = isset($description['betListByDates']) ? $description['betListByDates'] : $betListByDates;

	    $this->appendToDebugLog('checkCustomizeBonusCondition getPlayerBetListByDates', ['playerId' => $this->playerId, 'betListByDates' => $betListByDates, 'playerBonusAmount' => $playerBonusAmount,'bonus_settings' => $bonus_settings]);
	    ...
	    if (!empty($bonus_settings)) {
	    	...
	    	foreach ($betListByDates as $deposit) {
