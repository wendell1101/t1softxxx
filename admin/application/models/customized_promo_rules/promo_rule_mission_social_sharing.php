...
condition:
{
    "class": "promo_rule_mission_social_sharing",
	"allowed_date": {
        "start": "2023-08-22",
        "end": "2033-08-22"
    },
 ...
 */
class Promo_rule_mission_social_sharing extends Abstract_promo_rule_mission{

    const SUBTYPE = 'sharing';
	...
	public function getClassName(){
		return 'promo_rule_mission_social_sharing';
	}
    ...
    protected function checkMission($description, &$extra_info) {
        ...
        // $release_date['start'] = $this->playerRegisterDate;
        // $release_date['end'] = $this->currentDate;
        ...
        $has_record = $this->player_trackingevent->getNotifyBySource($this->playerId, player_trackingevent::TRACKINGEVENT_SOURCE_TYPE_MISSION_JOIN_TELEGRAM);
        $is_interact = $this->utils->safeGetArray($extra_info, 'is_interact', false);//$extra_info['is_interact'];
        if(!$has_record && $is_interact) {
            $result = $this->player_trackingevent->createSettledNotify($this->playerId, player_trackingevent::TRACKINGEVENT_SOURCE_TYPE_MISSION_JOIN_TELEGRAM, array());
            $this->appendToDebugLog($this->getClassName() .' checkMission_'.$this->playerId , ["is_interact"=>$is_interact, "has_record" => $has_record, "createSettledNotify"=>$result]);

            $has_record = $result ? true : false;
        }
        $this->appendToDebugLog($this->getClassName() .' checkMission_'.$this->playerId , ["is_interact"=>$is_interact, "has_record" => $has_record]);
        $hit_mission =  !empty($has_record) ? true : false;
	...
	protected function _checkCustomizeBounsCondition($description, &$extra_info, &$errorMessageLang){
        ...
        $subtype = isset($description['subtype'])? $description['subtype'] : 'share';
