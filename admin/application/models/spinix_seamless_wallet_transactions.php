...
class spinix_seamless_wallet_transactions extends Base_game_logs_model {

	public $tableName = "spinix_seamless_wallet_transactions";
	...
	public function getRelatedBetExistingTransaction($round_id, $player_id) {
        $qry = $this->db->get_where($this->tableName, array('round_id' => $round_id, 'player_id' => $player_id, 'type' => 'bet'));
        $transaction = $this->getOneRow($qry);
		return $transaction;
	}	
...
EOD;
	...
	public function updateBetTransactionByRoundIdAndPlayerId($round_id,$player_id, $data){
...
UPDATE {$this->tableName}
		SET status = ?
WHERE
round_id = ? and player_id = ? and type=?
EOD;
