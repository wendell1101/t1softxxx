...
class qt_seamless_service_api extends BaseController {
    ...
    const WHITELIST_METHOD_FOR_EXPIRED_TOKEN = [
        'qt_seamless_service_api::balance',
		'qt_seamless_service_api::credit',
		'qt_seamless_service_api::rollback',
    ];

	const WHITELIST_METHOD_POSSIBLE_NO_WALLET_SESSION = [
		'qt_seamless_service_api::rewards',
		'qt_seamless_service_api::balance',
	];
	...
	public function __construct() {
		...
		$this->load->model(array('wallet_model','game_provider_auth','common_token','player_model','game_description_model','external_system',
			'common_seamless_wallet_transactions','original_game_logs_model','pgsoft_seamless_game_logs', 'original_seamless_wallet_transactions'));
		$this->requestParams = new stdClass();
        $this->processRequest();

        $this->gamePlatformId = QT_SEAMLESS_API;
        $this->game_api = $this->utils->loadExternalSystemLibObject(QT_SEAMLESS_API);
	}
	...
	public function transactions($type = null) {
		$this->gamePlatformId = $this->generateGamePlatformId();
		if(empty($this->gamePlatformId)){
			$addOrigin = true;
	        $origin = "*";
	        $pretty = false;
	        $partialOutputOnError = false;
    		$errorResponse['message'] = "(Error) Invalid game id prefix!!";
            return $this->returnJsonResult($errorResponse, $addOrigin, $origin, $pretty, $partialOutputOnError, self::ERROR_UNKNOWN['status']);
		} else{
	...
	private function debit() {
		...
		if(!empty($playerId)) {
			...
			$trans_success = $this->lockAndTransForPlayerBalance($playerId, function() use($self,$playerId,$player_info,&$response_code, $params) {
				...
				$this->checkRemoteWalletError($params);
	...
	private function checkRemoteWalletError($params){
		if($this->remote_Wallet_error_code){
			$failed_external_uniqueid = strtolower($params->txnType).'-'.$params->txnId;
			$failed_transaction_data = $md5_data = [
				'round_id' => $params->roundId,
				'transaction_id' =>$params->txnId,
				'external_game_id' =>$params->gameId,
				'player_id' => $this->game_api->getPlayerIdByGameUsername($params->playerId),
				'game_username' => $params->playerId,
				'amount' => $params->amount,
				'balance_adjustment_type' => strtolower($params->txnType),
				'action' => $params->txnType,
				'game_platform_id' => $this->game_api->getPlatformCode(),
				'transaction_raw_data' => json_encode($params),
				'remote_raw_data' => null,
				'remote_wallet_status' => $this->remote_Wallet_error_code,
				'transaction_date' => $this->formatDateTime($params->created),
	...
	private function saveFailedTransaction($query_type='insert', $data=[], $where=[]){
        ...
        $this->utils->debug_log("QT SEAMLESS SERVICE API: saveFailedTransaction",$query_type, $table_name, $data, $where);
    ...
    private function isFailedTransactionExist($where=[]){
        ...
        $this->utils->debug_log("QT SEAMLESS SERVICE API: isFailedTransactionExist",$table_name, $where, $isExisting);
        return $isExisting;
    }
    ...
    private function remoteWalletErrorCode(){
        ...
        if (method_exists($this->wallet_model, 'getRemoteWalletErrorCode')) {
            ...
            $this->utils->debug_log("QT SEAMLESS SERVICE API: remoteWalletErrorCode", $errorCode);
            return $errorCode;
        }
        return null;
    }
	...
	private function credit() {
		...
		if(!empty($playerId)) {
			...
			$trans_success = $this->lockAndTransForPlayerBalance($playerId, function() use($self,$playerId,$player_info,&$response_code, $params) {
				...
				list($lastId, $type) = $this->original_seamless_wallet_transactions->getLastStatusOfCommonData('common_seamless_wallet_transactions', ["round_id" => $params->roundId, 'game_id' => $params->gameId], 'transaction_type');
				if(empty($lastId)){ #no round id
					$response_code['statusCode'] 	= self::ERROR_REQUEST_DECLINED["code"];
					$response_code['status'] 		= self::ERROR_REQUEST_DECLINED["status"];
					$response_code['message'] 		= "Round id does not exist.";
					return false;
				}

                if(!empty($type) && $type == self::ROLLBACK){
                	$rows = $this->original_seamless_wallet_transactions->queryPlayerTransactions('common_seamless_wallet_transactions', $this->player_id, $params->gameId, $params->roundId);
                	$debit_cnt = 0;
                	$rollback_cnt = 0;
                	if(!empty($rows)){
                		foreach ($rows as $key => $row) {
                			...
                			if($row['transaction_type'] == self::ROLLBACK){
                				$rollback_cnt++;
                			}
                		}

                		if($rollback_cnt >= $debit_cnt){
                			$response_code['statusCode'] 	= self::ERROR_REQUEST_DECLINED["code"];
							$response_code['status'] 		= self::ERROR_REQUEST_DECLINED["status"];
							$response_code['message'] 		= "No more withdrawal for deposit.";
							return false;
                		}
                	}
                }
	...
	private function rollback() {
		...
		if(!empty($playerId)) {
			...
			$trans_success = $this->lockAndTransForPlayerBalance($playerId, function() use($self,$playerId,$player_info,&$response_code, $params) {
				...
				list($lastId, $type) = $this->original_seamless_wallet_transactions->getLastStatusOfCommonData('common_seamless_wallet_transactions', ["round_id" => $params->roundId, 'game_id' => $params->gameId], 'transaction_type');
				if(empty($lastId)){ #no round id
					$response_code['balance'] 		= $player_balance;
					return true;
				}

                if(!empty($type) && $type == self::CREDIT){
                    $response_code['statusCode'] 	= self::ERROR_REQUEST_DECLINED["code"];
					$response_code['status'] 		= self::ERROR_REQUEST_DECLINED["status"];
					$response_code['message'] 		= "Round already settled.";
					return false;
                }
                ...
                if(!empty($type) && $type == self::ROLLBACK){
                	...
                	if(!empty($rows)){
                		...
                		if($rollback_cnt >= $debit_cnt){
                			$response_code['statusCode'] 	= self::ERROR_REQUEST_DECLINED["code"];
							$response_code['status'] 		= self::ERROR_REQUEST_DECLINED["status"];
							$response_code['message'] 		= "No more withdrawal for rollback.";
							return false;
                		}
                	}
                }
	...
	protected function balance(){
		...
		/**
	 * /qt_seamless_service_api/accounts/<username>/get_balance
	 */
	protected function get_balance(){
		$game_code = isset($_GET["gameId"]) ? $_GET["gameId"] : null;
		$gamePlatformId = $this->getGamePlatformIdBasedOnGameId($game_code);
	...
	private function getGamePlatformIdBasedOnGameId($gameId){
		if(!empty($gameId)){
			$array = explode('-', $gameId);
            $prefix = isset($array[0]) ? $array[0] : "UNKNOWN";
	...
	private function adjustWallet($action, $player_info, $trans_record) {
		...
		$remoteWalletExternalGameID = isset($trans_record->gameId) ? $trans_record->gameId : null;

		$isEnd = isset($trans_record->completed) ? ($trans_record->completed === true || strtolower($trans_record->completed) === 'true') : false;


		$roundId = isset($trans_record->roundId) ? $trans_record->roundId : null;

		$this->CI->utils->debug_log("remoteWallet params", [
			'is_end' => $isEnd,
			'roundId' => $roundId,
			'remoteWalletExternalGameID' => $remoteWalletExternalGameID,
			'action' => $action,
        ...
        if($action == self::DEBIT) {
            ...
            if($this->utils->compareResultFloat($betAmount, '>', 0)) {
                ...
                if($will_deduct) {
					...
					if(method_exists($this->wallet_model, 'setUniqueidOfSeamlessService')){
						$this->wallet_model->setUniqueidOfSeamlessService($uniqueIdOfSeamlessService, $remoteWalletExternalGameID);
					}
					...
					if(method_exists($this->wallet_model, 'setExternalGameId')){
						$this->wallet_model->setExternalGameId($remoteWalletExternalGameID);
					}

                    $betAmount = $this->game_api->gameAmountToDB($betAmount);

                    $this->CI->utils->debug_log('QT HACKSAW BET AMOUNT', $betAmount);
                    $deduct_balance = $this->wallet_model->decSubWallet($player_info['playerId'], $this->game_api->getPlatformCode(), $betAmount, $after_balance);
			...
			if(is_null($after_balance)) $after_balance = $this->game_api->queryPlayerBalance($player_info['username'])['balance'];
			...
			if(method_exists($this->wallet_model, 'setRelatedUniqueidOfSeamlessService')){
                $related_unique_id = isset($trans_record->betId) ? $this->utils->mergeArrayValues([
                    'game',
			...
			if ($this->utils->compareResultFloat($amount, '=', 0)) {
				$this->utils->debug_log("QT SEAMLESS SERVICE API: (debitCreditAmountToWallet) amount 0 call remote wallet", 'param', $trans_record);
				...
				if(method_exists($this->utils, 'isEnabledRemoteWalletClient')){
					if ($this->utils->isEnabledRemoteWalletClient()) {
						if(method_exists($this->wallet_model, 'incRemoteWallet')){
							$this->wallet_model->incRemoteWallet($player_info['playerId'], $amount, $this->game_api->getPlatformCode(), $after_balance);
						}
					}
				}
				...
				$add_balance = $this->wallet_model->incSubWallet($player_info['playerId'], $this->game_api->getPlatformCode(), $amount, $after_balance);
				$this->remote_Wallet_error_code = $this->remoteWalletErrorCode();
				...
				if (!$add_balance) { #check if success, possible balance is locked and response timeout on remote wallet
					$response_code['statusCode'] = self::ERROR_REQUEST_DECLINED['code'];
					$response_code['message'] = self::ERROR_REQUEST_DECLINED['message'];
					$response_code['status'] = self::ERROR_REQUEST_DECLINED['status'];
					return $response_code;
			...
			if($this->utils->compareResultFloat($amount, '=', 0)){
				...
				$succ = $this->wallet_model->incRemoteWallet($player_info['playerId'], $amount, $this->game_api->getPlatformCode(), $after_balance);
			}elseif($this->utils->compareResultFloat($amount, '>', 0)) {
				$amount = $this->game_api->gameAmountToDB($amount);
				$uniqueid =  $action.'-'.$trans_record->txnId;
				$uniqueIdOfSeamlessService=$this->game_api->getPlatformCode().'-'.$uniqueid;            
