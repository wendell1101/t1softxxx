...
//http://admin.idr.staging.seamlessgateway.t1t.in/sv388_remote_service_api
//https://allwecan.info/single-wallet-testing game provider automated testing
require_once dirname(__FILE__) . '/BaseController.php';

class Sv388_remote_service_api extends BaseController {

	protected $gamePlatformId = SV388_SEAMLESS_GAME_API;
	protected $defaultGameCurrency = "PTI";
	...
	const ACTION_VOID_BET = 'voidBet';
	const ACTION_CANCEL_BET = 'cancelBet';
	...
	*/
	...
	public function __construct() {
		...
		parent::__construct();
		$this->load->model(array('wallet_model','game_provider_auth','common_token','player_model', 'game_description_model', 'sv388_transactions'));
			...
			if($action == "tip"){
				...
				if($this->utils->compareResultFloat($totalTipAmount, '>', $beforeBalance)) {
					...
					foreach ($txns as $key => $txn) {
						$tipRow = $this->sv388_transactions->isRowByTransactionId($txn['platformTxId']);
						if(!empty($tipRow)){
				...
				foreach ($txns as $key => $txn) {
					...
					if(!empty($tipRow)){
						if($tipRow['action'] == "tip" ){
							...
							$cancelExist = $this->sv388_transactions->isExternalUniqueExist($cancelId);
							if($cancelExist){
								$success = true; #always
								continue;
							} else {
	...
	*/
	public function playerBet($playerId, $playerUsername, $gameUsername, $fullRequest, $paramsValue) {
        ...
        $success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramsValue) {
            ...
            $savedBetTransactions = [];

            $transIdConcat = '';             
            
            //get initial balance
            $initialBalance = $controller->game_api->queryPlayerBalanceByPlayerId($playerId)['balance'];

            foreach ($paramsValue as &$trans_record) {
                $temp = $trans_record;
                $userId = $trans_record['userId'];

		        if (isset($game_usernames[$userId])) {
		            $player_details = $game_usernames[$userId];

		            if(isset($player_details['player_id'])) {
                        $playerUsername = $player_details['username'];
                        $playerId = $player_details['player_id'];
		            }
		        }else {
                    $player_details = (array) $controller->common_token->getPlayerCompleteDetailsByGameUsername($userId, $this->game_api->getPlatformCode());
		            $playerUsername = $player_details['username'];
		            $playerId = $player_details['player_id'];
		            $game_usernames[$userId] = $player_details;
		            //$game_usernames[$playerId] = $playerUsername;
		        }

                $txn = $trans_record;
                $temp['_playerUsername'] = $playerUsername;
                //$temp['_txn'] = $txn;
                $temp['_playerId'] = $playerId;
                $temp['_duplicateBet'] = false;
                $temp['_betTrans'] = [];
                //$temp['_txn'] = $txn;
                $temp['_settleTrans'] = null;
                $temp['_cancelTrans'] = null;
                $temp['_isCancelledBet'] = false;
                $temp['_isVoidBet'] = false;
                $temp['_toAdjustBalance'] = true;
                $temp['_transactionSaved'] = true;
                $temp['_withPendingCancel'] = false;
                $temp['_betTransactionSettled'] = false;                  
                $temp['_relTrans'] = [];


                $totalBetAmount += $trans_record['betAmount'];
    			$transaction_id = isset($trans_record['platformTxId']) ? $trans_record['platformTxId'] : null;
                $transIdConcat .= $transaction_id;

                
                $relatedTrans = $controller->sv388_transactions->getAllRelatedTransactions($transaction_id, $playerId);
                if(!empty($relatedTrans)){
                
                    //cehck bet to settle exist
                    foreach($relatedTrans as $rel){
                        if($rel['action']==self::ACTION_BET){
                            $temp['_duplicateBet'] = true;
                            $temp['_toAdjustBalance'] = false;
                            $totalBetAmount -= $trans_record['betAmount'];
                            // if($rel['before_balance']>0){
                            //     $temp['_toAdjustBalance'] = false;
                            // }
                            /*if($rel['action']['action_status']==Game_logs::STATUS_PENDING){
                                $temp['_toAdjustBalance'] = true;
                            }*/
                        }
                    }
                    ...
                    //cehck bet to settle exist
                    foreach($relatedTrans as $rel){
                        if($rel['action']==self::ACTION_CANCEL_BET){
                            $temp['_cancelTrans'] = $rel;                            
                            $temp['_withPendingCancel'] = true;
                            $totalBetAmount -= $trans_record['betAmount'];
                        }
                    }
                ...
                //save settle transaction data as pending settlement (will wait for balance adjustment)
                $txn['action_status'] = Game_logs::STATUS_PENDING;

                $controller->CI->utils->debug_log(__METHOD__. '<----------------------------- txn', $txn);

                $success = $controller->processTransactionRecord($txn, 'bet', 0, 0);
                if(!$success){
                    $temp['_transactionSaved'] = false;                                             
                    $temp['_toAdjustBalance'] = false;
                    $withError = true;
                    continue;
                }
    
                $savedBetTransactions[] = $temp;
            }//end foreach trans paramsValue

            //do not proceed if with error with saving trans
            if($withError){
            ...
            if($initialBalance<$totalBetAmount){
                $status_code = [
                    'code' => self::STATUS_CODE_NOT_ENOUGH_BALANCE
                ];
            ...
            //deduct total bet
            $remoteUniqueId = md5($transIdConcat);
            //$remoteUniqueId = $transIdConcat;
            $controller->wallet_model->setUniqueidOfSeamlessService($remoteUniqueId); 
            $isDeduct = $controller->wallet_model->decSubWallet($playerId, $controller->gamePlatformId,$totalBetAmount);
            ...
            $this->CI->utils->debug_log(__METHOD__. '<----------------------------- savedBetTransactions', $savedBetTransactions);

            //for each bet transactions update set as settled and add before and after balance as confirmation that bet was processed
            $tempAfterBalance = $tempBeforeBalance = $initialBalance;

            $withError = false;
            foreach($savedBetTransactions as $pendingBet){
                $playerId = $pendingBet['_playerId'];

                //cehck if settled bet exist
                $isbetExists = $controller->sv388_transactions->isBetExists($pendingBet['platformTxId'],Game_logs::STATUS_SETTLED);

                $transactionId = (isset($pendingBet['platformTxId'])&&!empty($pendingBet['platformTxId']))?$pendingBet['platformTxId']:null;
                

                if(isset($pendingBet['_cancelTrans'])&&!empty($pendingBet['_cancelTrans'])){
                   $betAmount = abs($pendingBet['betAmount']);
                    //$tempAfterBalance = $tempBeforeBalance - $betAmount;
                    $updateBetData = [
                        #'action_status'=>Game_logs::STATUS_SETTLED,
                        "before_balance"=>$tempBeforeBalance,
                        "after_balance"=>$tempAfterBalance-$betAmount
                    ];
                    $updateBetResp = $controller->sv388_transactions->updateTransactionData($transactionId, 'bet', $updateBetData);
                    if(!$updateBetResp){
                        $this->CI->utils->error_log('<----------------------------- Error update bet transactions:', $pendingBet, 'updateBetData', $updateBetData);
                    }

                    $updateCancelData = [
                        #'action_status'=>Game_logs::STATUS_SETTLED,
                        "before_balance"=>$tempBeforeBalance-$betAmount,
                        "after_balance"=>$tempAfterBalance
                    ];
                    $updateCancelBetResp = $controller->sv388_transactions->updateTransactionData($transactionId, 'cancelBet', $updateCancelData);
                    if(!$updateCancelBetResp){
                        $this->CI->utils->error_log('<----------------------------- Error update cancelBet transactions:', $pendingBet, 'updateCancelData', $updateCancelData);
                    }
                    
                }else{
                    $tempAfterBalance = $tempBeforeBalance - abs($pendingBet['betAmount']);
                    $updateBetData = [
                        #'action_status'=>Game_logs::STATUS_SETTLED,
                        "before_balance"=>$tempBeforeBalance,
                        "after_balance"=>$tempAfterBalance
                    ...
                    $updateBetResp = $controller->sv388_transactions->updateTransactionData($transactionId, 'bet', $updateBetData);
                    if(!$updateBetResp){
                        $this->CI->utils->error_log('<----------------------------- Error update bet transactions:', $pendingBet);
                    }
                }

                $tempBeforeBalance = $tempAfterBalance;
            }

            $status_code['after_balance'] = $controller->game_api->gameAmountToDB($tempAfterBalance);
	...
	*/
	public function settle($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
        ...
        $withError = false;
        $game_usernames = [];
        //process all settlement transaction save as pending
        $savedSettleTransactions = [];
        foreach ($paramsValue as $paramValue) {
		    if(isset($paramValue['userId'])) {
                ...
                if (isset($game_usernames[$userId])) {
                    ...
                    $player_details = (array) $this->common_token->getPlayerCompleteDetailsByGameUsername($userId, $this->game_api->getPlatformCode());
		            $playerUsername = $player_details['username'];
		            $playerId = $player_details['player_id'];
		            $game_usernames[$userId] = $player_details;
		            //$game_usernames[$playerId] = $playerUsername;
                ...
                $transactionId = (isset($paramValue['platformTxId'])&&!empty($paramValue['platformTxId']))?$paramValue['platformTxId']:null;
                ...
                &$savedSettleTransactions) {
                    ...
                    $temp['_txn'] = $txn;
                    $temp['_playerId'] = $playerId;
                    $temp['_betIsMissing'] = true;
                    $temp['_duplicateSettle'] = false;
                    $temp['_betTrans'] = [];
                    $temp['_settleTrans'] = null;
                    $temp['_isCancelledBet'] = false;
                    $temp['_isVoidBet'] = false;
                    $temp['_balanceAdjusted'] = false;
                    $temp['_transactionSaved'] = true;
                    $temp['_betTransactionSettled'] = true;                
                    $temp['_relTrans'] = [];
                    $temp['_toAdjustBalance'] = true; 

                    $winAmount = isset($paramValue['winAmount']) ? $paramValue['winAmount']: 0;

                    $relatedTrans = $this->sv388_transactions->getAllRelatedTransactions($transactionId, $playerId);
                    ...
                    if(!empty($relatedTrans)){
                        ...
                        foreach($relatedTrans as $rel){
                            if($rel['action']==self::ACTION_SETTLE){
                                $temp['_duplicateSettle'] = true;                            
                                $temp['_settleTrans'][] = $rel;         
    
                                if((int)$rel['action_status']==Game_logs::STATUS_SETTLED){              
                                    $temp['_balanceAdjusted'] = true; 
                                    $temp['_toAdjustBalance'] = false;                                     
                                    $savedSettleTransactions[] = $temp;
                                    return true;
                                }elseif((int)$rel['action_status']==Game_logs::STATUS_PENDING){                                          
                                    $temp['_toAdjustBalance'] = true; 
                                    $savedSettleTransactions[] = $temp;
                                }
                            }
                    ...
                    if($temp['_toAdjustBalance']){           
                        
                        if($this->utils->isEnabledRemoteWalletClient()){
                            $before_balance = $this->wallet_model->getRemoteWalletAfterBalance();
                        }else{
                            $before_balance = $this->game_api->queryPlayerBalanceByPlayerId($playerId)['balance'];
                        }

                        //save settle transaction data as pending settlement (will wait for balance adjustment)
                        $txn['action_status'] = Game_logs::STATUS_SETTLED;
                        //$txn['action_status'] = Game_logs::STATUS_SETTLED;
                        $expectedAfterBalance = $before_balance+$winAmount;
                        $settleSaved = $this->processTransactionRecord($txn, 'settle', $before_balance, $expectedAfterBalance);
                        ...
                        $updateBetData = ['action_status'=>Game_logs::STATUS_SETTLED];
                        $updateBetResp = $this->sv388_transactions->updateTransactionData($transactionId, 'bet', $updateBetData);
                        if(!$updateBetResp){                                        
                            $temp['_toAdjustBalance'] = false;
                            $temp['_betTransactionSettled'] = false;  
                            $temp['_balanceAdjusted'] = false;                             
                        ...
                        //call increment
                        if($winAmount<=0){
                            $this->CI->utils->debug_log(__METHOD__. " win amount is 0 no need to call remote: ", $temp);
                            $success = true;
                            
                            $temp['_balanceAdjusted'] = true;     
                        }else{
                            $success = $this->wallet_model->incSubWallet($playerId, $controller->gamePlatformId,$winAmount);
                            if(!$success){
                                $temp['_balanceAdjusted'] = false;     
                                
                                $this->CI->utils->debug_log(__METHOD__. " Balance adjustment error ", $temp);
                            }
                        }
        ...
        $this->CI->utils->debug_log(__METHOD__. " savedSettleTransactions ", $savedSettleTransactions);
	...
	public function unsettle($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				...
				$existingRow = $controller->sv388_transactions->isRowBySettleId("settle-".$transaction_id);
				...
				$unsettledTransactions = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'unsettle');
				...
				$betTransactions = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'bet');
				...
				$updateResp = $this->sv388_transactions->updateTransactionData($transaction_id, 'settle', $updateData);
				if(!$updateResp){
				...
				$updateResp = $this->sv388_transactions->updateTransactionData($transaction_id, 'bet', $updateData);
				if(!$updateResp){
					$status_code['code'] = '9999';
					return false;
				}
				...
				$existingUnsettle = $controller->sv388_transactions->isUnsettleIdExist($transaction_id);
				if(!$existingUnsettle) {
					if($betStatus == 'WIN') {
	...
	*/
	public function voidBet($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
        ...
        foreach ($paramsValue as $paramValue) {
		    if(isset($paramValue['userId'])) {
                ...
                if (isset($game_usernames[$userId])) {
		            ...
		            $playerId = $player_details['player_id'];
		            $game_usernames[$userId] = $player_details;
                    $game_usernames[$userId]['total_bet_amount'] = 0;
                    $game_usernames[$userId]['transaction_ids'] = [];
                    $game_usernames[$userId]['player_void_bets'] = [];                   
                
                    $game_usernames[$userId]['before_balance'] = 0;
                    $game_usernames[$userId]['after_alance'] = 0;
                ...
                $game_usernames[$userId]['transaction_ids'][] = $transactionId;

                $relatedTrans = $this->sv388_transactions->getAllRelatedTransactions($transactionId, $playerId);

                $betAmount = isset($paramValue['betAmount']) ? abs($paramValue['betAmount']): 0;

                
                $game_usernames[$userId]['player_void_bets'][] = $paramValue;
                ...
                $temp['_playerId'] = $playerId;
                $temp['_betIsMissing'] = true;
                $temp['_duplicateVoidBet'] = false;
                $temp['_betTrans'] = [];
                $temp['_settleTrans'] = [];
                $temp['_voidBetTrans'] = null;
                $temp['_isCancelledBet'] = false;
                $temp['_isBetVoided'] = false;
                $temp['_toAdjustBalance'] = true;
                $temp['_transactionSaved'] = true;
                $temp['_betTransactionSettled'] = true;                
                $temp['_relTrans'] = [];

                $temp['_beforeBalance'] = 0;
                $temp['_afterBalance'] = 0;
                ...
                if(!empty($relatedTrans)){
                    ...
                    //check if voidbet transactions already exist
                    foreach($relatedTrans as $rel){
                        if($rel['action']==self::ACTION_VOID_BET){
                            $temp['_duplicateVoidBet'] = true;                            
                            $temp['_voidBetTrans'][] = $rel;                            
                            $temp['_toAdjustBalance'] = false;                          
                            $temp['_isBetVoided'] = true;

                            if((int)$rel['action_status']==Game_logs::STATUS_SETTLED){ 
                                //$savedVoidTransactions[] = $temp;  
                ...
                if($temp['_toAdjustBalance']){
                    ...
                    //save voidBet transaction
                    
                    $expectedAfterBalance = $before_balance+$betAmount;

                    if(!$temp['_duplicateVoidBet']){

                        /**/$txn['action_status'] = Game_logs::STATUS_PENDING;
                        $voidBetSaved = $this->processTransactionRecord($txn, 'voidBet', $before_balance, $expectedAfterBalance);
                        if(!$voidBetSaved){
                            $temp['_transactionSaved'] = false;                                             
                            $temp['_toAdjustBalance'] = false;
                            $temp['_balanceAdjusted'] = false; 
                            ...
                            $expectedAfterBalance = $expectedAfterBalance - $betAmount;                  
                    
                            $game_usernames[$userId]['before_balance'] = $before_balance;
                            $game_usernames[$userId]['after_alance'] = $expectedAfterBalance;
    
                            //collect total bet amount per player
                            $game_usernames[$userId]['total_bet_amount'] += $betAmount;
                        }
                        ...
                        //collect total bet amount per player
                        $game_usernames[$userId]['total_bet_amount'] += $betAmount;
                    }//end _duplicateVoidBet

                }// end _toAdjustBalance

            }//end check userId

        }//end foreach paramsValue



        //increase balance in 1 go based on accumulated total bet amount per player
        foreach($game_usernames as $userId => $playerIncrement){
            $playerId =$playerIncrement['player_id'];
            $totalBetAmount = $playerIncrement['total_bet_amount'];
            if($totalBetAmount<=0){
                continue;
            }
            $success = $this->lockAndTransForPlayerBalance($playerId, function() use($playerId, 
            ...
            $totalBetAmount) {
                ...
                $tempUniqueIds = json_encode($playerIncrement['transaction_ids']);
                
                $uniqueid_of_seamless_service = $controller->game_api->getPlatformCode().'-voidBet-'.md5($tempUniqueIds);
                $this->wallet_model->setUniqueidOfSeamlessService($uniqueid_of_seamless_service); 

                //call increment
                $success = $this->wallet_model->incSubWallet($playerId, $controller->gamePlatformId,$totalBetAmount);

                if(!$success) {
                    $this->CI->utils->error_log(__METHOD__. " failed to increment: ", 'playerIncrement', $playerIncrement,$playerId, $controller->gamePlatformId,$totalBetAmount);
                }

                foreach($playerIncrement['transaction_ids'] as $transId){
                    //flag transactions as void
                    $updateBetData = [
                        'action_status'=>Game_logs::STATUS_VOID
                    ];
                    $this->sv388_transactions->updateTransactionData($transId, 'bet', $updateBetData);

                    //update void                    
                    $updateVoidBetData = [
                        'remote_wallet_uniqueid'=>$uniqueid_of_seamless_service,                        
                        'action_status'=>Game_logs::STATUS_SETTLED
                    ];
                    $this->sv388_transactions->updateTransactionData('voidBet-'.$transId, 'voidBet', $updateVoidBetData);
        ...
        $this->CI->utils->debug_log(__METHOD__. " savedVoidTransactions ", $savedVoidTransactions);
	...
	public function unvoidBet($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				...
				//get bet
				$existingBetRow = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'bet');
				...
				$existingVoidBetRow = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'voidBet');
				...
				$existingUnvoidBetRow = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'unvoidBet');
				...
				$updateLogs = $this->sv388_transactions->updateGameLog($existingRow['id'],$paramsValue);
	...
	public function voidSettle($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				$betAmount = isset($paramValue['betAmount']) ? $paramValue['betAmount'] : null;
				$transaction_id = isset($paramValue['platformTxId']) ? $paramValue['platformTxId'] : null;
				$this->CI->utils->debug_log('cholotransactino', $transaction_id,$betAmount);

				//check if there is settle
				$existingRow = $this->sv388_transactions->isRowBySettleId( 'settle-'.$transaction_id);
				...
				$existingVoidSettleRow = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'voidSettle');				
	...
	public function unvoidSettle($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				...
				$existingUnvoidRow = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'unvoidSettle');				
				...
				$existingSettleRow = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'settle');				
				...
				$existingRow = $this->sv388_transactions->isRowByVoidedTransactionId($transaction_id);
	...
	public function adjustBet($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				...
				$existingRow = $this->sv388_transactions->isTransactionIdAlreadyExists($transaction_id);
				# check if Transaction ID does not exist
				if(!$existingRow){
	...
	public function refund($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				...
				$existingRow = $this->sv388_transactions->isRowByTransactionId($transaction_id);
				# check if Transaction ID does not exist
				if(empty($existingRow)){
					$status_code['code'] = '1017';
					return false;
	...
	*/
	public function give($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				...
				$existingRow = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'give');
	...
	protected function doDeduct($playerId,$playerUsername,$betAmount,$fullRequest,$data){
		...
		// $unique_transaction = isset($data['external_uniqueid']) ? $data['external_uniqueid'] : null;
		// $uniqueid_of_seamless_service = $this->game_api->getPlatformCode().'-'.$unique_transaction;
		// $this->wallet_model->setUniqueidOfSeamlessService($uniqueid_of_seamless_service);
        ...
        if($action == 'bet' || $action == 'unvoidBet') {
        	...
        	foreach ($data as $trans_record) {
    			...
    			$transaction_id = isset($trans_record['platformTxId']) ? $trans_record['platformTxId'] : null;
				$existingRow = $this->sv388_transactions->getTransactionRecord($transaction_id, 'bet');
				...
				//get pending cancel
				$existingCancelRow = $this->sv388_transactions->getTransactionRecord('cancelBet-' . $transaction_id, 'cancelBet');
				if(!empty($existingCancelRow) && $existingCancelRow['action_status']==Game_logs::STATUS_PENDING){
					$pendingCancel[] = $existingCancelRow;
				}
        	}

			$this->CI->utils->debug_log(__METHOD__. ' pendingCancel:', $pendingCancel, $data);
            ...
            $initialBeforeBalance = $this->game_api->queryPlayerBalance($playerUsername)['balance'];
            $finalAfterBalance = $initialBeforeBalance;
            $computedAfterBalance = $initialBeforeBalance;
            
			$this->CI->utils->debug_log('<-----------------------------bermar initialBeforeBalance:', $initialBeforeBalance);

            $remoteUniqueIdArr = array_column($data, 'platformTxId');
            $remoteUniqueId = implode($remoteUniqueIdArr);
            $remoteUniqueId = md5($remoteUniqueId);

            //do single total deduction
            $isDeduct = $this->wallet_model->decSubWallet($playerId, $this->gamePlatformId,$totalAmount);
            //$finalAfterBalance = $this->wallet_model->getRemoteWalletAfterBalance();
            if($this->utils->isEnabledRemoteWalletClient()){
                $finalAfterBalance = $this->wallet_model->getRemoteWalletAfterBalance();
            }else{
                $finalAfterBalance = $this->game_api->queryPlayerBalanceByPlayerId($playerId)['balance'];
            }
			...
			$this->CI->utils->debug_log('<-----------------------------bermar getRemoteWalletAfterBalance:', $finalAfterBalance);

            if($isDeduct){

                //for each bet process transaction
                foreach ($data as $trans_record) {
                    $betAmount = $trans_record['betAmount'];
                    $computedAfterBalance = $computedAfterBalance - abs($betAmount);
                    $status_code['after_balance'] = $computedAfterBalance;
                    $trans_record['action_status'] = Game_logs::STATUS_PENDING;

            
                    $this->CI->utils->debug_log('<-----------------------------bermar computedAfterBalance:', $computedAfterBalance);
                    
                    $insertResponse = $this->processTransactionRecord($trans_record,$action,$before_balance['balance'],$computedAfterBalance);
                    if(!$insertResponse){
                        $status_code['code'] = '9999';
                        return false;
                    }
                }//end for each bet trans process

                //processing pending cancelbet
                foreach ($data as $trans_record) {
                    //process pending cancel
                    foreach($pendingCancel as $cancel){
                        $trimmed_platformTxId = str_replace('cancelBet-', '', $cancel['platformTxId']);
                        if($trimmed_platformTxId==$trans_record['platformTxId']){
                            //increment balance
                            $isIncrement = $this->wallet_model->incSubWallet($playerId, $this->gamePlatformId,abs($betAmount));
                            if($isIncrement == false) {
                                $status_code['code'] = '9999';
                                return false;
                            }

                            $cancelBeforeBalance = $computedAfterBalance;
                            $computedAfterBalance = $computedAfterBalance + abs($betAmount);
                            $status_code['after_balance'] = $computedAfterBalance;
                            ...
                            'after_balance'=>$computedAfterBalance];
                            $updateResp = $this->sv388_transactions->updateCancelBetTransactionData($transaction_id, $updateData);
                            if(!$updateResp){
                                $status_code['code'] = '9999';
                                return false;
                            }
                            
                            //update transaction flag as cancelbet
                            $updateBetData = ['action_status'=>Game_logs::STATUS_CANCELLED];
                            $updateResp = $this->sv388_transactions->updateTransactionData($transaction_id, 'bet', $updateBetData);
                            
                        }
                    }	//end process pending balance per bet

                }//end for each bet trans process
            }//end is deduct checking
    ...
    protected function doIncrement($playerId,$playerUsername,$addAmount,$fullRequest,$data){
        ...
        if($action == 'cancelBet') {
    		foreach ($data as $trans_record) {
				$transaction_id = isset($trans_record['platformTxId']) ? $trans_record['platformTxId'] : null;
				$this->CI->utils->debug_log('cholotransaction_id', $transaction_id);
				$trans_record['action_status'] = Game_logs::STATUS_SETTLED;

				$before_balance = $this->game_api->queryPlayerBalance($playerUsername);

				//check if cancelAlready processed					
				$isCancelledAlready = $this->sv388_transactions->getTransactionRecord('cancelBet-' . $transaction_id, 'cancelBet');
				//cancelles action_status=settled meaning the cancel is valid
				$this->CI->utils->debug_log('cholo1', $isCancelledAlready);
				if($isCancelledAlready&&$isCancelledAlready['action_status']==Game_logs::STATUS_SETTLED){	
					$this->CI->utils->debug_log('cholo2', $isCancelledAlready);				
					$status_code['after_balance'] = $this->game_api->queryPlayerBalance($playerUsername)['balance'];
					$status_code['code'] = '0000';
					return $status_code;					
				}

				$existingRow = $this->sv388_transactions->isRowByBetTransactionId($transaction_id);
				$this->CI->utils->debug_log('cholo3', $existingRow);		
				...
				//flag bet cancelled
				$updateData = ['action_status'=>Game_logs::STATUS_CANCELLED];
				$this->CI->utils->debug_log('cholo5', $updateData);
				$updateResp = $this->sv388_transactions->updateTransactionData($transaction_id, 'bet', $updateData);
				if(!$updateResp){
    ...
    */
    private function processTransactionRecord(&$trans_record,$action,$before_balance=null,$after_balance=null) {
		...
		$data['group_transaction_id'] = isset($trans_record['platformTxId']) ? $trans_record['platformTxId'] : null;
        
		$data['response_result_id'] = isset($trans_record['response_result_id']) ? $trans_record['response_result_id'] : null;
		        
        $data['external_uniqueid'] = $action . "-" . $data['platform'] .'-'. $data['platformTxId'];
        ...
        if($action == 'settle' || $action == 'unsettle') {
			...
			$data['external_uniqueid'] = $action . "-" . $data['platform'] .'-'. $data['platformTxId'] . '-' . $promotionId;
		}elseif($action == 'resettle') {
			$data['winAmount'] = isset($trans_record['resettle_win_amount']) ? $trans_record['resettle_win_amount'] : null;
			$data['platformTxId'] = $action . "-" . $data['platformTxId'];
			//$data['external_uniqueid'] = $data['platform'] .'-'. $data['platformTxId'];
		}elseif($action == 'voidBet'){
			$data['platformTxId'] = $action . "-" . $data['platformTxId'];
			//$data['external_uniqueid'] = $data['platform'] .'-'. $data['platformTxId'];
		}elseif($action == 'voidSettle'){
			$data['platformTxId'] = $action . "-" . $data['platformTxId'];
			//$data['external_uniqueid'] = $data['platform'] .'-'. $data['platformTxId'];
		}elseif($action == 'cancelBet'){
			$data['platformTxId'] = $action . "-" . $data['platformTxId'];
			//$data['external_uniqueid'] = $data['platform'] .'-'. $data['platformTxId'];
		} else {
			//$data['external_uniqueid'] = $action . "-" . $data['platform'] .'-'. $data['platformTxId'];
		...
		$this->CI->utils->debug_log('choloresettle_is_rejected', $resettle_is_rejected);
		$trans_record = $data;
        ...
        if($action == 'voidBet'){
            ...
            $existExternal = $this->sv388_transactions->isExternalUniqueExist($data['external_uniqueid']);
            if($existExternal) {
                return true;
            }
        }
		...
		/*if($action == 'settle' || $action == 'resettle') {
			...
			if($action == 'resettle' && $resettle_is_rejected){
				$this->CI->utils->debug_log('choloresettle_is_rejected1', $resettle_is_rejected);
				$trans_record['action_status'] = Game_logs::STATUS_REJECTED;
			}
		}*/
		$this->CI->utils->debug_log(__METHOD__,'Sexy Baccarat (processTransactionRecord) insert',$trans_record);
    ...
    private function insertTransactionWithLog($data) {
        ...
        $affected_rows = $this->sv388_transactions->insertTransaction($data);
        ...
        if($affected_rows) {
            ...
            $this->transaction_for_fast_track['id'] = $this->CI->sv388_transactions->getLastInsertedId();
	...
	public function resettle($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				...
				$winAmount = isset($paramValue['winAmount']) ? $paramValue['winAmount'] : null;
				$transaction_id = isset($paramValue['platformTxId']) ? $paramValue['platformTxId'] : null;
				$betRow = $controller->sv388_transactions->getTransactionRecord($transaction_id, 'bet');
				...
				$resettle = $controller->sv388_transactions->getTransactionRecord('resettle-' . $transaction_id, 'resettle');
				if(!empty($resettle)){
					$status_code['code'] = self::SUCCESS;
					return true;
				}

				$settleRow = $controller->sv388_transactions->getTransactionRecord('settle-' . $transaction_id, 'settle');
				if(empty($settleRow)){
					$status_code['code'] = self::STATUS_CODE_FAIL;
					return false;
				}
				...
				if($this->utils->compareResultFloat($ressettleWinAmount, '<', 0)) {
					...
					$this->CI->utils->debug_log('choloresettleWinAmount',$ressettleWinAmount);
				...
				$updateData = ['action_status'=>Game_logs::STATUS_SETTLED];
				$this->CI->utils->debug_log('choloupdateData',$updateData);
				$updateResp = $this->sv388_transactions->updateTransactionData($transaction_id, 'bet', $updateData);
				$this->CI->utils->debug_log('choloupdateResp',$updateResp);
				if(!$updateResp){
					$status_code['code'] = '9999';
					return false;
				}
		...
		$this->CI->utils->debug_log('<-----------------------------Status Code choloupdate', $status_code);
		...
		$this->CI->utils->debug_log('<-----------------------------Response Params: choloupdate', $response);
