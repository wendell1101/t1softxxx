...
class Testing_lib_game_agbbin_api extends BaseTesting {

	private $platformCode = AGBBIN_API;
	...
	//	$this->testCreatePlayer();
		//$this->testDepositToAGBBIN();
		//$this->testPlayerBalanceAGBBIN();
		//$this->testWithdrawalToAGBBIN();
		//$this->blockPlayer();
		//$this->unblockPlayer();
		//$this->queryGameRecords();
		//$this->testSyncGameLogs();
		//$this->testUpdateAGBBINGameDescription();
		$this->testSyncMergeToGameLogs();
		//$this->testIsPlayerExist();
	}

	public function testUpdateAGBBINGameDescription(){
		$rlt = $this->api->updateAGBBINGameDescription();
	}
	...
	private function testCreatePlayer() {
		...
		$this->test($result['success'], true, 'test create player for AG');
		// log_messAGBBINe('error', 'get password:' . $password);
		// $this->test($password, $player['password'], 'test create player for AGBBINBBIN');
	}

	private function testDepositToAGBBIN() {
		...
		$this->test($balResult['success'], true, 'Test Player Deposit to AGBBIN');
	}

	private function testWithdrawalToAGBBIN() {
		$playerName = 'asriinew2';
		$balResult = $this->game_platform_manager->withdrawFromGame($playerName, 5);
		log_message("error", var_export($balResult, true));
		$this->test($balResult['success'], true, 'Test Player Withdrawal to AGBBIN');
	}

	private function testPlayerBalanceAGBBIN() {
		$playerName = 'asriinew2';
	...
	private function testSyncGameLogs() {
		...
		$token = 'abc123';
		$dateTimeFrom = new DateTime('2016-11-15 00:00:00');
		$dateTimeTo = new DateTime('2016-11-16 23:59:59');
	...
	private function testIsPlayerExist() {
		...
		$this->test($rlt['success'], true, 'testIsPlayerExist for AGBBIN');
