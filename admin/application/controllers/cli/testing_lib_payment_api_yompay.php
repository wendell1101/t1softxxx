...
class Testing_lib_payment_api_yompay extends BaseTesting {

	private $platformCode = YOMPAY_PAYMENT_API;
	private $api = null;
	...
	public function init() {
		...
		$this->test($apiClassName, 'payment_api_yompay', 'Test loaded API\'s class name. Expected: payment_api_yompay');
		...
		$this->test($this->api->getPrefix(), 'yompay', 'Test loaded API\'s prefix. Expected: yompay');
	...
	private function testCreateUrlAndCallback() {
		...
		## Simulate the params returned from yompay gateway
		## Reference: code sample, callback.php
		$params = array(
			'mer_no' => $this->api->getSystemInfo('yompay_MER_NO'),
			'order_no' => $order->secure_id,
			'order_amount' => number_format($amount, 2, '.', ''),
			'return_params' => 'NA',
			'trade_no' => '1234567',
			'trade_time' => time(), # current unix timestamp
			'trade_status' => '1',
		);
		$params['sign'] = $this->api->signCallback($params);
		...
		$params = array(
			...
			'trade_no' => '1234567',
			'trade_time' => time(), # current unix timestamp
			'trade_status' => '2', # Status != 1 means fail
		);
		$params['sign'] = $this->api->signCallback($params);
		...
		$params = array(
			'mer_no' => $this->api->getSystemInfo('yompay_MER_NO'),
			'order_no' => '00000', # invalid order number
			'order_amount' => number_format($amount, 2, '.', ''),
			'return_params' => 'NA',
			'trade_no' => '1234567',
			'trade_time' => time(), # current unix timestamp
		...
		$params = array(
			'mer_no' => $this->api->getSystemInfo('yompay_MER_NO'),
			'order_no' => $order->secure_id,
			'order_amount' => number_format(9999, 2, '.', ''),  # wrong payment amount
			'return_params' => 'NA',
			'trade_no' => '1234567',
			'trade_time' => time(), # current unix timestamp
			'trade_status' => '1',
