...
class Testing_lib_payment_api_24kpay extends BaseTesting {

	private $platformCode = PAY24K_PAYMENT_API;
	private $api = null;
	...
	public function init() {
		...
		$this->test($apiClassName, 'payment_api_24kpay', 'Test loaded API\'s class name. Expected: payment_api_24kpay');
		...
		$this->test($this->api->getPrefix(), '24kpay', 'Test loaded API\'s prefix. Expected: 24kpay');
	}
	...
	private function testCreateUrlAndCallback() {
		...
		$transactionCode = 'W0000009300000002'; # Enter a transactionCode form walletaccount, status must be wait_API
		...
		$this->test($result['success'], true,
			"Test API callbackFromServer function. Expected: success = true. Manually check withdrawal status for [$transactionCode], expected: paid");
