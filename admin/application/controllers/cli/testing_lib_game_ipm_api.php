...
class Testing_lib_game_ipm_api extends BaseTesting {

	private $platformCode 	= SPORTSBOOK_API;
	private $platformName 	= 'IPM';
	private $api 			= null;
	...
	public function init() {
		...
		$username = 'test' . random_string('numeric', 12);
	...
	public function testAll() {
		...
		var_dump(['createPlayer' => $result]);

		$result = $this->testQueryForwardGame();
		var_dump(['queryForwardGame' => $result]);

		$result = $this->testIsPlayerExist();
		var_dump(['isPlayerExist' => $result]);

		$result = $this->testLogin();
		var_dump(['login' => $result]);

		$result = $this->testLogout();
		var_dump(['logout' => $result]);

		$result = $this->testDepositToGame();
		var_dump(['depositToGame' => $result]);

		$result = $this->testCheckFundTransfer($result['transferId']);
		var_dump(['checkFundTransfer' => $result]);

		$result = $this->testWithdrawFromGame();
		var_dump(['withdrawFromGame' => $result]);

		$result = $this->testQueryPlayerBalance();
		var_dump(['queryPlayerBalance' => $result]);

		$result = $this->testBatchQueryPlayerBalance();
		var_dump(['batchQueryPlayerBalance' => $result]);

		$result = $this->testSyncOriginalGameLogs();
		var_dump(['syncOriginalGameLogs' => $result]);

		$result = $this->testSyncMergeToGameLogs();
		var_dump(['syncMergeToGameLogs' => $result]);
	}
	...
	private function testQueryForwardGame() {
		$rlt = $this->api->queryForwardGame($this->test_player->username, null);
		return $rlt;
	}
	...
	private function testLogin() {
		$rlt = $this->api->login($this->test_player->username);
		return $rlt;
	}
	...
	private function testCheckFundTransfer($transferId) {
		$rlt = $this->api->checkFundTransfer($this->test_player->username, $transferId);
		return $rlt;
	}
	...
	private function testSyncOriginalGameLogs() {
		...
		$dateTimeTo 	 = new DateTime('2016-07-29 23:59:59');
