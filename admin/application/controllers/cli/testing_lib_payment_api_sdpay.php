...
class Testing_lib_payment_api_sdpay extends BaseTesting {

	private $platformCode = SDPAY_PAYMENT_API;
	private $api = null;
	...
	public function init() {
		...
		$this->test($apiClassName, 'payment_api_sdpay', 'Test loaded API\'s class name. Expected: payment_api_sdpay');
		...
		$this->test($this->api->getPrefix(), 'sdpay', 'Test loaded API\'s prefix. Expected: sdpay');
	...
	private function testGeneratePaymentUrlForm() {
		...
		$this->test($result['type'], Abstract_payment_api::REDIRECT_TYPE_URL,
