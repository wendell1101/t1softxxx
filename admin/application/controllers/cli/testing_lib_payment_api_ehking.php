...
class Testing_lib_payment_api_ehking extends BaseTesting {

	private $platformCode = EHKING_PAYMENT_API;
	private $api = null;
	...
	public function init() {
		...
		$this->test($apiClassName, 'payment_api_ehking', 'Test loaded API\'s class name. Expected: payment_api_ehking');
		...
		$this->test($this->api->getPrefix(), 'ehking', 'Test loaded API\'s prefix. Expected: qianwang');
	...
	private function testCallbackSign() {
		...
		$json='{"completeDateTime":"2016-10-27 19:45:32","hmac":"ca77eb0fbaa43cf27f9dcea4c2cae186","merchantId":"120140677","orderAmount":"800","orderCurrency":"CNY","requestId":"D615446720647","serialNumber":"f9c593d899714a7998c37e1aafa36dae","status":"SUCCESS","totalRefundAmount":"0","totalRefundCount":"0"}';

		$data=json_decode($json, true);
		$sign=$this->api->server_sign($data);

		$this->utils->debug_log('data', $data, 'sign', $sign);
		$this->test($sign, $data['hmac'], 'Test API sign');

		$this->api->verify($data, $data['hmac']);
