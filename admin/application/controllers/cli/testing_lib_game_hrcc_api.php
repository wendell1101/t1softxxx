...
class Testing_lib_game_hrcc_api extends BaseTesting {

	private $platformCode = HRCC_API;
	private $platformName = 'HRCC';
	private $api = null;
	...
	private function testDeposit() {
		...
		$this->test($rlt['success'], true, 'Test Player Deposit to HRCC');
	...
	private function testWithdraw() {
		...
		$this->test($rlt['success'], true, 'Test Player Withdraw to HRCC');
	...
	private function testSyncGameLogs() {
		...
		$this->test($rlt['success'], true, 'Test syncOriginalGameLogs to HRCC');
	}

	private function testSyncMergeToGameLogs() {
		//$api = $this->game_platform_manager->initApi($this->platformCode);
		$token = 'abc123';
		$dateTimeFrom = new DateTime('2016-07-07 00:00:00');
		$dateTimeTo = new DateTime('2016-07-07 23:59:59');

		$playerName = 'swtest008';

		$this->api->syncInfo[$token] = array("playerName" => $playerName, "dateTimeFrom" => $dateTimeFrom, "dateTimeTo" => $dateTimeTo);
		$rlt = $this->api->syncMergeToGameLogs($token);
		$this->test($rlt['success'], true, 'Test syncMergeToGameLogs to HRCC');
	}
	...
	private function testQueryForwardGame() {
		...
		// var_dump($rlt);exit();
		$this->test($rlt['success'], true, 'Test testQueryForwardGame to HRCC');
	}
}