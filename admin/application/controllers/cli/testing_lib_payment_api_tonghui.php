...
class Testing_lib_payment_api_tonghui extends BaseTesting {

	private $platformCode = TONGHUI_PAYMENT_API;
	private $api = null;
	...
	public function init() {
		...
		$this->test($apiClassName, 'payment_api_tonghui', 'Test loaded API\'s class name. Expected: [payment_api_tonghui], Actual: ['.$apiClassName.']');
		...
		$this->test($this->api->getPrefix(), 'tonghui', 'Test loaded API\'s prefix. Expected: [tonghui], Actual: ['.$this->api->getPrefix().']');
	}
	...
	private function testGeneratePaymentUrlForm() {
		...
		# Call with parameter to turn off redirect to 2nd url
		...
		$this->test($resultComplete, true,
			"Test API generatePaymentUrlForm function return value has all required fields. Expected: [True], Actual [$resultComplete]");
		...
		$this->test($result['success'], true,
			'Test API generatePaymentUrlForm function return field "success". Expected: [True], Actual: ['.$result['success'].']');
		$this->test($result['type'], Abstract_payment_api::REDIRECT_TYPE_FORM,
			'Test API generatePaymentUrlForm function return field "type". Expected: ['. Abstract_payment_api::REDIRECT_TYPE_FORM .'], Actual: ['.$result['type'].']');
		$this->test(!empty($result['url']), true,
			'Test API generatePaymentUrlForm function return field "url". Expected: non-empty string, Actual: ['.$result['url'].']');
		...
		$this->test(array_key_exists('sign', $result['params']) && !empty($result['params']['sign']), true,
			'Test API generatePaymentUrlForm function return field "params" must contain a non-empty signature. Expected: non-empty string, Actual: ['.$result['params']['sign'].']');
		$this->test(array_key_exists('input_charset', $result['params']) && !empty($result['params']['input_charset']), true,
			'Test API generatePaymentUrlForm function return field "params" must contain a non-empty input_charset. Expected: non-empty string, Actual: ['.$result['params']['input_charset'].']');
		$this->test(array_key_exists('req_referer', $result['params']) && !empty($result['params']['req_referer']), true,
			'Test API generatePaymentUrlForm function return field "params" must contain a non-empty req_referer. Expected: non-empty string, Actual: ['.$result['params']['req_referer'].']');
	...
	private function testCreateUrlAndCallback() {
		...
		## Simulate the params returned from TONGHUI gateway
		## Reference: code sample, callback.php
		$params = array(
			'merchant_code' => $this->api->getSystemInfo('tonghui_merchant_code'),
			'order_no' => $order->secure_id,
			'order_amount' => number_format($amount, 2, '.', ''),
			'order_time' => '2016-05-15 00:00:00',
			'notify_type' => 'back_notify',
			'trade_no' => '12345',
			'trade_time' => '2016-05-15 00:01:00',
		...
		# Order Numbers do not match. order_no: 'invalid order number'
		$params = array(
			'merchant_code' => $this->api->getSystemInfo('tonghui_merchant_code'),
			'order_no' => 'invalid order number',
			'order_amount' => number_format($amount, 2, '.', ''),
			'order_time' => '2016-05-15 00:00:00',
			'notify_type' => 'back_notify',
			'trade_no' => '12345',
		...
		# Payment amounts do not match. order_amount: 123.45
		$params = array(
			'merchant_code' => $this->api->getSystemInfo('tonghui_merchant_code'),
			'order_no' => $order->secure_id,
			'order_amount' => number_format('123.45', 2, '.', ''),
			'order_time' => '2016-05-15 00:00:00',
			'notify_type' => 'back_notify',
			'trade_no' => '12345',
			'trade_time' => '2016-05-15 00:01:00',
