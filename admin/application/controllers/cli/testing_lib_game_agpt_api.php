...
class Testing_lib_game_agpt_api extends BaseTesting {

	private $platformCode = AGPT_API;
	...
	public function testAll() {
		...
		//$this->testDepositToAGPT();
		//$this->testPlayerBalanceAGPT();
		//$this->testWithdrawalToAGPT();
		//$this->blockPlayer();
		//$this->unblockPlayer();
	...
	private function testCreatePlayer() {
		...
		$this->test($result['success'], true, 'test create player for AGPT');
		// log_message('error', 'get password:' . $password);
		// $this->test($password, $player['password'], 'test create player for AGPT');
	}

	private function testDepositToAGPT() {
		...
		$this->test($balResult['success'], true, 'Test Player Deposit to AGPT');
	}

	private function testWithdrawalToAGPT() {
		$playerName = 'asriinew2';
		$balResult = $this->game_platform_manager->withdrawFromGame($playerName, 5);
		log_message("error", var_export($balResult, true));
		$this->test($balResult['success'], true, 'Test Player Withdrawal to AGPT');
	}

	private function testPlayerBalanceAGPT() {
		$playerName = 'asriinew2';
	...
	private function testIsPlayerExist() {
		...
		$this->test($rlt['success'], true, 'testIsPlayerExist for AGPT');
