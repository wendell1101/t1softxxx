...
//http://admin.idr.staging.seamlessgateway.t1t.in/sexybaccarat_remote_service_api
//https://allwecan.info/single-wallet-testing game provider automated testing
require_once dirname(__FILE__) . '/BaseController.php';

class Sexybaccarat_remote_service_api extends BaseController {
				...
				foreach ($txns as $key => $txn) {
					...
					if(!empty($tipRow)){
						if($tipRow['action'] == "tip" ){
							...
							$externalUniqueId = $action.'-'.$txn['platform'].'-'.$uniqueId;
							$cancelExist = $this->sexy_baccarat_transactions->isExternalUniqueExist($externalUniqueId);
							if($cancelExist){
								$success = true; #always
								continue;
							} else {
	...
	public function playerBet($playerId, $playerUsername, $gameUsername, $fullRequest, $paramsValue) {
        ...
        $success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramsValue) {
            ...
            foreach ($paramsValue as &$trans_record) {
                ...
                $relatedTrans = $controller->sexy_baccarat_transactions->getAllRelatedTransactions($transaction_id, $playerId);
            ...
            foreach($savedBetTransactions as $pendingBet){
                ...
                $isbetExists = $controller->sexy_baccarat_transactions->isBetExists($pendingBet['platformTxId'],Game_logs::STATUS_SETTLED);
                ...
                if(isset($pendingBet['_cancelTrans'])&&!empty($pendingBet['_cancelTrans'])){
                   $betAmount = abs($pendingBet['betAmount']);
                    ...
                    $updateBetResp = $controller->sexy_baccarat_transactions->updateTransactionData($transactionId, 'bet', $updateBetData);
                    if(!$updateBetResp){
                    ...
                    $updateCancelBetResp = $controller->sexy_baccarat_transactions->updateTransactionData($transactionId, 'cancelBet', $updateCancelData);
        ...
        foreach ($paramsValue as $paramValue) {
		    if(isset($paramValue['userId'])) {
                ...
                &$savedSettleTransactions) {
                    ...
                    $relatedTrans = $this->sexy_baccarat_transactions->getAllRelatedTransactions($transactionId, $playerId);
                    ...
                    if($temp['_toAdjustBalance']){           
                        ...
                        $updateBetResp = $this->sexy_baccarat_transactions->updateTransactionData($transactionId, 'bet', $updateBetData);
                        if(!$updateBetResp){                                        
                            $temp['_toAdjustBalance'] = false;
                            $temp['_betTransactionSettled'] = false;  
                            $temp['_balanceAdjusted'] = false;                             
        ...
        foreach($game_usernames as $userId => $playerIncrement){
            ...
            $totalBetAmount) {
                ...
                foreach($playerIncrement['transaction_ids'] as $transId){
                    ...
                    $this->sexy_baccarat_transactions->updateTransactionData($transId, 'bet', $updateBetData);
                    ...
                    $this->sexy_baccarat_transactions->updateTransactionData('voidBet-'.$transId, 'voidBet', $updateVoidBetData);
...
###################################################################
		    ...
		    if(isset($paramValue['userId'])) {
                ...
                $relatedTrans) {
                    ...
                    if($temp['_toAdjustBalance']){           
                        ...
                        //flag bet transaction as voided
                        $updateBetData = [
                            'action_status'=>Game_logs::STATUS_VOID,
                            'cancel_before'=>$before_balance,
                            'cancel_after'=>$expectedAfterBalance,
                        ];
                        $updateBetResp = $this->sexy_baccarat_transactions->updateTransactionData($transactionId, 'bet', $updateBetData);
                        if(!$updateBetResp){                                        
                            $temp['_toAdjustBalance'] = false;
                        ...
                        if($betAmount>0){
                            //call increment
                            $success = $this->wallet_model->incSubWallet($playerId, $controller->gamePlatformId,$betAmount);

                            if(!$success) {
                                $temp['_balanceAdjusted'] = false;  
                                $this->CI->utils->error_log(__METHOD__. " failed to increment: ", $paramValue, 'temp', $temp);
                            }		
                        }else{
                            $this->CI->utils->debug_log(__METHOD__. " bet amount is 0 no need to call remote: ", $temp);
                            $success = true;                        
                            $temp['_balanceAdjusted'] = true;    
                        }


                    }//end if _toAdjustBalance 
	...
	public function getDateNow(){
		$newDate = new DateTime();
		$milliseconds = floor($newDate->format('u') / 1000); // Convert microseconds to milliseconds
		$date = $newDate->format('Y-m-d\TH:i:s.') . sprintf('%03d', $milliseconds) . $newDate->format('P');
		return $date;
	}
	...
	public function voidSettle($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				...
				$existingRow = $this->sexy_baccarat_transactions->isRowBySettleId( 'settle-'.$transaction_id);
	...
	protected function doDeduct($playerId,$playerUsername,$betAmount,$fullRequest,$data){
        ...
        if($action == 'bet' || $action == 'unvoidBet') {
        	...
        	foreach ($data as $trans_record) {
				...
				//get pending cancel
				$existingCancelRow = $this->sexy_baccarat_transactions->getTransactionRecord('cancelBet-' . $transaction_id, 'cancelBet');
                ...
                foreach ($data as $trans_record) {
                    ...
                    foreach($pendingCancel as $cancel){
                        ...
                        if($trimmed_platformTxId==$trans_record['platformTxId']){
                            ...
                            $updateResp = $this->sexy_baccarat_transactions->updateCancelBetTransactionData($transaction_id, $updateData);
                            if(!$updateResp){
                                $status_code['code'] = '9999';
                                return false;
                            }
                            ...
                            $updateResp = $this->sexy_baccarat_transactions->updateTransactionData($transaction_id, 'bet', $updateBetData);
		...
		if($action == 'resettle'){
			$isDeduct = $this->wallet_model->decSubWalletAllowNegative($playerId, $this->gamePlatformId,$betAmount);
		}else{
    ...
    protected function doIncrement($playerId,$playerUsername,$addAmount,$fullRequest,$data){
        ...
        if($action == 'cancelBet') {
    		foreach ($data as $trans_record) {
				...
				$isCancelledAlready = $this->sexy_baccarat_transactions->getTransactionRecord('cancelBet-' . $transaction_id, 'cancelBet');
	...
	public function resettle($playerId,$playerUsername,$gameUsername,$fullRequest,$paramsValue) {
		...
		foreach ($paramsValue as $paramValue) {
			...
			$success = $this->lockAndTransForPlayerBalance($playerId, function() use($controller,$playerId,$playerUsername,&$status_code,$fullRequest,&$paramValue) {
				...
				$resettle = $controller->sexy_baccarat_transactions->getTransactionRecord('resettle-' . $transaction_id, 'resettle');
				if(!empty($resettle)){
					$status_code['code'] = self::SUCCESS;
					return true;
				}

				$settleRow = $controller->sexy_baccarat_transactions->getTransactionRecord('settle-' . $transaction_id, 'settle');
				if(empty($settleRow)){
					$status_code['code'] = self::STATUS_CODE_FAIL;
					return false;
				}
