...
class Softswiss_seamless_service_api extends BaseController
{
    private $request_headers;
    private $request_params;
    private $callback_method;
    private $game_platform_id;
    private $softswiss_provider_for_acceptance;
    private $game_api;
    private $casino_id;
    private $auth_token;
    private $language;
    ...
    public function __construct()
    {
        ...
        $this->custom_message = '';
        $this->softswiss_provider_for_acceptance = $this->utils->getConfig('softswiss_provider_for_acceptance');
        $this->game_platform_id = $this->softswiss_provider_for_acceptance;
        $this->provider_identifier = self::ACCEPTANCE_TEST_PROVIDER_IDENTIFIER;
        $this->precision = 2;
        $this->conversion = 1;
        $this->callback_method = '__null';
    }
    ...
    public function index($method)
    {
        ...
        $this->utils->debug_log(__CLASS__, __METHOD__, 'request_params', $this->request_params);
        ...
        /* if (!isset($this->request_params['game'])) {
            $this->custom_message = 'Parameter game is required to get the sub provider identifier.';
            return $this->setResponse(self::HTTP_RESPONSE_STATUS_CODE_500, self::API_ERROR_502);
        } */

        // get game platform id using game parameter.
        $game_code = !empty($this->request_params['game']) ? $this->request_params['game'] : null;
        $provider_identifier = $this->getProviderIdentifierByGameCode($game_code);
        $this->game_platform_id = $this->getGamePlatformId($provider_identifier);

        if (is_null($this->game_platform_id)) {
            $this->game_platform_id = $this->softswiss_provider_for_acceptance;
            $this->custom_message = "Invalid game parameter. Can't find provider identifier";
        ...
        if ($this->game_api) {
            ...
            $this->provider_identifier = $this->game_api->provider_identifier;
        ...
        /* if ($this->provider_identifier != $provider_identifier) {
            $this->custom_message = "Invalid provider identifier in extra info.";
            return $this->setResponse(self::HTTP_RESPONSE_STATUS_CODE_500, self::API_ERROR_502);
        } */

        if (in_array($this->callback_method, $this->whitelist_ip_validate_api_methods)) {
        ...
        if (in_array($this->callback_method, $this->game_api_active_validate_api_methods)) {
        ...
        if (in_array($this->callback_method, $this->game_api_maintenance_validate_api_methods)) {
    ...
    private function playRequest()
    {
        ...
        foreach ($actions as $request_params) {
            ...
            list($success, $balance, $action_result, $http_response, $operator_response) = $this->$action($request_params); // bet or win
    ...
    private function bet($request_params)
    {
        $this->callback_method = __FUNCTION__;
        ...
        $is_rollback_exist = $this->original_seamless_wallet_transactions->isTransactionExistCustom($this->transaction_table, [
        ...
        if ($is_rollback_exist) {
            $success = true;
            $transaction_data = $this->original_seamless_wallet_transactions->querySingleTransactionCustom($this->transaction_table, ['rollback_original_action_id' => $transaction_id]);
        ...
        if (!$this->transaction_already_exist) {
            ...
            $success = true;
            $transaction_data = $this->original_seamless_wallet_transactions->querySingleTransactionCustom($this->transaction_table, ['transaction_id' => $transaction_id]);
            $balance = !empty($transaction_data['after_balance']) ? $transaction_data['after_balance'] : 0;
    ...
    private function win($request_params)
    {
        $request_params['amount'] = $request_params['amount'] / $this->conversion;
        $this->callback_method = __FUNCTION__;
        $balance = 0;
        ...
        $is_bet_exist = $this->original_seamless_wallet_transactions->isTransactionExistCustom($this->transaction_table, [
    ...
    private function processRollbackRequest($request_params)
    {
        ...
        $transaction_data = $this->original_seamless_wallet_transactions->querySingleTransactionCustom($this->transaction_table, $where);
            ...
            if ($amount > $before_balance) {
                ...
                if ($amount == 0) {
                    ...
                    if (!$success) {
                        ...
                        $after_balance = $this->queryPlayerBalance($player_id);
                    }
                }
            }
    ...
    private function getProviderIdentifierByGameCode($game_code)
    {
        if (!empty(explode(':', $game_code)[0])) {
            $provider_identifier = explode(':', $game_code)[0];
        } else {
    ...
    private function getGamePlatformId($provider_identifier)
    {
        if (!is_null($provider_identifier)) {
            // add new sub provider here
            switch ($provider_identifier) {
                case 'acceptance': //this is for acceptance test of main provider
                    $game_platform_id = $this->softswiss_provider_for_acceptance;
                    break;
                case 'softswiss':
                    $game_platform_id = SOFTSWISS_BGAMING_SEAMLESS_GAME_API;
                    break;
                case 'evolution':
                    $game_platform_id = SOFTSWISS_EVOLUTION_SEAMLESS_GAME_API;
                    break;
                case 'spribe':
                    $game_platform_id = SOFTSWISS_SPRIBE_SEAMLESS_GAME_API;
                    break;
                case 'thunderkick':
                    $game_platform_id = SOFTSWISS_THUNDERKICK_SEAMLESS_GAME_API;
                    break;
                case 'evoplay':
                    $game_platform_id = SOFTSWISS_EVOPLAY_SEAMLESS_GAME_API;
                    break;
                case 'bsg':
                    $game_platform_id = SOFTSWISS_BETSOFT_SEAMLESS_GAME_API;
                    break;
                case 'wazdan':
                    $game_platform_id = SOFTSWISS_WAZDAN_SEAMLESS_GAME_API;
                    break;
                case 'pushgaming':
                    $game_platform_id = SOFTSWISS_PUSHGAMING_SEAMLESS_GAME_API;
                    break;
                default:
                ...
                switch ($rule) {
                                ...
                                if (isset($request_params[$param])) {
                                    ...
                                    */
                                    ...
                                    if (empty($this->player_details)) {
                                        ...
                                        if (in_array($this->callback_method, $this->game_api_player_blocked_validate_api_methods)) {
                    ...
                    case 'unique':
                        ...
                        switch ($param) {
                            case 'action_id':
                                if (isset($request_params[$param])) {
                                    if ($this->original_seamless_wallet_transactions->isTransactionExistCustom($this->transaction_table, ['transaction_id' => $request_params[$param]])) {
    ...
    private function validateAction()
    {
        ...
        if ($this->callback_method == self::TRANSACTION_TYPE_ROLLBACK) {
            ...
            $is_already_processed_by_rollback = $this->original_seamless_wallet_transactions->isTransactionExistCustom($this->transaction_table, [
                'transaction_type' => self::TRANSACTION_TYPE_ROLLBACK,
                'game_username' => $game_username,
                'game_code' => $game_code,
                'round_id' => $round_id,
    ...
    private function preprocessRequestData($query_type, $transaction_data)
    {
        ...
        if ($query_type == self::INSERT) {
            ...
            $processed_data['md5_sum'] = $this->original_seamless_wallet_transactions->generateMD5SumOneRow($processed_data, self::MD5_FIELDS_FOR_ORIGINAL, self::MD5_FLOAT_AMOUNT_FIELDS);
            ...
            $processed_data['md5_sum'] = $this->original_seamless_wallet_transactions->generateMD5SumOneRow($processed_data, self::MD5_FIELDS_FOR_ORIGINAL_UPDATE, self::MD5_FLOAT_AMOUNT_FIELDS_UPDATE);
        }

        $processed_transaction_id = $this->original_seamless_wallet_transactions->insertOrUpdateTransactionData($this->transaction_table, $query_type, $processed_data, $field_id, $id, $update_with_result);
    ...
    private function setResponse($http_response, $operator_response = [])
    {
        ...
        if (!empty($this->processed_multiple_transaction) && is_array($this->processed_multiple_transaction)) {
            foreach ($this->processed_multiple_transaction as $processed_transaction_id) {
                ...
                $this->original_seamless_wallet_transactions->updateTransactionDataWithoutResult($this->transaction_table, $updated_data, 'id', $processed_transaction_id);
            }
        }
