<ol class="breadcrumb">
	<?php if ($day): ?>
		...
		<li><a href="/report_management/new_members"><?=lang('report.sum05')?></a></li>
		<li><a href="/report_management/new_members/<?=$year?>"><?=$year?></a></li>
		<li><a href="/report_management/new_members/<?=$year?>/<?=$month?>"><?=$month?></a></li>
		...
		<table class="table table-condensed table-bordered table-hover" id="summary-report" style="width:100%">
			<thead>
				<tr>
					...
					<?php if ($this->permissions->checkPermissions('affiliate') && !$this->utils->isEnabledFeature('hide_affiliate')) {?>
						<th data-data="affiliate_username"><?=lang('Affiliate Username')?></th>
					<?php }?>
...
<script type="text/javascript">
	...
	var affiliateUsername = function (data, type, row) {
		<?php if ($this->permissions->checkPermissions('affiliate') && !$this->utils->isEnabledFeature('hide_affiliate')) {?>
		if(data==null){
			return '';
		}else{
			return '<a href="/affiliate_management/userInformation/' + row.affiliateId + '">' + data + '</a>';
		}
		<?php }else{?>
			return data;
		<?php }?>
    ...
    $(document).ready(function(){
		...
		var ajax_uri = <?php if ($day): ?>
					'/api/new_members/<?=$year?>/<?=$month?>/<?=$day?>';
				<?php elseif ($month): ?>
					'/api/new_members/<?=$year?>/<?=$month?>';
				<?php elseif ($year): ?>
					'/api/new_members/<?=$year?>';
				<?php else: ?>
					'/api/new_members/null/null/null/<?=$dateFrom?>/<?=$dateTo?>';
				<?php endif ?>;
            ...
            buttons: [
                {
                    ...
                    filename:  '<?=lang('report.sum05').' '. $d->format('Y_m_d_H_i_s') . '_' . rand(1, 9999)?>'
                }            ],
			<?php } ?>
