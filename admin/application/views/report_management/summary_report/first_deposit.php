...
<ol class="breadcrumb">
	<?php if ($day): ?>
		...
		<li><a href="/report_management/first_deposit"><?= lang('report.sum07'); ?></a></li>
		<li><a href="/report_management/first_deposit/<?=$year?>"><?=$year?></a></li>
		<li><a href="/report_management/first_deposit/<?=$year?>/<?=$month?>"><?=$month?></a></li>
		...
		<table class="table table-condensed table-bordered table-hover" id="summary-report" style="width:100%">
			<thead>
				<tr>
					...
					<th data-data="date"><?=lang('First Deposit Date/Time')?></th>
			...
			<tfoot class="summary-report-ft hide">
                    <tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		var ajax_uri = <?php if ($day): ?>
					'/api/first_deposit/<?=$year?>/<?=$month?>/<?=$day?>';
				<?php elseif ($month): ?>
					'/api/first_deposit/<?=$year?>/<?=$month?>';
				<?php elseif ($year): ?>
					'/api/first_deposit/<?=$year?>';
				<?php else: ?>
					'/api/first_deposit/null/null/null/<?=$dateFrom?>/<?=$dateTo?>';
				<?php endif ?>
        ...
        var dataTable = $('#summary-report').DataTable( {
            ...
            buttons: [
                {
                    ...
                    filename:  '<?=lang('report.sum07').' '. $d->format('Y_m_d_H_i_s') . '_' . rand(1, 9999)?>'
                }
            ],
			<?php } ?>
			...
			{
				className: 'text-right amount-col',
				targets: [ 3 ]
			}],
			order: [[2, 'desc']],

			ajax: function (data, callback, settings) {
				$('.summary-report-ft').addClass('hide');
				$.post(ajax_uri, post_data, function(data) {
					callback(data);
				},'json').done(function(data, textStatus, jqXHR) {
					console.log('112.data', data)
					if( parseFloat(data.total.amount) > 0 ){
						$('.summary-report-ft').removeClass('hide');
					}
		...
		function calcSubtotal(){
			var subtotal = 0
			$('td.amount-col').each(function(){
				var amount = $(this).text().split(',').join('');
				subtotal += parseFloat(amount);
			})

			$('.summary-report-ft-col.amount-col').text( subtotal.currencyFormat(2, 3, ',', '.') );
		}
	 ...
	 */
	Number.prototype.currencyFormat = function(n, x, s, c) {
