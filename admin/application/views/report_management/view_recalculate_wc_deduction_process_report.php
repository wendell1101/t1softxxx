        ...
        <div class="panel-heading">
            <h4 class="panel-title">
                ...
                <span class="pull-right">
                    <a data-toggle="collapse" href="#collapseRecalculateWcDeductionProcessReport" class="btn btn-xs <?=$this->utils->getConfig('use_new_sbe_color') ? 'btn-primary' : 'btn-info'?> <?=$this->config->item('default_open_search_panel') ? '' : 'collapsed'?>"></a>
                </span>
            </h4>
        </div>

        <div id="collapseRecalculateWcDeductionProcessReport" class="panel-collapse <?=$this->config->item('default_open_search_panel') ? '' : 'collapse in'?>">
            <div class="panel-body">
                <div class="row">
                    <!-- Date -->
                    <div class="col-md-3 col-lg-3">
                        <label class="control-label"><?php echo lang('Date'); ?></label>
                        <input id="search_recaclculate_wc_deduction_process_date" class="form-control input-sm dateInput user-success" data-start="#by_date_from" data-end="#by_date_to" data-time="false" autocomplete="off">
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 p-t-15 text-right">
                        <input class="btn btn-sm btn_payall m-r-30 <?=$this->utils->getConfig('use_new_sbe_color') ? 'btn-portage' : 'btn-danger'?>" type="button" id="syncRecalculateWcDeductionProcessReport" value="<?php echo lang('Sync Report'); ?>"/>
...
<div class="panel panel-primary">
    <div class="panel-heading custom-ph">
        <h4 class="panel-title custom-pt"><i class="icon-bullhorn"></i> <?php echo lang('wc_dudection_process.recalculate.title'); ?></h4>
    </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#syncRecalculateWcDeductionProcessReport").click(function(){
            //confirm
            if(confirm("<?php echo lang('wc_dudection_process.comfirm_sync_report'); ?>")){
                $("#search-form").attr("method", "POST")
                    .attr("action", "<?php echo site_url('/vipsetting_management/sync_recalculate_wc_deduction_process_report'); ?>")
                    .submit();
            }
        });
        ...
        $('#btnResetFields').click(function() {
            ...
            $("#by_username").val("");
            $("#search_recaclculate_wc_deduction_process_date").val($("#by_date_from").val() +" to "+ $("#by_date_from").val());
        });
        ...
        $('#report_table').DataTable({
            ...
            buttons: [
                ...
                ,{
                    ...
                    action: function ( e, dt, node, config ) {
                        ...
                        <?php if($this->utils->isEnabledFeature('export_excel_on_queue')){?>
                            $("#_export_excel_queue_form").attr('action', site_url('/export_data/recalculate_withdraw_condition_deduction_report'));
            ...
            ajax: function (data, callback, settings) {
                ...
                $.post(base_url + "api/recalculate_wc_deduction_process_report", data, function(data) {
                    callback(data);
