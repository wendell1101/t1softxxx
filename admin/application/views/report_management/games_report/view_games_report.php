            ...
            <form id="form-filter" class="form-horizontal" method="GET" onsubmit="return validateForm();">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="affiliate_tag_list[]" id="affiliate_tag_list" multiple="multiple" class="form-control input-md">
                            <?php if (!empty($affiliate_tags)): ?>
                                <option value="notag" id="notag_in_affiliate_tag_list" <?=is_array($selected_affiliate_tags) && in_array('notag', $selected_affiliate_tags) ? "selected" : "" ?>><?=lang('player.tp12')?></option>
                                <?php foreach ($affiliate_tags as $tag): ?>
                                    <option value="<?=$tag['tagId']?>" <?=is_array($selected_affiliate_tags) && in_array($tag['tagId'], $selected_affiliate_tags) ? "selected" : "" ?> ><?=$tag['tagName']?></option>
                                <?php endforeach ?>
                            <?php endif ?>
                        </select>
                    </div>
                    <?php if ($this->utils->isIncludedInGamesReportSearch('storeCode')) { ?>
                    <div class="col-md-2">
                            <label for="agency_code" class="control-label"><?= lang('Agency Code'); ?></label>
                            <input type="text" class="form-control input-sm" id="agency_code" name="agency_code" value="<?= $conditions['agency_code']; ?>" />
                    </div>
                    <?php } ?>
                </div>
                <div class="row">
...
<script type="text/javascript">
    ...
    function renderGameTable(data){
        ...
        var playerTotalBetsPerGame = data.player_total_bets_per_game,
            ...
            th = "<th>" + message.playerId + "</th><th>" + message.playerUsername + "</th><th>" + message.playerTag + "</th>",
            rowHeaders=[];
        ...
        rowHeaders.push(message.playerTag);
        ...
        for (var i = 0; i < len; i++) {
            ...
            rowObj['player_tag'] = playerRow.player_tag.replace(/<\/?[^>]+(>|$)/g, "");

            tds += '<td>' + playerRow.player_id + '</td>';
            tds += '<td>' + playerRow.username + '</td>';
            tds += '<td>' + playerRow.player_tag + '</td>'; //OGP-25040
    ...
    } // EOF function renderGameTable(data){...
...
//---------------------Multi select search start----------------------------
    ...
    }); // EOF $('.game-types-all').each(function(index, value) {...
   ...
   //loop through checkboxes and attach each event listener
    ...
    }); // EOF $('.game-types').each(function(index, value) {...
   ...
   $('#tag_list,#affiliate_tag_list').multiselect({
    ...
    }); // EOF $('#tag_list,#affiliate_tag_list').multiselect({...
       ...
       var dataTable = $('#myTable').DataTable({
            ...
            ajax: function (data, callback, settings) {
               ...
               // formData.push({name: 'game_type', value: loadParams.gameTypeId});
                ...
                var _ajax = $.post(base_url + "api/gameReports", data, function(data) {
                    console.log(data)
        ...
        function set_aff_agent_by_group_by() {
            ...
            switch (val_group_by) {
                case 'aff_id' :
                    ...
                    break;
                // case 'agent_id' :
                //     $('#affiliate_agent').val(3);   // 3 - under agent only
                //     set_disabled_but('#affiliate_agent', 3);
                //     break;
                default :
            }
        }
