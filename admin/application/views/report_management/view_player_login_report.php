    ...
    <div class="panel-heading">
        <h4 class="panel-title">
            ...
            <span class="pull-right">
                <a data-toggle="collapse" href="#viewPlayerLoginReport" class="btn btn-xs btn-primary <?=$this->config->item('default_open_search_panel') ? '' : 'collapsed'?>"></a>
            </span>
        </h4>
    </div>
    <div id="viewPlayerLoginReport" class="panel-collapse <?=$this->config->item('default_open_search_panel') ? '' : 'collapse in'?>">
        <div class="panel-body">
            <form id="search-form" action="<?= site_url('/report_management/viewPlayerLoginReport'); ?>" method="get">
                <div class="row">
                    <!-- Date -->
                    <div class="form-group col-md-3 col-lg-3">
                        <label class="control-label">
                            <?= lang('player_login_report.datetime'); ?>:
                        </label>
                   ...
                   <!-- login result -->
                   <div class="form-group col-md-3 col-lg-3">
                       <label for="by_login_result" class="control-label"><?=lang('player_login_report.login_result');?> </label>
                       <?=form_dropdown('by_login_result', $login_result, $conditions['by_login_result'], 'class="form-control input-sm group-reset"'); ?>
                   </div>
                   <!-- player status -->
                   <div class="form-group col-md-3 col-lg-3">
                       <label for="by_player_status" class="control-label"><?=lang('player_login_report.player_status');?> </label>
                       <?=form_dropdown('by_player_status', $player_status, $conditions['by_player_status'], 'class="form-control input-sm group-reset"'); ?>
                   </div>
                </div>
                ...
                <div class="row">
                    <!-- login from -->
                    <div class="form-group col-md-3 col-lg-3">
                        <label for="by_login_from" class="control-label"><?=lang('player_login_report.login_from');?> </label>
                        <?=form_dropdown('by_login_from', $login_from, $conditions['by_login_from'], 'class="form-control input-sm group-reset"'); ?>
                    </div>
                    <!-- login ip -->
                    <div class="form-group col-md-3 col-lg-3">
                    ...
                    <!-- Client End -->
                    <div class="form-group col-md-3 col-lg-3">
                        <label for="by_client_end" class="control-label"><?=lang('Client End');?> </label>
                        <?=form_dropdown('by_client_end', $client_end, $conditions['by_client_end'], 'class="form-control input-sm group-reset"'); ?>
                    </div>
                    <!-- Client End -->
                    <div class="form-group col-md-3 col-lg-3">
                        <label for="group_by" class="control-label"><?=lang('report.g14')?> </label>
                        <?=form_dropdown('group_by', $group_by, $conditions['group_by'], 'class="form-control input-sm group-reset"'); ?>
                    </div>
                </div>
    ...
    $(document).ready(function(){
        ...
        <?php } ?>
                ...
                ,{
                    ...
                    action: function ( e, dt, node, config ) {
                       ...
                       var d = {'extra_search': form_params, 'export_format': 'csv', 'export_type': 'queue',
                            ...
                            $("#_export_excel_queue_form").attr('action', site_url('/export_data/playerLoginReport'));
            ...
            ajax: function (data, callback, settings) {
                ...
                $.post(base_url + "api/playerLoginReport", data, function(data) {
                    callback(data);
