...
foreach ($player_report as $key => $value) {
		...
		?>
									<tr>
										<td></td>
										<td class="input-sm">
											<a href="<?=BASEURL . 'report_management/viewPlayerReportToday/' . urlencode($date)?>">
												<?=$value['date']?>
											</a>
										</td>
										<td class="input-sm"><?=$value['total_player']?></td>
										<td class="input-sm">
											<?php if ($value['registered_player'] != 0) {?>
												<a href="<?=BASEURL . 'report_management/viewRegisteredPlayerToday/' . urlencode($date)?>">
													<?=$value['registered_player']?>
												</a>
											<?php
} else {
			echo $value['registered_player'];
		}
		?>
										</td>
