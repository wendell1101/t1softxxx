    ...
    <div class="panel-heading">
        <h4 class="panel-title">
            ...
            <span class="pull-right">
                <a data-toggle="collapse" href="#collapseAchieveThresholdReport" class="btn btn-xs btn-primary <?=$this->config->item('default_open_search_panel') ? '' : 'collapsed'?>"></a>
            </span>
        </h4>
    </div>
    <div id="collapseAchieveThresholdReport" class="panel-collapse <?=$this->config->item('default_open_search_panel') ? '' : 'collapse in'?>">
        <div class="panel-body">
            <form id="search-form" action="<?= site_url('/report_management/view_player_achieve_threshold_report'); ?>" method="get">
                <div class="row">
                    <!-- Date -->
                    <div class="form-group col-md-3 col-lg-3">
                        <label class="control-label">
                            <?= lang('sys.achieve.threshold.datetime'); ?>:
                        </label>
                   ...
                   <div class="form-group col-md-2 col-lg-2">
                       <label for="status" class="control-label"><?=lang('sys.achieve.threshold.transaction.type');?> </label>
    ...
    <div class="panel-body">
        <div class="table-responsive">
            <table class="table table-striped table-hover table-condensed" id="result_table">
                <thead>
                    <tr>
                        <th><?=lang("sys.achieve.threshold.datetime")?></th>
                        <th><?=lang("Username")?></th>
                        <th><?=lang("sys.achieve.threshold.transaction.type")?></th>
                        <th><?=lang("sys.achieve.threshold.threshold.amount")?></th>
                        <th><?=lang("sys.achieve.threshold.achieve.amount")?></th>
                    </tr>
                </thead>
                <tbody></tbody>
                <tfoot>
    ...
    $(document).ready(function(){
        ...
        var dataTable = $('#result_table').DataTable({
            ...
            buttons: [
                ...
                <?php if( $this->permissions->checkPermissions('show_player_deposit_withdrawal_achieve_threshold') ){ ?>
                ,{
                    ...
                    action: function ( e, dt, node, config ) {
                       ...
                       var d = {'extra_search': form_params, 'export_format': 'csv', 'export_type': 'queue',
                            ...
                            $("#_export_excel_queue_form").attr('action', site_url('/export_data/achieveThresholdReport'));
            ...
            ajax: function (data, callback, settings) {
                ...
                $.post(base_url + "api/achieveThresholdReport", data, function(data) {
                    callback(data);
