    ...
    <div class="col-md-12">
        <div class="panel panel-primary">
            <div class="panel-heading custom-ph">
                <h4 class="panel-title custom-pt pull-left"><i class="icon-settings"></i> <?= $game_type_lang ?>&nbsp;&nbsp;-> <?= lang('cms.navigation.gameProviders') ?></h4>
                <div class="pull-right">
                    <a href="/cms_management/refreshNavigationGamePlatform/<?= $game_type_id ?>" class="btn btn-info btn-xs">
                        <span class="glyphicon glyphicon-refresh"></span> &nbsp; <?= lang('cms.navigation.gameProvider.refresh') ?>
                    </a>
                </div>
            ...
            <div class="panel-body m-t-20">
                <div class="col-md-12">
                    <table class="table table-condensed table-bordered no-footer table-striped" id="DataTables_Table_0" role="grid">
                        ...
                        <thead>
                        <tr role="row">
                            <th class="sorting_disabled" rowspan="1" colspan="1"><?= lang('API ID') ?></th>
                        ...
                        <tbody>
                            <?php if(!empty($game_platforms)){ ?>
                                <?php foreach($game_platforms as $key => $game_platform){ ?>
                                <tr role="row" class="odd">
                                    <td><?= $game_platform['game_platform_id'] ?></td>
                                    <td><?= $game_platform['game_platform_lang']['en'] ?></td>
                                    <td><?= $game_platform['game_platform_lang']['cn'] ?></td>
                                    <td><?= $game_platform['game_platform_lang']['id'] ?></td>
                                    <td><?= $game_platform['game_platform_lang']['vt'] ?></td>
                                    <td><?= $game_platform['game_platform_lang']['kr'] ?></td>
                                    <td><?= !empty($game_platform['game_platform_lang']['th']) ? $game_platform['game_platform_lang']['th'] : $game_platform['game_platform_lang']['en'] ?></td>
                                    <td><?= $game_platform['order'] ?></td>
                                    <td>
                                        <?php if($game_platform['status'] != 0){ ?>
                                    ...
                                    <td>
                                        <a href="javascript:void(0)" title="Edit" class="modify-game-type" data-id='<?= $game_platform['id'] ?>'>
                                            <span class="glyphicon glyphicon-edit"></span>
                                        </a>
                                        <?php if(!empty($game_platform['icon'])){?>
                                        <a href="javascript:void(0)" title="View Providers" data-icon="<?= site_url('resources/images/cms_game_platforms/' . $game_platform['icon']) ?>" class="view-game-platform_icon">
                            ...
                            <?php } ?>
                            <?php }else{ ?>
                                <td colspan="10" class="text-center"><?= lang('cms.navigation.noGamePlatformAdded') ?></td>
                            <?php } ?>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="modal-content">
                <form id="modify-game-type" action="/cms_management/postModifyGamePlatform" method="POST" enctype="multipart/form-data">
                    <div class="modal-header modal-header-info">
                        ...
                        <h4 class="modal-title m-5"><?= lang('cms.navigation.editGamePlatform') ?></h4>
                    </div>
...
<script>
    $( document ).ready(function() {
        ...
        function showFormModal(id) {
            ...
            var url = "/cms_management/viewModifyGamePlatform/" + id;
