<?php
/**
 *   filename:   edit_structure.php
 *   date:       2016-05-03
 *   @brief:     view for edit and update a structure 
 */
?>

<!-- form edit-structure {{{1 -->    
<form method="POST" id="agency_main_form" 
    action="<?=site_url('agency_management/verify_update_structure')?>" accept-charset="utf-8">
    <input type="hidden" name="structure_id" value="<?=$conditions['structure_id']?>" />
    <div class="panel panel-primary ">
        <!-- panel heading of edit-structure {{{2 -->    
        <div class="panel-heading">
            <h4 class="panel-title pull-left">
                <i class="glyphicon glyphicon-list-alt"></i> 
                <?=lang('Edit Agent Template');?> 
            </h4>
            <div class="pull-right">Fields with (<font style="color:red;">*</font>) are required.</div>
            <div class="clearfix"></div>
        </div> <!-- panel heading of edit-structure }}}2 -->    

        <!-- panel body of edit-structure  {{{2 -->    
        <div class="panel panel-body" id="edit-structure_panel_body">
            <!-- Basic Info {{{3 -->    
            <div class="col-md-12">
                ...
                <div class="col-md-3 fields">
                    ...
                    value="<?=$conditions['structure_name'];?>" data-toggle="tooltip" 
                    title="<?=lang('Agent Template Name');?>">
            ...
            <div class="col-md-12">
                ...
                <fieldset>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                class="form-control " value="<?=set_value('allowed_level', $conditions['allowed_level']);?>" 
                    ...
                    <!--  leve names {{{4 -->
                    <?php $level_cnt= $conditions['allowed_level']; ?>
                    <?php for ($i = 0; $i < $level_cnt; $i++) { ?>
                    <?php $level_id = 'old_level_' . $i; ?>
                    <div class="col-md-12" id="<?=$level_id?>" name="<?=$level_id?>" >
                        <?php if($i < $level_cnt){echo 'Level '.$i.': '. $conditions['allowed_level_names'][$i];}?>
                    </div> 
                    <?php } ?>
                    <?php for ($i = 0; $i < $max_allowed_level; $i++) { ?>
                    <?php $val = $conditions['allowed_level_names'][$i]; ?>
                    <?php $level_id = 'allowed_level_' . $i; ?>
                    <?php $agent_level_id = 'agent_level_' . $i; ?>
                    <input type="hidden" id="<?=$agent_level_id?>" name="<?=$agent_level_id?>" value="<?=$val?>">
                    <div class="col-md-12" id="<?=$level_id?>" name="<?=$level_id?>" >
                        <?php if($i < $level_cnt){echo 'Level '.$i.': '. $val;}?>
                    </div> 
                    <?php } ?>
                    <!-- Level names}}}4 -->
                </fieldset>
            ...
            <div class="row">
                ...
                <div class="col-md-6 col-lg-6" style="padding: 10px;">
                    <?php $reset_url=site_url('agency_management/edit_structure/'.$conditions['structure_id']);?>
            ...
            <!-- button row }}}3 -->
        </div> <!-- panel body of edit-structure  }}}2 -->    
    </div>
</form> <!-- end of form edit-structure }}}1 -->    
...
$(document).ready(function(){
    ...
    var ajax_url = "<?=site_url('agency_management/edit_agent_validation_ajax')?>";
    var labels = '<?=json_encode($labels)?>';
    var fields = '<?=json_encode($fields)?>';
    //agency_form_validation(ajax_url, fields, labels);
    agency_form_validation_edit(ajax_url, fields, labels);
