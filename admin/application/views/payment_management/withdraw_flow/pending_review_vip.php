...
<style type="text/css">
    select#custom-tags-list option:disabled {
        color: #B7B4B6;
    }
</style>

<td colspan="1">
    <input type="checkbox" id="pending_custom" type="checkbox" name="pending_custom" value="1" <?=$setting['pendingCustom']['enabled'] ? 'checked' : ''?> />
    <label><?=lang('pay.pendingreviewcustom')?></label>
</td>
<td>
    <div id="player-custom-tagged-info">
        <div id="recorded-custom-tag" class="col col-md-10 no-gutter">
            <?=lang('player.tp12')?>
        </div>
        <?php if ($this->permissions->checkPermissions('edit_player_tag')): ?>
            <button style="margin-bottom:2px;" type="button" id="add-player-custom-tag" title="<?=lang('con.plm72')?>" class="btn btn-info btn-xs pull-right"><i class="glyphicon glyphicon-edit"></i><?=lang('con.plm72')?></button>
        <?php endif ?>
    </div>
    <div id="player-custom-tagged-form">
        <?php if ($this->permissions->checkPermissions('edit_player_tag')): ?>
            <span id="success-glyph-custom-tag" class="glyphicon glyphicon-ok text-success"></span>
            <span id="ajax-status-custom-tag"><?=lang('text.loading')?></span>
            <button style="margin-bottom:2px;" type="button" id="save-player-custom-tag" title="<?=lang('player.ui63')?>" class="btn btn-success btn-xs pull-right"><i class="glyphicon glyphicon-floppy-disk"></i><?=lang('player.ui63')?></button>
            <button style="margin-bottom:2px;" type="button" id="cancel-player-custom-tag" title="<?=lang('player.ui65')?>" class="btn btn-danger btn-xs pull-right"><i class="glyphicon glyphicon-remove"></i><?=lang('player.ui65')?></button>
            <input type="hidden" id="custom-tags-options" name="custom-tags-options" sbe-ui-toogle="tagsinput" data-freeInput="false" />
            <select id="custom-tags-list"  name="custom-tags-list" class="form-control input-sm"></select>
        <?php endif ?>
    </div>
</td>
...
<script type="text/javascript">
    ...
    var PLAYER_TAGGING = (function () {
        var playerTaggedInfo = $('#player-custom-tagged-info'),
            playerTaggedForm = $('#player-custom-tagged-form'),
            addTag = $('#add-player-custom-tag'),
            playerTaggedInput = $('#custom-tags-options'),
            tagList = $('#custom-tags-list'),
            save = $("#save-player-custom-tag"),
            cancel = $("#cancel-player-custom-tag"),
            recordedTag = $("#recorded-custom-tag"),
            ajaxStatus = $("#ajax-status-custom-tag"),
            GET_ALLTAGS_URL = '<?php echo site_url('payment_management/getAllTagsForPendingReview/true') ?>/',
            PENDING_REQUEST_TAG_URL = '<?php echo site_url('payment_management/pendingRequestcustomTags') ?>',
            successGlyph = $("#success-glyph-custom-tag"),
            ...
            has_playerTagged = "<?=($customTaggedStatus) ? 0 : 1?>";
        ...
        function getAllTags() {
            $.ajax({
                ...
                for (var i = 0; i < tagsLength; i++) {
                    ...
                    if($.inArray(tags[i].tagId, chosenTag) >= 0){
