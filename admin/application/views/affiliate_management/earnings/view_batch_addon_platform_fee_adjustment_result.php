...
<div class="container">
    <div class="panel panel-primary">
        <div class="panel-heading">
            <h4 class="panel-title"><span style="font-weight: bold;"><?php echo lang('Batch Addon Platform Fee Adjustment Result');?></span></h4>
        </div>
    ...
    <div class="row download_link" style="display:none;">
        <div class="col-md-12">
            <?php echo lang('Failed List'); ?>: <a href="" class="download_url"></a>
        </div>
    </div>
</div>
...
// var failedResult = $('#failedResult').DataTable({
//     searching: true,
//     autoWidth: false,
//     dom:"<'panel-body'<'pull-right'f>><'dt-information-summary1 text-info pull-left' i>t<'panel-body'<'pull-right'p><'dt-information-summary2 text-info pull-left' i>>",
//     stateSave: false,
    ...
    if(data['queue_result']){
        ...
        if(queue_result.progress === undefined){
           ...
           $('.progresslabel').html("");
            ...
            $('#successCount').html(addCommas(queue_result.successCount));
            $('#failCount').html(addCommas(queue_result.failCount));
            $('#processedRows').html(addCommas(queue_result.processedRows));
            $('#totalRows').html(addCommas(queue_result.totalCount));
            // var failedResultData = queue_result.failedList
            // var len = failedResultData.length, i = 0;
            // failedResult.clear().draw();
            // for (var i=0; i<len; i++) {
            //   var failedRow =  [failedResultData[i].username,failedResultData[i].reason];
            //   failedResult.row.add(failedRow).draw(false);
            // }
            var failCount = parseInt(queue_result.failCount);
            if(failCount > 0 && data.done){
                var url=queue_result['failed_log_filepath'];
