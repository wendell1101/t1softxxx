...
<div class="panel panel-primary">
    <div class="panel-heading custom-ph">
        <h4 class="panel-title custom-pt">
            <i class="icon-note"></i> <?=lang('cms.promoPlayerList');?>
                        ...
                        <table class="table table-bordered table-hover dataTable" id="my_table" style="margin: 0px 0 0 0; width: 100%;">
                            <thead>
                                <tr>
                                    ...
                                    <th><?=lang('cms.processedBy');?></th>
                                    <th><?=lang('cms.applicationStatus');?></th>
	...
	foreach ($promoAppList as $row) {
		?>
                                    <tr>
                                        ...
                                        <td><a href="<?=site_url('player_management/userInformation/' . $row['playerId'])?>" data-toggle='tooltip' data-original-title='<?=lang("cms.checkPlayerDetails")?>' data-placement="right"><?=$row['username']?></a></td>
			...
			'screenx' => '0',
			'screeny' => '0');?>
                                            <?='<span data-toggle="tooltip" title="' . lang('cms.checkCmsPromo') . '" data-placement="right">' . anchor_popup(site_url('cms_management/viewPromoDetails/' . $row['promoCmsSettingId']), $row['promoTitle'], $atts_popup) . '</span>'?></td>
                                        <td>
                                        ...
                                        <td><?=$row['dateApply']?></td>
                                        <td><?=$row['dateProcessed'] == '' ? '<i class="help-block">' . lang("lang.norecyet") . '<i/>' : $row['dateProcessed']?></td>
                                        <td><?=$row['processedBy'] == '' ? '<i class="help-block">' . lang("lang.norecyet") . '<i/>' : $row['processedBy']?></td>
                                        <td>
                                            <?php
if ($row['transactionStatus'] == 0) {
			echo lang("cms.applicationStatusRequest");
		} elseif ($row['transactionStatus'] == 1) {
			echo lang("cms.applicationStatusApproved");
		} elseif ($row['transactionStatus'] == 2) {
			echo lang("cms.applicationStatusDeclined");
		} elseif ($row['transactionStatus'] == 3) {
			echo lang("cms.applicationStatusCancelled");
		}
		?>
                                        </td>
                                        <td style="width:12%;">
                                            <?php if ($row['transactionStatus'] == 1) {?>
                                                    <a class="btn btn-xs btn-danger" href="<?=site_url('marketing_management/cancelPromoApplication/' . $row['playerpromoId'])?>"><?=lang('lang.cancel');?></a>
                                            <?php } elseif ($row['transactionStatus'] == 0) {?>
                                                    <a class="btn btn-xs btn-success" href="<?=site_url('marketing_management/approve_player_promo/' . $row['playerpromoId'])?>"><?=lang('lang.approve');?></a>
                                                    <a href='#' class="btn btn-xs btn-danger" data-toggle='modal' data-target='#promoCancel' onclick='viewPromoDeclineForm(<?=$row['playerpromoId']?>)'>
                                                        <?=lang('lang.decline');?>
                                                    </a>
                                            <?php } elseif ($row['transactionStatus'] == 2 || $row['transactionStatus'] == 3) {?>
                                                <i><?=lang("cms.noAvailableAction");?></i>
                                            <?php }
		?>
                                        </td>
                                    </tr>
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <form method="post" action="<?=site_url('marketing_management/declinePromoApplicationRequest')?>">
                            <?=lang('cms.declinePromoAppReason');?>
